...
public class ReactionDescription implements java.io.Serializable, cbit.util.Cacheable{
	...
	private cbit.util.KeyValue vcellRXID = null;
	//
...
public ReactionDescription(String argReactionName,String argReactionType,cbit.util.KeyValue argVCellRXID){
	...
	if(	!argReactionType.equals(this.UNKNOWN_REACTIONTYPE) &&
		!argReactionType.equals(ReactionStep.DATABASE_REACTTYPE_FLUX) &&
		!argReactionType.equals(ReactionStep.DATABASE_REACTTYPE_SIMPLE)){
...
public void addReactionElement(SpeciesDescription argReactElement,String originalSpeciesContextName,int argStoich,char argType){
	...
	if(argType == RX_ELEMENT_PRODUCT || argType == RX_ELEMENT_REACTANT){
		...
		if(!getReactionType().equals(UNKNOWN_REACTIONTYPE) && !getReactionType().equals(ReactionStep.DATABASE_REACTTYPE_SIMPLE)){
	...
	if(argType == RX_ELEMENT_FLUX && argStoich != 1){
		...
		if(!getReactionType().equals(UNKNOWN_REACTIONTYPE) && !getReactionType().equals(ReactionStep.DATABASE_REACTTYPE_FLUX)){
...
public cbit.util.KeyValue getVCellRXID(){
	return vcellRXID;
}
 ...
 */
public ReactionCanvasDisplaySpec toReactionCanvasDisplaySpec() {
	...
	ReactionCanvasDisplaySpec rcds = new ReactionCanvasDisplaySpec(left.toString(),right.toString(),center.toString(),null,ReactionCanvasDisplaySpec.ARROW_RIGHT);
	return rcds;
}
