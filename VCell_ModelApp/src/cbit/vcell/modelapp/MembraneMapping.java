...
public MembraneMapping(cbit.vcell.model.Membrane membrane, SimulationContext argSimulationContext) {
	...
	try {
		setParameters(new StructureMappingParameter[] {
						new StructureMappingParameter(getInitialVoltageName(), ExpressionFactory.createExpression(0.0), ROLE_InitialVoltage,VCUnitDefinition.UNIT_mV),
						new StructureMappingParameter(DefaultNames[ROLE_SpecificCapacitance], ExpressionFactory.createExpression(1.0), ROLE_SpecificCapacitance,VCUnitDefinition.UNIT_pF_per_um2),
						new StructureMappingParameter(DefaultNames[ROLE_SurfaceToVolumeRatio], ExpressionFactory.createExpression(1.0), ROLE_SurfaceToVolumeRatio,VCUnitDefinition.UNIT_per_um),
						new StructureMappingParameter(DefaultNames[ROLE_VolumeFraction], ExpressionFactory.createExpression(0.2), ROLE_VolumeFraction,VCUnitDefinition.UNIT_DIMENSIONLESS),
...
public IExpression getTotalVolumeCorrection(SimulationContext simulationContext) throws ExpressionException {
	...
	if (getResolved(simulationContext)) {
		return ExpressionFactory.createExpression(1.0);
//		throw new Exception("volume correction for a resolved membrane is meaningless, but maybe a 1.0 would be harmless");
		...
		//
		IExpression exp = ExpressionFactory.createExpression(simulationContext.getNameScope().getSymbolName(getSurfaceToVolumeParameter())+
