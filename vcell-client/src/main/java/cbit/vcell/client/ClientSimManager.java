...
public class ClientSimManager implements java.beans.PropertyChangeListener {
    ...
    public static class LocalVCSimulationDataIdentifier extends VCSimulationDataIdentifier implements LocalVCDataIdentifier {
        ...
        public LocalVCSimulationDataIdentifier(VCSimulationIdentifier vcSimID, int jobIndex, File localDir) {
            ...
            this.localSimDir = localDir;
        }

        public File getLocalDirectory() {
            return this.localSimDir;
        }
    }
    ...
    private final SimulationStatusHash simHash = new SimulationStatusHash();
    ...
    public ClientSimManager(DocumentWindowManager documentWindowManager, SimulationWorkspace simWorkspace) {
        ...
        this.getSimWorkspace().addPropertyChangeListener(this);
        this.initHash(this.getSimWorkspace().getSimulations());
    }

    public User getLoggedInUser() {
        return this.getDocumentWindowManager().getUser();
    }

    DocumentWindowManager getDocumentWindowManager() {
        return this.documentWindowManager;
    }

    public UserPreferences getUserPreferences() {
        UserPreferences up = this.documentWindowManager.getUserPreferences();
        return up;
    }

    public SimulationStatus getSimulationStatus(Simulation simulation) {
        SimulationStatus cachedSimStatus = this.simHash.getSimulationStatus(simulation);
    ...
    public SimulationWorkspace getSimWorkspace() {
        return this.simWorkspace;
    }

    public void preloadSimulationStatus(Simulation[] simulations) {
        this.initHash(simulations);
    }

    private void initHash(Simulation[] simulations) {
        this.simHash.changeSimulationInstances(simulations);
        if (simulations != null) {
            for (int i = 0; i < simulations.length; i++) {
                SimulationStatus simStatus = this.simHash.getSimulationStatus(simulations[i]);
                if (simStatus == null || simStatus.isUnknown()) {
                    // try to get status from server
                    simStatus = this.getDocumentWindowManager().getRequestManager().getServerSimulationStatus(simulations[i].getSimulationInfo());
                    if (simStatus != null) {
                        this.simHash.setSimulationStatus(simulations[i], simStatus);
                    } else {
                        this.simHash.setSimulationStatus(simulations[i], SimulationStatus.newNeverRan(simulations[i].getScanCount()));
                    }
                }
            }
        }
    ...
    public void propertyChange(java.beans.PropertyChangeEvent evt) {
        if (evt.getPropertyName().equals("simulations")) {
            this.simHash.changeSimulationInstances((Simulation[]) evt.getNewValue());
        }
        if (evt.getPropertyName().equals("simulationOwner")) {
            this.initHash(((SimulationOwner) evt.getNewValue()).getSimulations());
        }
    }


    public void runSimulations(Simulation[] simulations) {
        this.runSimulations(simulations, null);
    }

    public void runSimulations(Simulation[] simulations, AsynchClientTask[] endTasks) {
        this.getDocumentWindowManager().getRequestManager().runSimulations(this, simulations, endTasks);
    }
    ...
    public void showSimulationResults(OutputContext outputContext, Simulation[] simulations, ViewerType viewerType) {
        ...
        for (Simulation simulation : simulations) {
            if (simulation.getSimulationInfo() != null && this.getSimulationStatus(simulation).getHasData()) {
                v.add(simulation);
            }
        }
        ...
        AsynchClientTask[] taskArray = this.showSimulationResults0(false, viewerType);
        ClientTaskDispatcher.dispatch(this.getDocumentWindowManager().getComponent(), hashTable, taskArray, false, true, null);
    }
    ...
    public void getBatchSimulationsResults(OutputContext outputContext, Simulation simulation, Component requester) throws java.beans.PropertyVetoException {
        ...
        SimulationOwner simOwner = this.getSimWorkspace().getSimulationOwner();
            ...
            public void run(Hashtable<String, Object> hashTable) throws Exception {
                ...
                for (Simulation simCandidate : allSims) {
                    if (simCandidate.getName().startsWith(namePrefix) && simCandidate.getName().contains("_bat_")) {
                        ...
                        try {
                            simData = ClientSimManager.this.importBatchSimulation(outputContext, simCandidate);
...
//						double[] res = osrs.extractColumn(4);
        ...
        ClientTaskDispatcher.dispatch(this.documentWindowManager.getComponent(), new Hashtable<String, Object>(), taskArray, false, false, null);
    }
    ...
    private ODESimData importBatchSimulation(OutputContext outputContext, Simulation sim) throws DataAccessException, RemoteProxyException, ExpressionException {
        ...
        ODEDataManager dm = (ODEDataManager) this.getDocumentWindowManager().getRequestManager().getDataManager(outputContext, vcSimulationDataIdentifier, false);
    ...
    private AsynchClientTask[] showSimulationResults0(final boolean isLocal, final ViewerType viewerType) {
        ...
        final DocumentWindowManager documentWindowManager = this.getDocumentWindowManager();
        AsynchClientTask retrieveResultsTask = new AsynchClientTask("Retrieving results", AsynchClientTask.TASKTYPE_NONSWING_BLOCKING) {
            ...
            public void run(Hashtable<String, Object> hashTable) throws Exception {
                ...
                for (int i = 0; i < simsToShow.length; i++) {
                    ...
                    if (simWindow != null) {
                        ...
                        this.setFinalWindow(hashTable, childWindow);
                        ...
                        if (viewerController != null && ex == null) { // no failure
                            ...
                            ClientSimManager.this.getSimWorkspace().getSimulationOwner().getOutputFunctionContext().addPropertyChangeListener(viewerController);
                            ...
                            viewer.setSimulationModelInfo(new SimulationWorkspaceModelInfo(ClientSimManager.this.getSimWorkspace().getSimulationOwner(), sim.getName()));
                            viewer.setDataViewerManager(documentWindowManager);

                            SimulationWindow newWindow = new SimulationWindow(vcSimulationIdentifier, sim, ClientSimManager.this.getSimWorkspace().getSimulationOwner(), viewer);
                            GeneralGuiUtils.addCloseWindowKeyboardAction(newWindow.getDataViewer());
                            documentWindowManager.addResultsFrame(newWindow);
                            this.setFinalWindow(hashTable, viewer);
    ...
    public void showSimulationStatusDetails(Simulation[] simulations) {
        ...
        for (Simulation simulation : simulations) {
            ...
            ssdp.setSimulationStatusDetails(new SimulationStatusDetails(this.getSimWorkspace(), simulation));
            DialogUtils.showComponentCloseDialog(this.getDocumentWindowManager().getComponent(), ssdp, "Simulation Status Details");
            ssdp.setSimulationStatusDetails(null);
        }
    }

    public void stopSimulations(Simulation[] simulations) {
        this.getDocumentWindowManager().getRequestManager().stopSimulations(this, simulations);
    }
    ...
    void updateStatusFromServer(Simulation simulation) {
        ...
        // get cached status
        //
        SimulationStatus oldStatus = this.getSimulationStatus(simulation);
        SimulationStatus serverStatus = this.getDocumentWindowManager().getRequestManager().getServerSimulationStatus(simulation.getSimulationInfo());
        ...
        // update cache
        this.simHash.setSimulationStatus(simulation, newStatus);

        System.out.println("---ClientSimManager.updateStatusFromServer[newStatus=" + newStatus + "], simulation=" + simulation);
        if (oldStatus != newStatus) {
            int simIndex = this.getSimWorkspace().getSimulationIndex(simulation);
            this.getSimWorkspace().firePropertyChange(SimulationWorkspace.PROPERTY_NAME_SIMULATION_STATUS, -1, simIndex);
        }
    }
    ...
    public void updateStatusFromStartRequest(final Simulation simulation, SimulationStatus newStatusFromServer) {
        ...
        this.simHash.setSimulationStatus(simulation, newStatusFromServer);
        int simIndex = this.getSimWorkspace().getSimulationIndex(simulation);
        this.getSimWorkspace().firePropertyChange(SimulationWorkspace.PROPERTY_NAME_SIMULATION_STATUS, -1, simIndex);
    }
    ...
    public void runSmoldynParticleView(final Simulation originalSimulation) {
        SimulationOwner simulationOwner = this.simWorkspace.getSimulationOwner();
        Collection<AsynchClientTask> tasks;
        if (simulationOwner instanceof SimulationContext) {
            tasks = ClientRequestManager.updateMath(this.documentWindowManager.getComponent(), (SimulationContext) simulationOwner, false, NetworkGenerationRequirements.ComputeFullStandardTimeout);
        } else {
            tasks = new ArrayList<>();
        }
        ...
        AsynchClientTask pv = new AsynchClientTask("starting particle view", AsynchClientTask.TASKTYPE_NONSWING_BLOCKING) {
            ...
            public void run(Hashtable<String, Object> hashTable) throws Exception {
                ...
                this.getClientTaskStatusSupport().addProgressDialogListener(new ProgressDialogListener() {
        ...
        tasks.add(pv);
        ClientTaskDispatcher.dispatchColl(this.documentWindowManager.getComponent(), new Hashtable<String, Object>(), tasks, false, true, null);
    }
    ...
    public void runQuickSimulation(final Simulation originalSimulation, ViewerType viewerType) {
        ...
        final SimulationOwner simulationOwner = this.simWorkspace.getSimulationOwner();

        // ----------- update math if it is from biomodel (simulationContext)
        if (simulationOwner instanceof SimulationContext) {
            Collection<AsynchClientTask> ut = ClientRequestManager.updateMath(this.documentWindowManager.getComponent(), ((SimulationContext) simulationOwner), false, NetworkGenerationRequirements.ComputeFullStandardTimeout);
            taskList.addAll(ut);
        }
        ...
        if (originalSimulation.getScanCount() > 1) {
            try {
                String simultaneousSims = DialogUtils.showInputDialog0(this.getDocumentWindowManager().getComponent(), "Local multi-scan simulation, enter maximum simulataneous sims to run at once", "1");
        ...
        AsynchClientTask runSimTask = new AsynchClientTask("running simulation", AsynchClientTask.TASKTYPE_NONSWING_BLOCKING) {
            ...
            public void run(Hashtable<String, Object> hashTable) throws Exception {
                ...
                while (true) {
                    ...
                    if (this.getClientTaskStatusSupport().isInterrupted()) {
                        solver.stopSolver();
...
//						}
                ...
                ArrayList<AnnotatedFunction> outputFunctionsList = ClientSimManager.this.getSimWorkspace().getSimulationOwner().getOutputFunctionContext().getOutputFunctionsList();
        ...
        AsynchClientTask[] showResultsTask = this.showSimulationResults0(true, viewerType);
        Collections.addAll(taskList, showResultsTask);
                ...
                if (simulation.getScanCount() > 1) {
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            final SimulationWindow haveSimulationWindow = ClientSimManager.this.getDocumentWindowManager().haveSimulationWindow(simulation.getSimulationInfo().getAuthoritativeVCSimulationIdentifier());
...
//										System.out.println("ScanNum="+scanNum+" "+"FinalStatus="+solverStatus);
        ...
        AsynchClientTask[] taskArray = new AsynchClientTask[taskList.size()];
        taskList.toArray(taskArray);
        ClientTaskDispatcher.dispatch(this.documentWindowManager.getComponent(), new Hashtable<String, Object>(), taskArray, true, true, null);
    }
