...
public abstract class StandardVCellSwingWorker extends VCellSwingWorker {
    private static final Logger lg = LogManager.getLogger(StandardVCellSwingWorker.class);

    protected AsynchProgressPopup progressPopup = null;
    protected Window windowParent = null;
    protected Component focusOwner = null;

    protected final List<AsynchClientTask> taskList;
    protected final Hashtable<String, Object> resultsHash;

    protected final boolean shouldShowProgressPopup;
    protected final boolean shouldBlockInput;
    protected final boolean shouldInformOfProgress;
    protected final boolean shouldBeCancelable;
    protected final String threadBaseName;
    protected final ProgressDialog customProgressDialog;
    protected final Component requestingComponent;
    protected final ProgressDialogListener progressDialogListener;

    protected final ClientTaskDispatcher.StopStrategy stopStrategy;

    public StandardVCellSwingWorker(String threadBaseName, List<AsynchClientTask> taskList, Component requestingComponent,
     ...
     * @throws Exception  if an exception was generated by running the task
     */
    public static void runTask(AsynchClientTask currentTask, Hashtable<String, Object> hash, Collection<AsynchClientTask> taskList) throws Exception {
        ...
        if (lg.isTraceEnabled()){
            ...
            String[] steStrings;
            if (ste == null) steStrings = new String[0];
            else steStrings = Arrays.stream(ste).map(StackTraceElement::toString).toArray(String[]::new);
            msg += "\n" + String.join("\n\t", steStrings);
        }
        lg.debug(msg);
        ...
        for (AsynchClientTask.KeyInfo requiredKey : currentTask.requiredKeys()) {
            ...
            if (obj == null) {
                if (sb == null) sb = StandardVCellSwingWorker.initStringBuilder(currentTask);
        ...
        if (sb == null) { // no problems found
            currentTask.run(hash); // Run the AsyncClientTask the user provided...many layers ago.
            return;
        }
    ...
    public Hashtable<String, Object> verifyAndRun() {
        if (this.shouldShowProgressPopup) {
            if (this.customProgressDialog == null) {
                this.progressPopup = new AsynchProgressPopup(this.requestingComponent, "WORKING...", "Initializing request", Thread.currentThread(), this.shouldBlockInput, this.shouldInformOfProgress, this.shouldBeCancelable, this.progressDialogListener);
            } else {
                this.progressPopup = new AsynchProgressPopup(this.requestingComponent, this.customProgressDialog, Thread.currentThread(), this.shouldBlockInput, this.shouldInformOfProgress, this.shouldBeCancelable, this.progressDialogListener);
            }
            this.progressPopup.setStopStrategy(this.stopStrategy);
            if (this.shouldBlockInput) {
                this.progressPopup.startKeepOnTop();
            } else {
                this.progressPopup.start();
            }
        }
        ...
        for (int i = 0; i < this.taskList.size(); i++) {
            ...
            try {
                currentTask.setClientTaskStatusSupport(this.progressPopup);
                ...
                if (this.progressPopup != null) {
                    this.progressPopup.setVisible(currentTask.shouldShowProgressPopup());
                    if (!this.shouldInformOfProgress) {
                        this.progressPopup.setProgress(i * 100 / this.taskList.size()); // beginning of task
                    }
                    this.progressPopup.setMessage(currentTask.getTaskName());
                }

                // If the hash results have an error, don't run the task.
                if (this.resultsHash.containsKey(TaskEventKeys.HASH_DATA_ERROR.toString())) continue;

                // If the task has an error, don't run it.
                if (this.resultsHash.containsKey(TaskEventKeys.TASK_ABORTED_BY_ERROR.toString()) && currentTask.skipIfAbort()) {
                    continue;
                }

                // If task is supposed to be skipped, don't run it.
                if (this.resultsHash.containsKey(TaskEventKeys.TASKS_TO_BE_SKIPPED.toString())) {
                    ...
                    if (taskShouldBeSkipped) continue;
                }

                // If the popup recorded an interruption, don't run it.
                if (this.progressPopup != null && this.progressPopup.isInterrupted()) {
                    ClientTaskDispatcher.recordException(UserCancelException.CANCEL_GENERIC, this.resultsHash);
                }

                // If task was aborted, don't run it.
                if (this.resultsHash.containsKey(TaskEventKeys.TASK_ABORTED_BY_USER.toString())) {
                    ...
                    if (currentTask.skipIfCancel(exc)) continue;
                }

                // All indicators, green; launch it!
                try {
    ...
    public void performSwingPostProcessing() {
        this.announceTaskComplete();

        if (this.progressPopup != null) this.progressPopup.stop();
        ...
        if (this.resultsHash.containsKey(TaskEventKeys.TASK_ABORTED_BY_ERROR.toString())) {
            ...
            StandardVCellSwingWorker.dispatchFollowUp(this.resultsHash);
        }
        ...
        if (fw != null) {
            ...
            lg.debug("scheduling run with no final window");
        }
    }
