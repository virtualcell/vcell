...
class VCellPysideApp(QtGui.QMainWindow):
 ...
 #           self._timeLabel.setText("0.0")
    ...
    def _onSimulationSelected(self, dialog):
       ...
       dialog.close()
       print("-----")
       assert isinstance(selectedSim, vcell_visit.pyvcell.ttypes.SimulationDataSetRef)
       print(selectedSim)
    ...
    def _onSimulationSelected0(self, sim):
        ...
        try:
            ...
            all(isinstance(n, vcell_visit.pyvcell.ttypes.VariableInfo) for n in variables)
            if (len(variables)<1):
                print "no variables, ignoring dataset ... new"
                return

            currVar = variables[0]
            assert isinstance(currVar, vcell_visit.pyvcell.ttypes.VariableInfo)
                ...
                def successCallback(results):
                    ...
                    if self._sliceControl.isChecked():
                        self._sliceControl.setTitle(
                            visGuiSliceControls.sliceControl.CONST_SLICE_TITLE + " " + str(self.getExtentAlongSliderAxis(self._sliceControl.getSliceSlider().value())))
    ...
    def _onSliceAxisChange(self):
        ...
        self._sliceControl.setTitle(
            visGuiSliceControls.sliceControl.CONST_SLICE_TITLE + " " + str(self.getExtentAlongSliderAxis(self._sliceControl.getSliceSlider().sliderPosition())))
