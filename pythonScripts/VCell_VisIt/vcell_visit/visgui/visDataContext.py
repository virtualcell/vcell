class VisDataContext(object):
    def __init__(self):
        ...
        assert(isinstance(self._currentDataSet, vcell_visit.pyvcell.ttypes.SimulationDataSetRef) or self._currentDataSet == None);
 

    def getCurrentDataSet(self):
        assert(isinstance(self._currentDataSet, vcell_visit.pyvcell.ttypes.SimulationDataSetRef));
        return self._currentDataSet

    def setCurrentDataSet(self, dataSet):
        assert(isinstance(dataSet, vcell_visit.pyvcell.ttypes.SimulationDataSetRef));
    ...
    def getCurrentVariable(self):
        assert(isinstance(self._currentVariable,
                          vcell_visit.pyvcell.ttypes.VariableInfo) or self._currentVariable == None);
        return self._currentVariable

    def setCurrentVariable(self, variable):
        assert(isinstance(variable, vcell_visit.pyvcell.ttypes.VariableInfo) or self._currentVariable == None);
    ...
    def getVtuVariableFromDisplayName(self, varDisplayName):
        for varInfo in self._variableInfos:
            assert(isinstance(varInfo, vcell_visit.pyvcell.ttypes.VariableInfo));
