...
public class HungarianAlgorithm {
	...
	public static void main(String[] args) 
	{
		...
		for (int i=0; i<assignment.length; i++)
		{
			...
			System.out.println("array("+(assignment[i][0]+1)+","+((assignment[i][1]+1))+") = "+nf.format(array[assignment[i][0]][assignment[i][1]]));
			sum = sum + array[assignment[i][0]][assignment[i][1]];
			//</COMMENT>
		}
		
		System.out.println("\nThe "+sumType+" is: "+nf.format(sum));
		printTime((endTime - startTime)/1000.0);
		
	}

/**
 * HungarianAlgorithm constructor comment.
 */
