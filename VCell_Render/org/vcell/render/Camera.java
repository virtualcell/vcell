...
/**
 * Center should still be center with size corrected
 * @param gl
 * @param x
 * @param y
 * @param w
 * @param h
 */
public void applyProjection(GL gl)
{
	gl.glMatrixMode(GL.GL_PROJECTION);
	gl.glLoadIdentity();

	BoundingBox box = getProjectionBoundingBox();
	double left = box.getLoX();
	double right = box.getHiX();
	double bottom = box.getLoY();
	double top = box.getHiY();
	double nearOrtho = box.getLoZ();
	double farOrtho = box.getHiZ();
	//System.out.println("camera, scale=("+scale+"), oldArgs=("+_left+","+_right+","+_bottom+","+_top+"), newArgs=("+newLeft+","+newRight+","+newBottom+","+newTop+")");
	if (projectMode.equals(ProjectMode.PROJECT_PARALLEL)) {
        gl.glOrtho(left,right,bottom,top,nearOrtho,farOrtho);
	}else if (projectMode.equals(ProjectMode.PROJECT_PERSPECTIVE)) {
       gl.glFrustum(left,right,bottom,top,_nearFrustum,_farFrustum);
	}else{
       throw new RuntimeException("ProjectMode "+projectMode+" not expected");
	}
...
public Affine getInvProjection() {
...
{
   _scale = Math.max(Math.min(scale, 100.0),0.01);
}

public double getProjectionScale(){
	return _scale;
}
public BoundingBox getProjectionBoundingBox(){
	double centerX = (_right+_left)/2.0;
	double centerY = (_bottom+_top)/2.0;
	double centerZ = (_nearOrtho+_farOrtho)/2.0;
	double deltaX = (_right-_left)/_scale;
	double deltaY = (_top-_bottom)/_scale;
	double deltaZ = (_farOrtho-_nearOrtho)/_scale;
	double newLeft = centerX-deltaX/2.0;
	double newBottom = centerY-deltaY/2.0;
	double newRight = centerX+deltaX/2.0;
	double newTop = centerY+deltaY/2.0;
	double newNearOrtho = centerZ-deltaZ/2.0;
	double newFarOrtho = centerZ+deltaZ/2.0;
	return new BoundingBox(newLeft,newRight,newBottom,newTop,newNearOrtho,newFarOrtho);
}
