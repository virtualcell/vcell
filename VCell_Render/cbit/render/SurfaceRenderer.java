...
public class SurfaceRenderer {
	...
	public static class BoundingBoxInfo {
		public Vect3d[] boundingBoxCorners;
		public double minZ;
		public double maxZ;
 ...
 */
private void calculateProjectedCentroid(Polygon polygon,Vect3d centroid,Vect3d centroidProj) {
 ...
 * @return Polygon
 */
private java.awt.Polygon createScreenPolygon(Polygon polygon,int[] xPoints,int[] yPoints,double xOffset,double xScale,double yOffset,double yScale) {
				...
				if (bRenderA){
					...
					if (getTrackball().getCamera().projectLine(pt1a,pt2a,proj1,proj2)){
						...
						if(i==0){
							...
							Vect3d dirVect = new Vect3d(pt2x-pt1x,pt2y-pt1y,0);
 ...
 * @param g2d java.awt.Graphics
 */
protected ByteImage sampleVolume(SurfaceCollection surfaceCollection,int numX, int numY, int numZ) throws ImageException {
	double ox = fieldOrigin.getX();
	double oy = fieldOrigin.getY();
	//
...
if (hitList.getNumHits()==0){
	...
	return new ByteImage(pixels,fieldExtent,numX,numY,numZ);
}
}