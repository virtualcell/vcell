...
public class SurfaceViewerTool {
	...
	private Trackball trackball = new Trackball(new Camera());
	//
	...
	private GLJPanel fieldSurfaceCanvas = null;
	...
	private Vect3d fieldViewAngleRadians = null;
 ...
 */
public GLJPanel getSurfaceCanvas() {
	return fieldSurfaceCanvas;
}


/**
 * Gets the viewAngleRadians property (Vect3d) value.
 * @return The viewAngleRadians property value.
 * @see #setViewAngleRadians
 */
public Vect3d getViewAngleRadians() {
...
private void mouseDragged(MouseEvent e) {
   ...
   Vect3d oldValue = fieldViewAngleRadians;
	...
	switch(getCurrentManipulation()){
		...
		case MANIPULATOR_PAN: {
			Vect3d trSize = trackball.getSize();
...
public void resetView() {
	...
	trackball.setSize(size);
	trackball.setCenter(center);
	trackball.setTranslation(new Vect3d(0.0,0.0,0.0));
    ...
    trackball.setRotation(angleX,angleY,angleZ);
	setViewAngleRadians(trackball.getRotation());

	trackball.setScale(1.0); // fake out trackball to update camera
...
public void setSurfaceCanvas(SurfaceCanvas surfaceCanvas) {
	GLJPanel oldSurfaceCanvas = fieldSurfaceCanvas;
...
/**
 * Sets the viewAngleRadians property (Vect3d) value.
 * @param viewAngleRadians The new value for the property.
 * @see #getViewAngleRadians
 */
public void setViewAngleRadians(Vect3d viewAngleRadians) {
	Vect3d oldValue = fieldViewAngleRadians;
	fieldViewAngleRadians = viewAngleRadians;
	trackball.setRotation(viewAngleRadians.getX(),viewAngleRadians.getY(),viewAngleRadians.getZ());
