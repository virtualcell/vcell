...
public void getInvRotationMatrixGL(Affine mat) {
...
public void getRotationMatrixGL(Affine mat) {
...
//   vect.set(x,y,-z);
   return new Vect3d(x,y,Math.abs(z));
}
...
public void setRotation(double angleRadX, double angleRadY, double angleRadZ) {
	Vect3d rotationAngles = new Vect3d(angleRadX,angleRadY,angleRadZ);

	Matrix3d rotationMatrix = new Matrix3d();
	rotationMatrix.setCosineRotation(rotationAngles);

	Matrix3d invRotationMatrix = new Matrix3d();
...
//	scaleFactor *= this.scale;
	...
	gl.glTranslated(displacement.q[0], displacement.q[1], displacement.q[2]);
	currQuat.loadMatrix16(mat);
	gl.glTranslated(center.q[0],center.q[1],center.q[2]);
	gl.glMultMatrixd(mat, 0);
	gl.glTranslated(-center.q[0],-center.q[1],-center.q[2]);
//	System.out.println("trackball, scale=("+scaleFactor+"), center=("+center.q[0]+","+center.q[1]+","+center.q[2]+"), displacement=("+displacement.q[0]+","+displacement.q[1]+","+displacement.q[2]+")");
}
private void updateCamera() {
   ...
   getRotationMatrixGL(forwardAffine);
   getInvRotationMatrixGL(inverseAffine);
   ...
   camera.setProjectionScale(this.scale);
}
