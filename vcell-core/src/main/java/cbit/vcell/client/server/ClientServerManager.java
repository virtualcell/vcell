...
public class ClientServerManager implements SessionManager, DataSetControllerProvider {
    ...
    public void connectAs(InteractiveContext requester, String user) {
        this.reconnectStat = ReconnectStatus.NOT;
        switch (this.getClientServerInfo().getServerType()) {
            case SERVER_LOCAL: {
                this.clientServerInfo = ClientServerInfo.createLocalServerInfo(user);
                break;
            }
            case SERVER_REMOTE: {
                this.clientServerInfo = ClientServerInfo.createRemoteServerInfo(
                        this.getClientServerInfo().getApihost(), this.getClientServerInfo().getApiport(), this.getClientServerInfo().getPathPrefix_v0(),
                        user);
                break;
            }
        }
