...
public class GeometryFileWriter {
    ...
    public static void write(Writer writer, Geometry resampledGeometry) throws IOException{
        ...
        if(geometricRegions != null){
            for(GeometricRegion geometricRegion : geometricRegions){
                if(geometricRegion instanceof VolumeGeometricRegion){
                    numVolumeRegions++;
                    volRegionList.add((VolumeGeometricRegion) geometricRegion);
        ...
        //
        volRegionList.sort((reg1, reg2) -> Integer.compare(reg1.getRegionID(), reg2.getRegionID()));

        VolumeGeometricRegion[] volRegions = volRegionList.toArray(VolumeGeometricRegion[]::new);
        ...
        if(geometricRegions != null){
            for(GeometricRegion geometricRegion : geometricRegions){
                if(!(geometricRegion instanceof SurfaceGeometricRegion surfaceRegion)) continue;
        ...
        if(surfaceCollection != null){
            for(int i = 0; i < surfaceCollection.getSurfaceCount(); i++){
                ...
                for(int j = 0; j < surface.getPolygonCount(); j++){
                    ...
                    for(Node value : node){
                        if(!value.getMoveX() || (dimension > 1 && !value.getMoveY()) || (dimension == 3 && !value.getMoveZ())){
                            nodesOnBoundary++;
                        }
                    }
                    ...
                    if(nodesOnBoundary == 0){
                        for(Node value : node){
                            elementCoord.add(new cbit.vcell.render.Vect3d(value.getX(), value.getY(), value.getZ()));
                        }
                        elementCoord.scale(0.25);
                    } else if(nodesOnBoundary == 2){
                        for(Node value : node){
                            if(!value.getMoveX() || !value.getMoveY() || !value.getMoveZ()){
                                elementCoord.add(new cbit.vcell.render.Vect3d(value.getX(), value.getY(), value.getZ()));
                            }
                        }
                        ...
                        for(Node value : node){
                            if(!value.getMoveX() && !value.getMoveY() || !value.getMoveY() && !value.getMoveZ() || !value.getMoveX() && !value.getMoveZ()){
                                elementCoord.set(value.getX(), value.getY(), value.getZ());
                            }
                        }
                    } else {
