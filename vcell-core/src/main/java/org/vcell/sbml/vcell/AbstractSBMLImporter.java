...
public class AbstractSBMLImporter implements ISBMLImporter {
    ...
    protected org.sbml.jsbml.Model sbmlModel = null;
    protected final VCLogger vcLogger;
    protected final boolean shouldValidateSBML;

    // issue list for medium-level warnings while importing
    protected final Vector<Issue> localIssueList = new Vector<>();
    protected final IssueContext issueContext = new IssueContext();
    ...
    public AbstractSBMLImporter(VCLogger argVCLogger, boolean shouldValidateSBML){
        this.vcLogger = argVCLogger;
        this.shouldValidateSBML = shouldValidateSBML;
    }
    ...
    private static void finalizeCompartments(org.sbml.jsbml.Model sbmlModel, BioModel vcBioModel, SBMLSymbolMapping sbmlSymbolMapping, Vector<Issue> localIssueList, IssueContext issueContext){
        ...
        for(Compartment compartment : sbmlModel.getListOfCompartments()){
            ...
            try {
                ...
                localIssueList.add(new Issue(vcBioModel, issueContext, Issue.IssueCategory.SBMLImport_RestrictedFeature, msg, Issue.Severity.WARNING));
                ...
                logger.error(msg, e);
                localIssueList.add(new Issue(vcBioModel, issueContext, Issue.IssueCategory.SBMLImport_UnsupportedAttributeOrElement, msg, Issue.Severity.WARNING));
            }
        }
    }
    ...
    private static void addCompartments(org.sbml.jsbml.Model sbmlModel, int geometryDimension, BioModel vcBioModel,
        ...
        try {
            ...
            for(int i = 0; i < sbmlModel.getNumCompartments(); i++){
                ...
                if(sbmlCompartment.getOutside() == null || sbmlCompartment.getOutside().isEmpty()){
        ...
        if(sbmlModel.getNumEvents() > 0){
            ...
            for(int i = 0; i < sbmlModel.getNumEvents(); i++){
                try {
                    ...
                    // delay
                    BioEvent vcEvent = new BioEvent(eventName, BioEvent.TriggerType.GeneralTrigger, true, vcBioModel.getSimulationContext(0));

                    sbmlSymbolMapping.putBioEvent(event, vcEvent);

                    if(event.isSetDelay()){
                        ParameterContext.LocalParameter triggerDelayParameter = vcEvent.getParameter(BioEvent.BioEventParameterType.TriggerDelay);
                    ...
                    cbit.vcell.mapping.ParameterContext.LocalParameter vcTriggerParameter = vcEvent.getParameter(BioEvent.BioEventParameterType.GeneralTriggerFunction);
                    ...
                    // event assignments
                    ArrayList<BioEvent.EventAssignment> vcEvntAssgnList = new ArrayList<>();
                    for(int j = 0; j < event.getNumEventAssignments(); j++){
                        ...
                        if(vcellEventAssignTarget != null){
                            ...
                            BioEvent.EventAssignment vcEventAssignment = vcEvent.new EventAssignment(vcellEventAssignTarget, vcellEventAssignmentExpr);
...
//			throw new SBMLImportException("VCell doesn't support CompartmentTypes at this time");
            localIssueList.add(new Issue(vcBioModel, issueContext, Issue.IssueCategory.SBMLImport_RestrictedFeature,
...
//			throw new SBMLImportException("VCell doesn't support SpeciesTypes at this time");
    ...
    private static void addParameters(org.sbml.jsbml.Model sbmlModel, org.sbml.jsbml.ext.spatial.Geometry sbmlGeometry,
        ...
        ListOf<org.sbml.jsbml.Parameter> listofGlobalParams = sbmlModel.getListOfParameters();
        if(listofGlobalParams == null){
            logger.debug("No Global Parameters");
            return;
        }
        Model vcModel = vcBioModel.getModel();
        ArrayList<Model.ModelParameter> vcModelParamsList = new ArrayList<>();
        ...
        for(Model.ReservedSymbol rs : vcModel.getReservedSymbols()){
        ...
        for(int i = 0; i < sbmlModel.getNumParameters(); i++){
            org.sbml.jsbml.Parameter sbmlGlobalParam = listofGlobalParams.get(i);
            ...
            if(paramAnnotation != null && paramAnnotation.getNonRDFannotation() != null){
                ...
                if(paramElement != null){
                    ...
                    if(varDomainStr != null && !varDomainStr.trim().isEmpty()){
                        varDomain = new cbit.vcell.math.Variable.Domain(varDomainStr);
                    }
                    sbmlSymbolMapping.putRuntime(sbmlGlobalParam, new AnnotatedFunction(sbmlParamId, new Expression("DummyExpression"), varDomain, "", varType, AnnotatedFunction.FunctionCategory.OUTPUTFUNCTION));
                    continue;
                }
            }
            ...
            if(bSpatial){
                ...
                if(sbmlParamType instanceof DiffusionCoefficient diffCoeff){
                    if(diffCoeff.isSetVariable()){
                        ...
                        if(diffTargetSte instanceof SpeciesContext paramSpContext){
                            vcSpContextsSpec = vcBioModel.getSimulationContext(0).getReactionContext().getSpeciesContextSpec(paramSpContext);
                            SpeciesContextSpec.SpeciesContextSpecParameter diffusionParameter = vcSpContextsSpec.getDiffusionParameter();
                ...
                if(sbmlParamType instanceof AdvectionCoefficient advCoeff){
                    if(advCoeff.isSetVariable()){
                        ...
                        if(advTargetSte instanceof SpeciesContext paramSpContext){
                            vcSpContextsSpec = vcBioModel.getSimulationContext(0).getReactionContext().getSpeciesContextSpec(paramSpContext);
                            CoordinateKind coordKind = advCoeff.getCoordinate();
                            final SpeciesContextSpec.SpeciesContextSpecParameter scsAdvParam;
                            switch(coordKind){
                                case cartesianX:{
                ...
                if(sbmlParamType instanceof BoundaryCondition bCondn){
                    if(bCondn.isSetVariable()){
                        ...
                        if(bcondTargetSte instanceof SpeciesContext paramSpContext){
                            vcSpContextsSpec = vcBioModel.getSimulationContext(0).getReactionContext().getSpeciesContextSpec(paramSpContext);
                            SpeciesContextSpec.SpeciesContextSpecParameter scsParam = null;
                ...
                // Check for Boundary condition(s)
                if(sbmlParamType instanceof SpatialSymbolReference spatialSymbolRef){
            ...
            Model.ModelParameter vcellModelParameter = vcModel.getModelParameter(sbmlParamId);
            if(vcellModelParameter == null){
                ...
                if(reservedSymbolHash.contains(sbmlParamId)){
                    if(isRestrictedXYZT(sbmlParamId, vcBioModel, bSpatial)){
                        ...
                        Model.ReservedSymbol rs = vcBioModel.getModel().getReservedSymbolByName(sbmlParamId);
                ...
                // treat KMOLE special
                //
                Model.ReservedSymbol KMOLE = vcModel.getKMOLE();
                double KMOLE_value;
                try {
                ...
                if(sbmlParamId.startsWith("Voltage")){
                    ...
                    for (StructureMapping structureMapping : structureMappings) {
                        if (structureMapping instanceof MembraneMapping) {
                            StructureMapping.StructureMappingParameter voltage = ((MembraneMapping) structureMapping).getInitialVoltageParameter();
                ...
                //
                Model.ModelParameter vcGlobalParam = vcModel.new ModelParameter(vcParamName, new Expression(0.0), Model.ROLE_UserDefined, glParamUnitDefn);
                ...
                if(vcGlobalParam.getName().startsWith(ReservedParamPrefix)){
                    ...
                    for(Model.ModelParameter mp : vcModelParamsList){
        ...
        vcModel.setModelParameters(vcModelParamsList.toArray(new Model.ModelParameter[0]));
    }
        ...
        for(String sbmlSymbol : symbols){
            if(sbmlSymbol.equals(TIME_SYMBOL_OVERRIDE)){
                ...
                if(vcellSymbolTableEntry != null){
                    if(namescope instanceof SpeciesContextSpec.SpeciesContextSpecNameScope && vcellSymbolTableEntry instanceof SpeciesContextSpec.SpeciesContextSpecParameter){
                        vcellSymbolTableEntry = ((SpeciesContextSpec.SpeciesContextSpecParameter) vcellSymbolTableEntry).getSpeciesContext();
                    }
                    if(namescope instanceof SpeciesContextSpec.SpeciesContextSpecNameScope && vcellSymbolTableEntry instanceof StructureMapping.StructureMappingParameter){
                        vcellSymbolTableEntry = ((StructureMapping.StructureMappingParameter) vcellSymbolTableEntry).getStructure().getStructureSize();
                    }
    ...
    private static SBase findSBase(org.sbml.jsbml.Model sbmlModel, String sbmlSid){
        ...
        List<SBase> sbase_nodes = sbmlModel.filter(new IdFilter(sbmlSid))
                .stream().filter(n -> n instanceof SBase).map(n -> (SBase) n).toList();
        if(sbase_from_model != null && sbase_nodes.isEmpty()){
...
//	}
    ...
    private static void addReactionParticipants(org.sbml.jsbml.Model sbmlModel, org.sbml.jsbml.Reaction sbmlRxn, long level,
                                                ReactionStep vcRxn, SBMLSymbolMapping sbmlSymbolMapping,
                                                LambdaFunction[] lambdaFunctions, VCLogger vcLogger,
                                                Vector<Issue> localIssueList, IssueContext issueContext) throws Exception{
        //
        // add reactants (compress stoichiometry as needed)
        //
        HashMap<org.sbml.jsbml.Species, Integer> sbmlReactantsSpeciesHash = new HashMap<>();
        HashMap<org.sbml.jsbml.Species, SpeciesReference> sbmlReactantsSpeciesReferenceHash = new HashMap<>();
        for(int j = 0; j < sbmlRxn.getNumReactants(); j++){
            ...
            final org.sbml.jsbml.Species species = reactantSpeciesReference.getSpeciesInstance();
            if(reactantSpeciesReference.isSetStoichiometry()){
                double sbmlStoichAttribute = reactantSpeciesReference.getStoichiometry();
                if((int) sbmlStoichAttribute != sbmlStoichAttribute){
                    vcLogger.sendMessage(VCLogger.Priority.HighPriority,
                            ...
                            "Non-integer stoichiometry ('" + sbmlStoichAttribute + "' for reactant" +
                                    " '" + sbmlReactantSpId + "' in reaction '" + sbmlRxn.getId() + "')" +
                                    " not handled in VCell at this time.");
                }
                stoichiometry = (int) sbmlStoichAttribute;
            } else if(reactantSpeciesReference.isSetStoichiometryMath()){
                Expression stoichMathExpr = getExpressionFromFormula(reactantSpeciesReference.getStoichiometryMath().getMath(), lambdaFunctions, vcLogger);
                Expression vcellStoichExpr = adjustExpression(sbmlRxn, stoichMathExpr, vcRxn.getNameScope(), sbmlSymbolMapping, SymbolContext.RUNTIME);
                Expression vcRateExpression = new Expression(vcellStoichExpr);
                double mathStoich = 1.0;
                try {
                    mathStoich = vcRateExpression.evaluateConstant();
                    if(mathStoich != ((int) mathStoich)){
                        vcLogger.sendMessage(VCLogger.Priority.HighPriority,
                                VCLogger.ErrorType.UnsupportedConstruct,
                                "Non-integer stoichiometry ('" + mathStoich + "' for reactant" +
                                        " '" + sbmlReactantSpId + "' in reaction '" + sbmlRxn.getId() + "')" +
                                        " not handled in VCell at this time.");
                    }
                } catch(ExpressionException e){
                    vcLogger.sendMessage(VCLogger.Priority.HighPriority,
                            VCLogger.ErrorType.UnsupportedConstruct,
                            "Non-numeric stoichiometry ('" + stoichMathExpr.infix() + "' for product" +
                                    " '" + sbmlReactantSpId + "' in reaction '" + sbmlRxn.getId() + "')" +
                                    " not handled in VCell at this time.");
                }
                stoichiometry = (int) mathStoich;
            } else if(level < 3){
                stoichiometry = 1;
            } else {
        ...
        //
        for(Map.Entry<org.sbml.jsbml.Species, Integer> es : sbmlReactantsSpeciesHash.entrySet()){
            org.sbml.jsbml.Species species = es.getKey();
            int stoich = es.getValue();

            EditableSymbolTableEntry vcellSte = sbmlSymbolMapping.getSte(species, SymbolContext.RUNTIME);
            if(!(vcellSte instanceof SpeciesContext speciesContext)){
        ...
        //
        HashMap<org.sbml.jsbml.Species, Integer> sbmlProductsSpeciesHash = new HashMap<>();
        HashMap<org.sbml.jsbml.Species, SpeciesReference> sbmlProductsSpeciesReferenceHash = new HashMap<>();
        for(int j = 0; j < sbmlRxn.getNumProducts(); j++){
            ...
            final int stoichiometry;
            final org.sbml.jsbml.Species species = productSpeciesReference.getSpeciesInstance();
            if(productSpeciesReference.isSetStoichiometry()){
                ...
                if((int) sbmlStoichAttribute != sbmlStoichAttribute){
                    String msg = "Non-integer stoichiometry ('" + sbmlStoichAttribute + "' for product" +
                            " '" + sbmlProductSpId + "' in reaction '" + sbmlRxn.getId() + "')" +
                            " not handled in VCell at this time.";
                    vcLogger.sendMessage(VCLogger.Priority.HighPriority, VCLogger.ErrorType.UnsupportedConstruct, msg);
                    logger.error(msg);
                }
                stoichiometry = (int) sbmlStoichAttribute;
            } else if(productSpeciesReference.isSetStoichiometryMath()){
                Expression stoichMathExpr = getExpressionFromFormula(productSpeciesReference.getStoichiometryMath().getMath(), lambdaFunctions, vcLogger);
                Expression vcellStoichExpr = adjustExpression(sbmlRxn, stoichMathExpr, vcRxn.getNameScope(), sbmlSymbolMapping, SymbolContext.RUNTIME);
                Expression vcRateExpression = new Expression(vcellStoichExpr);
                double mathStoich = 1.0;
                try {
                    ...
                    if(mathStoich != ((int) mathStoich)){
                        vcLogger.sendMessage(VCLogger.Priority.HighPriority,
                                VCLogger.ErrorType.UnsupportedConstruct,
                                "Non-integer stoichiometry ('" + mathStoich + "' for product" +
                                        " '" + sbmlProductSpId + "' in reaction '" + sbmlRxn.getId() + "')" +
                                        " not handled in VCell at this time.");
                    }
                } catch(ExpressionException e){
        ...
        //
        for(Map.Entry<org.sbml.jsbml.Species, Integer> es : sbmlProductsSpeciesHash.entrySet()){
            org.sbml.jsbml.Species species = es.getKey();
            int stoich = es.getValue();
        ...
        for(int j = 0; j < sbmlRxn.getNumModifiers(); j++){
            ...
            org.sbml.jsbml.Species sbmlModifier = sbmlModel.getSpecies(sbmlSpId);
            ...
            if(!bFoundSpeciesContext){
                ...
                if(bFoundAsReactantOrProduct){
                    localIssueList.add(new Issue(speciesContext, issueContext, Issue.IssueCategory.SBMLImport_Reaction,
    ...
    private static void parseAssignmentAndInitialAssignmentExpressions(org.sbml.jsbml.Model sbmlModel, LambdaFunction[] lambdaFunctions,
        ...
        for(int i = 0; i < sbmlModel.getNumRules(); i++){
            ...
            if(rule instanceof org.sbml.jsbml.AssignmentRule assignmentRule){
...
//					&& !(assignmentTargetSte instanceof SpeciesContext)) {
        ...
        if(foundConstStructureSize){
            try {
                ...
                logger.warn(msg);
                localIssueList.add(new Issue(vcModel, issueContext, Issue.IssueCategory.SBMLImport_RestrictedFeature, msg, Issue.Severity.WARNING));
            } catch(Exception e){
    ...
    private static void readRateRules(org.sbml.jsbml.Model sbmlModel, LambdaFunction[] lambdaFunctions, SBMLSymbolMapping sbmlSymbolMapping,
        ...
        ListOf<Rule> rulesList = sbmlModel.getListOfRules();
        if(rulesList == null){
            logger.debug("No Rules specified");
            return;
        }
        for(Rule rule : rulesList){
            if(!(rule instanceof org.sbml.jsbml.RateRule sbmlRateRule)) continue;
    ...
    private static void convertLocalParameters(ReactionStep rs) throws ExpressionException, PropertyVetoException{
        Kinetics.KineticsParameter[] kps = rs.getKinetics().getKineticsParameters();
        for(Kinetics.KineticsParameter kp : kps){
        ...
        for(Kinetics.KineticsParameter kp : kps){
            if(kp.getRole() == Kinetics.ROLE_UserDefined){
                Model.ModelParameter mp = rs.getKinetics().getReactionStep().getModel().getModelParameter(kp.getName());
                if(mp != null){
    ...
    private static void createRateRules(org.sbml.jsbml.Model sbmlModel, BioModel vcBioModel, SBMLSymbolMapping sbmlSymbolMapping){
        ...
        for(Rule rule : sbmlModel.getListOfRules()){
            if(!(rule instanceof org.sbml.jsbml.RateRule rateRule)){
                continue;
            }
    ...
    private static void configureSpeciesContextSpecs(org.sbml.jsbml.Model sbmlModel, BioModel vcBioModel,
        ...
        try {
            ...
            for(SpeciesContext vcSpeciesContext : vcSpeciesContexts){
                ...
                if(!(sbase instanceof org.sbml.jsbml.Species sbmlSpecies)){
    ...
    private static void getReferencedSpeciesInExpr(org.sbml.jsbml.Model sbmlModel, Expression sbmlExpr,
        ...
        for(int i = 0; symbols != null && i < symbols.length; i++){
            org.sbml.jsbml.Parameter sbmlParam = sbmlModel.getParameter(symbols[i]);
            if(sbmlParam != null){
    ...
    public BioModel getBioModel() throws VCLoggerException {
        ...
        try {
            ...
            if((biomodelName == null) || biomodelName.trim().isEmpty()){
            ...
            logger.error(msg, e);
            throw new SBMLImportException("Inconsistent unit system. Cannot import SBML model into VCell.", SBMLImportException.Category.INCONSISTENT_UNIT, e);
        }
        ...
        //
        try {
            cbit.vcell.geometry.Geometry geometry = new cbit.vcell.geometry.Geometry(BioModelChildSummary.COMPARTMENTAL_GEO_STR, 0);
            SimulationContext simulationContext = new SimulationContext(vcBioModel.getModel(), geometry, null, null, SimulationContext.Application.NETWORK_DETERMINISTIC);
...
//			vcBioModel.getVCMetaData().printRdfStatements();
    ...
    protected SBMLDocument readSbmlDocument(File sbmlFile){
    ...
    protected SBMLDocument readSbmlDocument(InputStream sbmlInputStream){
    ...
    protected static void validateSBMLDocument(SBMLDocument document, VCLogger vcLogger) throws VCLoggerException{
    ...
    protected static void validateSBMLPackages(SBMLDocument document, Vector<Issue> localIssueList, IssueContext issueContext){
        int numPackages = 0;
        String msgPackages = "";
        try {
...
//			logger.trace("sbml id: " + unitName);
        ...
        // species
        ListOf<org.sbml.jsbml.Species> listOfSpecies = sbmlModel.getListOfSpecies();
        for(org.sbml.jsbml.Species sbmlSpecies : listOfSpecies){
    ...
    private static Structure getReactionStructure(org.sbml.jsbml.Model sbmlModel, org.sbml.jsbml.Reaction sbmlRxn, BioModel vcBioModel,
        ...
        if(compartmentNamesHash.size() == 1){
            ...
            return struct;
        } else if(compartmentNamesHash.isEmpty()){
            ...
            Iterator<Map.Entry<String, Integer>> iterator = structureFrequencyHash
                    .entrySet().iterator();
            Map.Entry<String, Integer> mostUsedStructureEntry = iterator.next();
            while (iterator.hasNext()) {
                Map.Entry<String, Integer> currentStructureEntry = iterator.next();
...
//			}
    ...
    private static void translateSBMLModel(org.sbml.jsbml.Model sbmlModel, BioModel vcBioModel,
        ...
        if(bSpatial){
            ...
            //
            List<SpeciesContextSpec.SpeciesContextSpecParameter> allDiffusionParameters = Arrays.stream(vcBioModel.getSimulationContext(0).getReactionContext().getSpeciesContextSpecs())
                    .map(SpeciesContextSpec::getDiffusionParameter).toList();
            for(SpeciesContextSpec.SpeciesContextSpecParameter diffParam : allDiffusionParameters){
                try {
...
//		addInitialAssignments(sbmlModel, lambdaFunctions, sbmlSymbolMapping);
        ...
        HashMap<Kinetics, SBase> constantKinetics = new HashMap<>();
...
//			vcBioModel.transformLumpedToDistributed();
            try {
                for(ReactionStep reactionStep : vcBioModel.getModel().getReactionSteps()){
                    if(reactionStep.getKinetics().getKineticsDescription().isLumped()){
                        ...
                        // deal with renaming of constant rate parameters for both transformed and not transformed cases
    ...
    private static void finalizeReactions(org.sbml.jsbml.Model sbmlModel, BioModel vcBioModel,
        ...
        for(int i = 0; i < vcBioModel.getModel().getReactionSteps().length; i++){
            Kinetics.KineticsParameter rateParam = vcBioModel.getModel().getReactionSteps(i).getKinetics().getAuthoritativeParameter();
            ...
            if(rateSymbols != null && rateSymbols.length == 1){
                ...
                if(paramAnnotation.getNonRDFannotation() != null){
                    ...
                    if(paramElement != null){
                        Model.ModelParameter globalParam = (Model.ModelParameter) sbmlSymbolMapping.getRuntimeSte(sbase);
                    ...
                    try {
                        ...
                        if(symbols != null){
                            for(String symbol : symbols){
                                ...
                                if(rxn != null){
                                    Kinetics.KineticsParameter rateParam = rxn.getKinetics().getAuthoritativeParameter();
    ...
    private static void checkIdentifiersNameLength(org.sbml.jsbml.Model sbmlModel, Vector<Issue> localIssueList, IssueContext issueContext){
        ...
        ListOf<org.sbml.jsbml.Parameter> listOfParameters = sbmlModel.getListOfParameters();
        boolean bLongParameterName = false;
        for(int i = 0; i < sbmlModel.getNumParameters(); i++){
            org.sbml.jsbml.Parameter param = listOfParameters.get(i);
            String paramName = param.getId();
    ...
    private static void addReactions(org.sbml.jsbml.Model sbmlModel, BioModel vcBioModel, boolean bSpatial,
        ...
        try {
            for(Reaction sbmlReaction : reactions){
                ...
                // the sbmlReaction
                addReactionParticipants(sbmlModel, sbmlReaction, level, vcReaction, sbmlSymbolMapping, lambdaFunctions, vcLogger, localIssueList, issueContext);
                ...
                if(kLaw != null){
                    ...
                    for(LocalParameter p : localParameters){
                        ...
                        Kinetics.KineticsParameter kineticsParameter = kinetics.getKineticsParameter(paramName);
                    ...
                    Kinetics.KineticsParameter kp = kinetics.getAuthoritativeParameter();
                    ...
                    for(Kinetics.KineticsProxyParameter kpp : kinetics.getProxyParameters()){
                        if(kpp.getTarget() instanceof Model.ModelParameter mp){
                            String[] symbols = mp.getExpression().getSymbols();
                            if(symbols != null){
                                for(String symbol : symbols){
                                    SymbolTableEntry ste = mp.getExpression().getSymbolBinding(symbol);
                                    if(ste instanceof SpeciesContext sc){
                    ...
                    for(LocalParameter param : localParameters){
                        ...
                        if(kineticsParameter != null){
                            ...
                            Kinetics.UnresolvedParameter ur = kinetics.getUnresolvedParameter(paramName);
                            if(ur != null){
                        ...
                        if(!lpSet){
                            ...
                            Kinetics.KineticsProxyParameter kpp = kinetics.getProxyParameter(paramName);
                            ...
                            if(kpp != null && kpp.getTarget() instanceof Model.ModelParameter){
...
//		case square:
     ...
     * @throws IllegalArgumentException if plugin or geometry not found
     */
    private static org.sbml.jsbml.ext.spatial.Geometry getSbmlGeometry(org.sbml.jsbml.Model sbmlModel, Vector<Issue> localIssueList, IssueContext issueContext){
        ...
        if(geometry.getListOfGeometryDefinitions().isEmpty()){
    ...
    private static void addGeometry(org.sbml.jsbml.Model sbmlModel, BioModel vcBioModel, LambdaFunction[] lambdaFunctions,
        ...
        { // local code block
            ...
            for(CoordinateComponent coordComponent : listOfCoordComps){
                ...
                org.sbml.jsbml.Parameter coordComponentParameter = getSpatialParameter(sbmlModel, spatialId);
                final Model.ReservedSymbol vcellCoordinateSymbol;
        ...
        for(int i = 0; i < sbmlGeometry.getListOfGeometryDefinitions().size(); i++){
            ...
            } else if(gd_temp instanceof SampledFieldGeometry sfg){
        ...
        cbit.vcell.geometry.Geometry vcGeometry = null;
        if(selectedGeometryDefinition == analyticGeometryDefinition
                || selectedGeometryDefinition == csGeometry){
            vcGeometry = new cbit.vcell.geometry.Geometry("spatialGeom", dimension);
                    ...
                    for (SampledVolume sampledVolume : sampledVolumes) {
                        ...
                        for (byte pixel : uncompressedImageInBytes) {
                            if (pixel >= minValue && pixel < maxValue) {
                                hashSet.add(Math.abs(pixel));
                            }
                        }
                        // determine median value for this sampledVolume (to use as the effective 'sampledValue')
                        List<Integer> sortedPixelValues = hashSet.stream().sorted().toList();
                        if (sortedPixelValues.isEmpty()) {
                            continue;
                        }
...
//				}
                ...
                vcGeometry = new cbit.vcell.geometry.Geometry("spatialGeom", vcImage);
            } catch(Exception e){
...
//					}
            ...
            if(selectedGeometryDefinition == analyticGeometryDefinition){
                ...
                for(AnalyticVolume analyticVol : aVolumes){
                    ...
                    CastingUtils.CastInfo<AnalyticSubVolume> ci = CastingUtils.attemptCast(AnalyticSubVolume.class, vcSubvolume);
                    if(!ci.isGood()){
...
//				CSGObject[] vcCSGSubVolumes = new CSGObject[n];
                for(int i = 0; i < n; i++){
                    ...
                    cbit.vcell.geometry.CSGObject vcellCSGObject = new cbit.vcell.geometry.CSGObject(null, sbmlCSGObject.getSpatialId(), i);
...
//				vcGeometry.getGeometrySpec().setSubVolumes(vcCSGSubVolumes);
        ...
        RegionImage.RegionInfo[] regionInfos = vcGsd.getRegionImage().getRegionInfos();
        ...
        try {
            ...
            for(int i = 0; i < sbmlModel.getNumCompartments(); i++){
                ...
                if(compMapping != null){
                    CastingUtils.CastInfo<Structure> ci = SBMLHelper.getTypedStructure(Structure.class, vcModel, cname);
                    if(ci.isGood()){
            ...
            for(org.sbml.jsbml.Parameter sbmlGlobalParam : sbmlModel.getListOfParameters()){
                ...
                if(!(paramType instanceof BoundaryCondition bCondn)){
                    continue;
                }
                ...
                if(bCondn.isSetVariable()){
                    ...
                    if(!(boundaryConditionTargetSTE instanceof SpeciesContext paramSpContext)){
                    ...
                    if(sm != null){
                        ...
                        for(CoordinateComponent coordComp : sbmlGeometry.getListOfCoordinateComponents()){
                            ...
                            if(vcellBCondParam != null){
                                if(bCondn.getParent() instanceof org.sbml.jsbml.Parameter sbmlBCondParam){
    ...
    private static org.sbml.jsbml.Parameter getSpatialParameter(org.sbml.jsbml.Model sbmlModel, String spatialId){
        org.sbml.jsbml.Parameter spatialParameter = null;
    ...
    private static SurfaceGeometricRegion getAssociatedSurfaceGeometricRegion(
        ...
        for(GeometricRegion vcGeometricRegion : geomeRegions){
            if(vcGeometricRegion instanceof SurfaceGeometricRegion surfaceRegion){
