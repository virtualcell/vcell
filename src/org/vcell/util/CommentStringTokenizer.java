...
/**
 * read line, stripping out all content with /* and * /
 * @return line, with comments removed
 * @throws IOException
 */
private String readLineWithoutComments() throws IOException {
	final String COMMENT_START = "/*";
	final String COMMENT_END = "*/";
	final int NOT_THERE = -1;
	String line = reader.readLine();
	int cmtStart = line.indexOf(COMMENT_START);
	while (cmtStart != NOT_THERE) {
		String headOfLine = line.substring(0, cmtStart);
		int cmtEnd = line.indexOf(COMMENT_END);
		if (cmtEnd == NOT_THERE) {
			while (cmtEnd == NOT_THERE) {
				 line = reader.readLine();
				 cmtEnd = line.indexOf(COMMENT_END);
			}
		}
		line = headOfLine + line.substring(cmtEnd + COMMENT_END.length());
		cmtStart = line.indexOf(COMMENT_START);
	}
	return line;
}
...
private void refreshCurrTokenizer() {
	...
	while (tokenLinkedList.size() == 0) { // read until there is something in the list
		...
		try {
			nextLine = readLineWithoutComments(); 
		} catch (IOException e) {			
			e.printStackTrace();
