...
public class OptXmlReader {	
	...
	private static OptRunResultSet getOptRunResultSet(Element optRunElement) throws DataConversionException {
		String bestObjectFuncValueString = optRunElement.getAttribute(OptXmlTags.OptSolverResultSetBestObjectiveFunction_Attr).getValue();
		double bestObjectFuncValue = Double.NaN;
		try {
			bestObjectFuncValue = Double.parseDouble(bestObjectFuncValueString);
		}catch (NumberFormatException e){
			if (bestObjectFuncValueString.contains("#INF")){
				if (bestObjectFuncValueString.contains("-1")){
					bestObjectFuncValue = Double.NEGATIVE_INFINITY;
				}else{
					bestObjectFuncValue = Double.POSITIVE_INFINITY;
				}
			}else{
				e.printStackTrace();
				throw e;
