	...
	for (int i = 0; i < sbmlModel.getNumInitialAssignments(); i++){
		try {
			...
			if (scs != null) {
				...
				localIssueList.add(new Issue(initAssgn, issueContext, IssueCategory.SBMLImport_UnsupportedAttributeOrElement , "Symbol '"+initAssgnSymbol+"' not a species or global parameter in VCell; initial assignment ignored.", Issue.SEVERITY_WARNING));
			...
			if (sbmlSpecies.isSetInitialConcentration()) { 		// If initial Concentration is set
				...
				if (initExpr == null) {
					...
					if (sbmlModel.getInitialAssignment(speciesName) == null) {
						localIssueList.add(new Issue(sbmlModel.getSpecies(speciesName), issueContext, IssueCategory.SBMLImport_MissingSpeciesInitCondition , "no initial condition for species "+speciesName+", assuming 0.0", Issue.SEVERITY_WARNING));
	...
	// For SBML level 2
		...
		for (int i = 0; i < listOfCompartments.size(); i++) {
			...
			if (dim == 3) {
				...
				if (modelVolumeUnit == null) {
					...
					localIssueList.add(new Issue(sbmlComp, issueContext, IssueCategory.Units, "unit for compartment '" + sbmlComp.getId() + "' (" + unitStr + ") : (" + sbmlUnitDefinition.getSymbol() + ") not compatible with current vol unit (" + modelVolumeUnit.getSymbol() + ")", Issue.SEVERITY_WARNING));
				...
				if (modelAreaUnit == null) {
					...
					localIssueList.add(new Issue(sbmlComp, issueContext, IssueCategory.Units, "unit for compartment '" + sbmlComp.getId() + "' (" + unitStr + ") : (" + sbmlUnitDefinition.getSymbol() + ") not compatible with current area unit (" + modelAreaUnit.getSymbol() + ")", Issue.SEVERITY_WARNING));
		...
		for (int i = 0; i < listOfSpecies.size(); i++) {
			...
			if (modelSubstanceUnit==null){
				...
				localIssueList.add(new Issue(sbmlSpecies, issueContext, IssueCategory.Units, "unit for species '" + sbmlSpecies.getId() + "' (" + unitStr + ") : (" + sbmlUnitDefinition.getSymbol() + ") not compatible with current substance unit (" + modelSubstanceUnit.getSymbol() + ")", Issue.SEVERITY_WARNING));
			...
			for (int i = 0; i < listOfReactions.size(); i++) {
				...
				if (kineticLaw != null) {
					...
					if (modelSubstanceUnit == null) {
						...
						localIssueList.add(new Issue(sbmlReaction, issueContext, IssueCategory.Units, "substance unit for reaction '" + sbmlReaction.getId() + "' (" + unitStr + ") : (" + sbmlUnitDefinition.getSymbol() + ") not compatible with current substance unit (" + modelSubstanceUnit.getSymbol() + ")", Issue.SEVERITY_WARNING));
					...
					if (modelTimeUnit == null) {
						...
						localIssueList.add(new Issue(sbmlReaction, issueContext, IssueCategory.Units, "time unit for reaction '" + sbmlReaction.getId() + "' (" + unitStr + ") : (" + sbmlUnitDefinition.getSymbol() + ") not compatible with current time unit (" + modelTimeUnit.getSymbol() + ")", Issue.SEVERITY_WARNING));
...
private void checkIdentifiersNameLength() throws Exception {
	...
	if (bLongCompartmentName || bLongSpeciesName || bLongParameterName || bLongReactionName) {
		...
		localIssueList.add(new Issue(issueSource, issueContext, IssueCategory.SBMLImport_UnsupportedAttributeOrElement, warningMsg, Issue.SEVERITY_WARNING));
...
protected void addGeometry() {
	...
	if (sbmlGeometry.getNumGeometryDefinitions() < 1) {
		localIssueList.add(new Issue(sbmlModel, issueContext, IssueCategory.SBMLImport_UnsupportedAttributeOrElement, "Geometry not deifned in spatial model.", Issue.SEVERITY_WARNING));
		return;
