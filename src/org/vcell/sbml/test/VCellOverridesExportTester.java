...
private static ODESolverResultSet solveSimulation(Simulation sim) {
	...
	try {
		...
		File cvodeInputFile = new File("C:\\Temp\\"+ TokenMangler.mangleToSName(sim.getName()) + ".cvodeInput");
		PrintWriter cvodePW = new PrintWriter(cvodeInputFile);
		cvodeFileWriter.writeIDAFile(cvodePW);
		cvodePW.close();
		
		// use the idastandalone solver
		File cvodeOutputFile = new File("C:\\Temp\\"+ TokenMangler.mangleToSName(sim.getName()) + ".cvode");
		...
		for (int k = 0; k < simContexts.length; k++){
			if (simContexts[k].getGeometry().getDimension() > 0 || simContexts[k].isStoch()) {
				...
				if (sims == null || sims.length == 0) { 
					...
					XmlUtil.writeXMLString(exportedSBMLStr, "C:\\SBMLRelated\\SBML_Testing\\SBMLValidationSuiteTests\\SBMLOverridesExportTests\\Rel_Results_4_25_08\\" + TokenMangler.mangleToSName(bioModel_1.getName()+"_"+simContexts[k].getName())+".xml");
					...
					for (int i = 0; i < sims.length; i++) {
						for (int j = 0; j < sims[i].getScanCount(); j++) {
							...
							//exportedSBMLStr = cbit.vcell.xml.XmlHelper.exportSBML(bioModel_1, 2, 1, simContexts[k].getName());
							XmlUtil.writeXMLString(exportedSBMLStr, "C:\\SBMLRelated\\SBML_Testing\\SBMLValidationSuiteTests\\SBMLOverridesExportTests\\Rel_Results_4_25_08\\" + TokenMangler.mangleToSName(bioModel_1.getName()+"_"+simContexts[k].getName()+"_"+sims[i].getName())+"_"+j+".xml");
							// Import the exported model
							bioModel_2 = (BioModel) cbit.vcell.xml.XmlHelper.importSBML(logger, exportedSBMLStr);
							
							// solve the original vcell model simulation.
							simJob.getWorkingSim().getSolverTaskDescription().setSolverDescription(SolverDescription.CVODE);
