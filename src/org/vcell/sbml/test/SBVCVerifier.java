...
private static ODESolverResultSet solveSimulation(Simulation sim, boolean hasFastSystems) {
	...
	//
	// If hasFastSystems in true, use Forward Euler solver, else use CVOde solver
	//
	...
	if (hasFastSystems) {
		...
		// Generate .cvodeInput string - On 4.4, using IDAFileWriter instead of 
		// CVODEFileWriter since the latter doesn't exist and the format for both files are almost same.
		try {
