...
/*   GraphEditUnhideNeighboursAction  --- by Oliver Ruebenacker, UCHC --- July 2007 to May 2010
...
public class GraphEditUnhideNeighborsAction<S extends UIShape<S>, G extends UIGraph<S, G>> 
extends GraphAction<S, G> {

	private static final long serialVersionUID = 6516914979839161661L;

	public GraphEditUnhideNeighborsAction(ActionSpecs newSpecs, CoreManager coreManager,
	...
	public void actionPerformed(ActionEvent event) {
		if(! (graph().model().selectedComps().isEmpty())) {
			...
			while(compIter.hasNext()) {
				graph().unhideNeighbors(compIter.next(), Visibility.hiderSelection);
			}
			updateUI();
		} else {
			S shape = graph().chosenShape();
			if(shape instanceof GraphShape) {
				graph().unhideNeighbors(shape.graphComp(), Visibility.hiderSelection);
				updateUI();					
			}
		} 
	}
