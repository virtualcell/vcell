...
public void touchSimulationJobStatus(Connection con, SimulationJobStatus simulationJobStatus) throws SQLException {
	if (simulationJobStatus == null || con == null){
		throw new IllegalArgumentException("Improper parameters for touchSimulationJobStatus()");
	}

	log.print("SimulationJobDbDriver.touch(simKey="+simulationJobStatus.getVCSimulationIdentifier().getSimulationKey()+")");
	
	String sql = "UPDATE " + jobTable.getTableName() +	" SET "  + jobTable.latestUpdateDate.getUnqualifiedColName()+"=sysdate " + 
			" WHERE " + jobTable.simRef + "=" + simulationJobStatus.getVCSimulationIdentifier().getSimulationKey() +
			" AND " + jobTable.jobIndex + "=" + simulationJobStatus.getJobIndex();
	//log.print(sql);			
	executeUpdate(con,sql);
