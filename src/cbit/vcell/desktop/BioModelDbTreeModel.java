...
private BioModelNode createBaseTree() throws DataAccessException {
	...
	for (int ownerIndex=0;ownerIndex<ownerList.size();ownerIndex++){
		...
		BioModelNode ownerNode = createOwnerSubTree(owner, bioModelInfos);
 ...
 * @param docManager cbit.vcell.clientdb.DocumentManager
 */
private BioModelNode createOwnerSubTree(User owner, BioModelInfo bioModelInfos[]) throws DataAccessException {
	//
	// for each user
	//
