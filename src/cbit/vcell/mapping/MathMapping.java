			...
			for (int j = 0; j < modelParameters.length; j++){
				...
				for (int k = 0; k < structureMappings.length; k++) {
					...
					if (modelParameters[j].getExpression().getSymbols() == null) {
						...
						// first pass enter dummy expr, so hash entry for any global variant is not null.
						// Second pass - check validity of global variant and create appropriate expression and add to hash.
									...
									if (sc != null) {
										...
										if (sp_struct.compareEqual(sm_struct)) {
											...
											if ((sm_struct instanceof Feature) && (sp_struct instanceof Membrane)) {
												...
												bValid = bValid && false;
												break;
											}	// end if ('sm' is feature and 'sp' is membrane)
										}	// end if (sp == sm)
									}	// end sc != null 
								}	// end (for ii)
							}	// end symbols != null
