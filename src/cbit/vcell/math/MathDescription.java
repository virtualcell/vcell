	...
	try {
		...
		while (tokens.hasMoreTokens()){
			...
			if (token.equalsIgnoreCase(VCML.VolumeVariable)){
				token = tokens.nextToken();
				VolVariable var = new VolVariable(Variable.getNameFromCombinedIdentifier(token));
...
//			addVariable0(outside);
			...
			if (token.equalsIgnoreCase(VCML.MembraneVariable)){
				token = tokens.nextToken();
				MemVariable var = new MemVariable(Variable.getNameFromCombinedIdentifier(token));
				varHash.addVariable(var);
				continue;
			}
			if (token.equalsIgnoreCase(VCML.FilamentVariable)){
				token = tokens.nextToken();
				FilamentVariable var = new FilamentVariable(Variable.getNameFromCombinedIdentifier(token));
				varHash.addVariable(var);
				continue;
			}
			if (token.equalsIgnoreCase(VCML.VolumeRegionVariable)){
				token = tokens.nextToken();
				VolumeRegionVariable var = new VolumeRegionVariable(Variable.getNameFromCombinedIdentifier(token));
				varHash.addVariable(var);
				continue;
			}
			if (token.equalsIgnoreCase(VCML.MembraneRegionVariable)){
				token = tokens.nextToken();
				MembraneRegionVariable var = new MembraneRegionVariable(Variable.getNameFromCombinedIdentifier(token));
				varHash.addVariable(var);
				continue;
			}
			if (token.equalsIgnoreCase(VCML.FilamentRegionVariable)){
				token = tokens.nextToken();
				FilamentRegionVariable var = new FilamentRegionVariable(Variable.getNameFromCombinedIdentifier(token));
				varHash.addVariable(var);
				continue;
			}
			...
			{
				token = tokens.nextToken();
				StochVolVariable var = new StochVolVariable(Variable.getNameFromCombinedIdentifier(token));
				varHash.addVariable(var);
				continue;
			}
			if (token.equalsIgnoreCase(VCML.Function)){
				token = tokens.nextToken();
				Expression exp = new Expression(tokens);
				Function function = new Function(Variable.getNameFromCombinedIdentifier(token),exp);
			...
			if (token.equalsIgnoreCase(VCML.VolumeRandomVariable)) {
				token = tokens.nextToken();
				RandomVariable randomVariable = new VolumeRandomVariable(Variable.getNameFromCombinedIdentifier(token), this, tokens);
				varHash.addVariable(randomVariable);
				continue;
			}
			if (token.equalsIgnoreCase(VCML.MembraneRandomVariable)) {
				token = tokens.nextToken();
				RandomVariable randomVariable = new MembraneRandomVariable(Variable.getNameFromCombinedIdentifier(token), this, tokens);
				varHash.addVariable(randomVariable);
				continue;
			}
