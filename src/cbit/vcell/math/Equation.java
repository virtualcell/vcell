 ...
 */
public final Enumeration<Variable> getRequiredVariables(Simulation simulation) throws Exception {
	Vector<Variable> requiredVarList = new Vector<Variable>();
	Vector<Expression> expList = getExpressions(simulation.getMathDescription());
...
public boolean isUsed(Simulation simulation, Variable var) throws Exception {
	Enumeration<Variable> enum1 = getRequiredVariables(simulation);
	while (enum1.hasMoreElements()){
		Variable varTmp = enum1.nextElement();
		if (varTmp == var){
			return true;
		}
	}		
...
public void getDiscontinuities(Simulation simulation, Vector<Discontinuity> discontinuities) throws ExpressionException, MathException {
	Vector<Expression> expressions = getExpressions(simulation.getMathDescription());
	for (Expression exp : expressions) {
		Expression exp1 = new Expression(exp);
		exp1.bindExpression(simulation);
		exp1 = simulation.substituteFunctions(exp1).flatten();
