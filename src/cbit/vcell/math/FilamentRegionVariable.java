/*
 * Copyright (C) 1999-2011 University of Connecticut Health Center
 *
 * Licensed under the MIT License (the "License").
 * You may not use this file except in compliance with the License.
 * You may obtain a copy of the License at:
 *
 *  http://www.opensource.org/licenses/mit-license.php
 */

package cbit.vcell.math;
/**
 * This class was generated by a SmartGuide.
 * 
 */
public class FilamentRegionVariable extends Variable {
/**
 * MembraneVariable constructor comment.
 * @param name java.lang.String
 */
public FilamentRegionVariable(String name, Domain domain) {
	super(name,domain);
}


/**
 * This method was created in VisualAge.
 * @return boolean
 * @param obj Matchable
 */
public boolean compareEqual(org.vcell.util.Matchable obj,boolean bIgnoreMissingDomain) {
	if (!(obj instanceof FilamentRegionVariable)){
		return false;
	}
	if (!compareEqual0(obj,bIgnoreMissingDomain)){
		return false;
	}
	
	return true;
}


/**
 * This method was created by a SmartGuide.
 * @return java.lang.String
 */
public String getVCML() {
	return VCML.FilamentRegionVariable+"   "+getQualifiedName();
}
}
