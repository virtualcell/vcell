 ...
 */
public class AsynchProgressPopup_orig extends AsynchGuiUpdater {
	private ProgressDialog_orig dialog = null;
 ...
 * 
 */
public AsynchProgressPopup_orig(Component requester, String title, String message, boolean inputBlocking, boolean knowsProgress) {
 ...
 * @param knowsProgress boolean
 */
public AsynchProgressPopup_orig(Component requester, String title, String message, boolean inputBlocking, 
...
private ProgressDialog_orig getDialog() {
	if (dialog == null) {
		Frame owner = javax.swing.JOptionPane.getFrameForComponent(requester);
		dialog = new ProgressDialog_orig(owner);
	...
	new SwingDispatcherAsync (){
		public void runSwing() {
			...
			if (knowsProgress) {
				...
				AsynchProgressPopup_orig.this.progress = localProgress;
			}
		}
		public void handleException(Throwable e) {
			e.printStackTrace();
...
private void startPrivate(final boolean bKeepOnTop) {
	new SwingDispatcherAsync (){
		public void runSwing() {
			...
			if (! knowsProgress) {
				AsynchProgressPopup_orig.super.start();
			}			
			if(bKeepOnTop){
...
public void stop() {
	new SwingDispatcherAsync (){
		public void runSwing() {
			...
			if (! knowsProgress) {
				AsynchProgressPopup_orig.super.stop();
			}
			getDialog().dispose();
		}
		public void handleException(Throwable e) {
