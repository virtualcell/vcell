...
//		frapParameterEstimatesLabel.setFont(new Font("", Font.BOLD, 14));
		...
		bleachEstimationComboBox.setPreferredSize(new Dimension(125, 25));
		bleachEstimationComboBox.setMinimumSize(new Dimension(125, 25));
		...
		scrollPane.setPreferredSize(new Dimension(75, 0));
...
//		panel_1.setBorder(new LineBorder(Color.black, 1, false));
		...
		gridBagConstraints_24.insets = new Insets(2, 2, 2, 2);
...
//					FrapDataAnalysisResults.BLEACH_TYPE_NAMES[bleachEstimationComboBox.getSelectedIndex()]+"')");
	...
	private void displayFit(FrapDataAnalysisResults frapDataAnalysisResults,double[] frapDataTimeStamps){
		if (frapDataAnalysisResults == null){
			...
			multisourcePlotPane.forceXYRange(
				new Range(0,frapDataTimeStamps[frapDataTimeStamps.length-1]),
				new Range(0,1.1));
		}
		table.repaint();
	}
