		...
		try {
			...
			exportServiceImpl.addExportListener(new ExportListener() {
				public void exportMessage(ExportEvent event) {
					...
					for (int i = 0; aExportListener != null && i < aExportListener.size(); i++) {
						ExportListener expListener = (ExportListener)aExportListener.elementAt(i);
						expListener.exportMessage(event);
					}
				}
			});
