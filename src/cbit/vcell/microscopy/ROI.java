...
//		ROI_EXTRACELLULAR
	 ...
	 */
	public ROI(int numX, int numY, int numZ, Origin origin, Extent extent, RoiType argROIType) throws ImageException {
		super();
		roiImages = new UShortImage[numZ];
		for (int i = 1; i < numZ; i++) {
			roiImages[i] = new UShortImage(new short[numX*numY],origin,extent,numX,numY,1);
		}
		...
		if(argRoiImage.getNumZ() > 1){
			try{
				...
				for (int z = 0; z < argRoiImage.getNumZ(); z++) {
					...
					Origin newOrigin = new Origin(argRoiImage.getOrigin().getX(),argRoiImage.getOrigin().getY(),argRoiImage.getOrigin().getZ() + z*(newExtent.getZ()));
