...
class SimulationWarning implements PropertyChangeListener {
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if (evt.getPropertyName() == Simulation.PROPERTY_NAME_SOLVER_TASK_DESCRIPTION
				&& evt.getOldValue() != evt.getNewValue()) {
			changeStrategy = DEFAULT_STRATEGY;
			SolverTaskDescription std = simulation.getSolverTaskDescription();
			if (std != null) {
				changeStrategy = changeStrategyFactory(std.getSolverDescription());
		...
		public void gatherIssues(IssueContext issueContext, List<Issue> issueList) {
			...
			synchronized (errors) {
				for (String text : errors) {
					Issue i = new Issue(simulation, issueContext, IssueCategory.SMOLYDN_DIFFUSION, text,
