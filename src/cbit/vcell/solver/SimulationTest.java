...
public static void testEquivalenceCapability(){
	try {
		cbit.vcell.solver.Simulation sim = cbit.vcell.solver.SimulationTest.getExample();
		cbit.vcell.solver.Simulation sim2 = null;
		long time_1 = System.currentTimeMillis();
		int numIterations = 500;
		// copy by cloneSerializable()
		for (int i = 0; i < numIterations; i++){
			sim2 = (cbit.vcell.solver.Simulation)org.vcell.util.BeanUtils.cloneSerializable(sim);
		}
		long time_2 = System.currentTimeMillis();
		// copy by copy-constructor
		for (int i = 0; i < numIterations; i++){
			sim2 = new cbit.vcell.solver.Simulation(sim,true);
		}
