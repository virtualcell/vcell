		...
		for (int i=0;i<users.length;i++)
		{
			...
			for (int j = 0; j < bioModelInfos.length; j++){
				...
				try {
					...
					log.exception(e2);
					logFilePrintStream.println("Biomodel : " + bioModelInfos[j].getVersion().getName() + " : \t"+ e2.getMessage() + "\n");
		...
		for (int i=0;i<users.length;i++)
		{
			...
			for (int j = 0; j < geoInfos.length; j++){
				...
				try {
					...
					log.exception(e2);
					logFilePrintStream.println("Geometry : " + geoInfos[j].getVersion().getName() + " : \t"+ e2.getMessage() + "\n");
					if (bAbortOnDataAccessException){
						throw e2;
					}
				}
