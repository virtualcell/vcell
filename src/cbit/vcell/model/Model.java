...
public void propertyChange(java.beans.PropertyChangeEvent evt) {
	...
	if (evt.getSource() instanceof ModelParameter && evt.getPropertyName().equals("name")){
		...
		// if modelparameter with 'oldName' is used in expr of another param, 
		// replace 'oldName' with 'newName' in each of those param expressions 
		for (int i = 0; i < fieldModelParameters.length; i++){
			try {
				Expression exp = new Expression(fieldModelParameters[i].getExpression());
				if (exp.hasSymbol(oldName)) {
					exp.substituteInPlace(new Expression(oldName), new Expression(newName));
					exp.bindExpression(this);
					fieldModelParameters[i].setExpression(exp);
				}
			} catch (ExpressionException e) {
