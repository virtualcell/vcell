...
private ArrayList<FluxReaction> getFluxReactionBySpeciesContext(SpeciesContext speciesContext) {
	ArrayList<FluxReaction> fluxRxnsList = new ArrayList<FluxReaction>();
	for (ReactionStep rs : fieldReactionSteps) {
		if (rs instanceof FluxReaction) {
			FluxReaction fluxRxn = (FluxReaction)rs;
			Membrane fluxMembrane = (Membrane)fluxRxn.getStructure();
			Structure speciesContextStruct = speciesContext.getStructure();
			boolean isAdjacentStructure = false;
			if (fluxMembrane.getInsideFeature().equals(speciesContextStruct) || fluxMembrane.getOutsideFeature().equals(speciesContextStruct)) {
				isAdjacentStructure = true;
			}
			if (fluxRxn.getFluxCarrier().compareEqual(speciesContext.getSpecies()) && isAdjacentStructure) {
				fluxRxnsList.add(fluxRxn) ;
			}
		}
	}
	return fluxRxnsList;
...
private String getFreeSpeciesName() {
	int count=0;
	String speciesName = null;
	while (true) {
		speciesName = "ss" + count;
 ...
 * @deprecated
 * 
 * This method is used only when connecting a spContextStart from startFeature to a spContextEnd in endFeature (separated by a membrane) in reactionCartoonTool, 
 * when spContextEnd does not have same species as spContextStart. 
 */
public void changeSpeciesInSpeciesContext(SpeciesContext spContextEnd, SpeciesContext spContextStart) throws IllegalArgumentException, PropertyVetoException {
	
	// if speciesContext is not being used in any reaction, change species in spContextEnd,
	ArrayList<FluxReaction> fluxRxnsForSpContextStart = getFluxReactionBySpeciesContext(spContextStart);
	ArrayList<FluxReaction> fluxRxnsForSpContextEnd = getFluxReactionBySpeciesContext(spContextEnd);
	if (fluxRxnsForSpContextEnd.isEmpty()  && fluxRxnsForSpContextStart.isEmpty()) { //both sides are not involved in fluxes
		Species species = addSpecies(new Species(getFreeSpeciesName(), null));
		spContextStart.setSpecies(species);
		spContextEnd.setSpecies(species);
	} else {
		if (fluxRxnsForSpContextEnd.size() > 0 && fluxRxnsForSpContextStart.isEmpty()) { //end side is part of other flux
			// check if species of speciesContextEnd has a speciesContext in start structure
			Species speciesEnd = spContextEnd.getSpecies();
			SpeciesContext speciesContextWithStartStructureEndSpecies = getSpeciesContext(speciesEnd, spContextStart.getStructure());
			if (speciesContextWithStartStructureEndSpecies == null) {
				spContextStart.setSpecies(speciesEnd);
			} else {
				// if the speciesContext needed for speciesEnd is already present in endStruct, but not used in any other flux reactions, grab it
				if (getFluxReactionBySpeciesContext(speciesContextWithStartStructureEndSpecies).isEmpty()) {
					Species species = addSpecies(new Species(getFreeSpeciesName(), null));
					// if speciesContextWithStartStructureEndSpecies does not have any flux reactions associated with it, change its species
					speciesContextWithStartStructureEndSpecies.setSpecies(species);
					// convert the species oc spContextStart to same as spContextEnd.
					spContextStart.setSpecies(speciesEnd);
				} else {
					// throw new IllegalArgumentException("Cannot create a flux between two different species");
				}
			}
		} else if (fluxRxnsForSpContextStart.size() > 0 && fluxRxnsForSpContextEnd.isEmpty()) { //start side is part of other flux
			// check if species of speciesContextStart has a speciesContext in end structure
			Species speciesStart = spContextStart.getSpecies();
			SpeciesContext speciesContextWithStartSpeciesEndStructure = getSpeciesContext(speciesStart, spContextEnd.getStructure());
			if (speciesContextWithStartSpeciesEndStructure == null) {
				spContextEnd.setSpecies(speciesStart);
			} else {
				// if the speciesContext needed for speciesStart is already present in startStruct, but not used in any other flux reactions, grab it
				if (getFluxReactionBySpeciesContext(speciesContextWithStartSpeciesEndStructure).isEmpty()) {
					Species species = addSpecies(new Species(getFreeSpeciesName(), null));
					// if speciesContextWithStartSpeciesEndStructure does not have any flux reactions associated with it, change its species
					speciesContextWithStartSpeciesEndStructure.setSpecies(species);
					// convert the species oc spContextStart to same as spContextEnd.
					spContextEnd.setSpecies(speciesStart);
				} else {
					// throw new IllegalArgumentException("Cannot create a flux between two different species");
		...
		} else { // both side are involved in two different fluxes
			if (!spContextStart.getSpecies().compareEqual(spContextEnd.getSpecies())) {
				// throw new IllegalArgumentException("Cannot create a flux between two different species");
			}
		}
	}
