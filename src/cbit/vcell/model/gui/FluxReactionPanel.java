...
//				FocusListener,
		...
		public void actionPerformed(java.awt.event.ActionEvent e) {
			if (e.getSource() == FluxReactionPanel.this.getJComboBox1()) 
				connEtoC3();
		};
		public void propertyChange(java.beans.PropertyChangeEvent evt) {
			if (evt.getSource() == FluxReactionPanel.this.getFluxReaction1() && (evt.getPropertyName().equals("fluxCarrier"))) 
				connEtoM7(evt);
			if (evt.getSource() == FluxReactionPanel.this.getFluxReaction1() && (evt.getPropertyName().equals("name"))) 
				connPtoP1SetTarget();
			if (evt.getSource() == FluxReactionPanel.this.getFluxReactionNameTextField() && (evt.getPropertyName().equals("text"))) 
				connPtoP1SetSource();
			if (evt.getSource() == FluxReactionPanel.this.getFluxReaction1() && (evt.getPropertyName().equals("kinetics"))) 
				connEtoM10(evt);
		};
...
public FluxReactionPanel(boolean pSubset) {
	super();
	subset = pSubset;
...
//		ivjJDialogContentPane.setLayout(new java.awt.GridBagLayout());
		...
		constraintsJLabel8.gridx = 0; constraintsJLabel8.gridy = gridy;
 ...
 */
/* WARNING: THIS METHOD WILL BE REGENERATED. */
public void setFluxReaction1(FluxReaction newValue) {
			...
			public void actionPerformed(java.awt.event.ActionEvent e) {
				...
				if (getKinetics() instanceof DistributedKinetics){
					try {
						...
						if (getKinetics().getKineticsDescription().isElectrical()){
							DialogUtils.showErrorDialog(FluxReactionPanel.this,"failed to translate into General Current Kinetics [pA]: "+e2.getMessage());
						}else{
							DialogUtils.showErrorDialog(FluxReactionPanel.this,"failed to translate into General Lumped Kinetics [molecules/s]: "+e2.getMessage());
						}
					}
				}else if (getKinetics() instanceof LumpedKinetics){
					try {
						...
						if (getKinetics().getKineticsDescription().isElectrical()){
							DialogUtils.showErrorDialog(FluxReactionPanel.this,"failed to translate into General Current Density Kinetics [pA/um2]: "+e2.getMessage());
						}else{
							DialogUtils.showErrorDialog(FluxReactionPanel.this,"failed to translate into General Kinetics [molecules/um2.s]: "+e2.getMessage());
						}
					}
				}
			}
