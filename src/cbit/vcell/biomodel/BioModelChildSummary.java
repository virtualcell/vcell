...
cbit.vcell.mapping.SimulationContext[] simContexts = savedBioModel.getSimulationContexts();
	...
	bmcs.appTypes = new String[simContexts.length];
	bmcs.scAnnots = new String[simContexts.length];
	bmcs.geoNames = new String[simContexts.length];
	bmcs.geoDims = new int[simContexts.length];
	bmcs.simNames = new String[simContexts.length][];
	bmcs.simAnnots = new String[simContexts.length][];
	
	for(int i=0;i<simContexts.length;i+= 1){
		bmcs.scNames[i] = simContexts[i].getName();
		bmcs.appTypes[i] = getAppType(simContexts[i].getMathDescription());
...
public static String getAppType(MathDescription md)
{
...
public static BioModelChildSummary fromDatabaseSerialization(String databaseSerialization) {
	...
	try{
		...
		while(st.hasMoreElements()){
			...
			{
				appTypesV.add(TokenMangler.getChildSummaryElementRestoredString(tokenStr));
...
public String toDatabaseSerialization() {
	...
	for(int i = 0; i < scNames.length;i+= 1){
		...
		sb.append(emptyConvention(TokenMangler.getChildSummaryElementEscapedString(appTypes[i]))+"\n");
