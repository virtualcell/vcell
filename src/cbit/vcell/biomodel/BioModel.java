	...
	public String getObjectPathDescription(Object source) {
		...
		if (source instanceof SymbolTableEntry) {
			...
			try {
				...
				description = "App(" + simulationContext.getNameScope().getPathDescription() + ") / " + ApplicationPanelTabID.simulations.getTitle();
			} catch (ObjectNotFoundException e) {
				e.printStackTrace();
				description = "App(" + "unknown" + ") / " + ApplicationPanelTabID.simulations.getTitle();
			}
