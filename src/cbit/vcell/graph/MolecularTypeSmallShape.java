...
//			MolecularComponentPattern mcp = mtp.getComponentPatternList().get(i);
			...
			MolecularComponentSmallShape mlcls = new MolecularComponentSmallShape(100, 50, mcp, shapeManager, graphicsContext, owner, this, issueManager);
...
//			MolecularComponentPattern mcp = mtp.getComponentPatternList().get(i);
			MolecularComponent mc = mt.getComponentList().get(i);
			MolecularComponentPattern mcp = mtp.getMolecularComponentPattern(mc);
			MolecularComponentSmallShape mcss = new MolecularComponentSmallShape(rightPos, y-2, mcp, shapeManager, graphicsContext, owner, this, issueManager);
	...
	public MolecularTypeSmallShape(int xPos, int yPos, MolecularType mt, SmallShapeManager shapeManager, Graphics graphicsContext, 
		...
		for(int i=numComponents-1; i >=0; i--) {
			...
			MolecularComponentSmallShape mlcls = new MolecularComponentSmallShape(100, 50, mc, shapeManager, graphicsContext, owner, this, issueManager);
			offsetFromRight += mlcls.getWidth() + MolecularComponentSmallShape.componentSeparation;
		}
		...
		for(int i=numComponents-1; i >=0; i--) {
			...
			// now that we know the dimensions of the molecular type shape we create the component shapes
			MolecularComponent mc = mt.getComponentList().get(i);
			MolecularComponentSmallShape mcss = new MolecularComponentSmallShape(rightPos, y-2, mc, shapeManager, graphicsContext, owner, this, issueManager);
			offsetFromRight += mcss.getWidth() + MolecularComponentSmallShape.componentSeparation;
			componentShapes.add(0, mcss);
		}
	}
