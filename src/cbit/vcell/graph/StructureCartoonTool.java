	...
	private void showMoveDialog(FeatureShape featureShape) {
		...
		try{
			...
			if(validDestinations == null || validDestinations.length == 0){
				DialogUtils.showErrorDialog(getGraphPane(), "No valid Destinations for Feature '"+featureShape.getFeature().getName()+"'");
			}else{
				...
				if(finalDestination == null){
					DialogUtils.showErrorDialog(getGraphPane(), "Feature with name "+featureSelection+" not found in model");
					return;
				}
				...
				if(neededSCArr != null){
					...
					if(message.length() > 0){
						String result =
							DialogUtils.showWarningDialog(getJDesktopPane(),
				...
				if(cleanupWantedV.size() > 0){
					...
					for(int i=0;i<cleanupWantedV.size();i+= 1){
						if(message.length() > 0){message+= "\n";}
						message+= "     "+(cleanupWantedV.elementAt(i)).getName();
					}
					if(message.length() > 0){
						String result =
							DialogUtils.showWarningDialog(getJDesktopPane(),
						...
						if(result.equals(UserMessage.OPTION_YES)){
							for(int i=0;i<cleanupWantedV.size();i+= 1){
								try{
									featureShape.getModel().removeSpeciesContext(cleanupWantedV.elementAt(i));
								}catch(Throwable e){
