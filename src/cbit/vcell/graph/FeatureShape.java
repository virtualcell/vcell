...
public Point getAttachmentLocation(int attachmentType) {
	return new Point(screenPos.x+screenSize.width/2,screenPos.y+labelSize.height*5/4);
}
...
public Dimension getPreferedSize_Testing(java.awt.Graphics2D g) {
	...
	try{
		...
		if (childShapeList.size()>0){
			...
			if(structCount == 0){
				preferedSize.width = box.width +defaultSpacingX*2;
				preferedSize.height = box.height+defaultSpacingY*2;
			}else{
				preferedSize.width = childStructureMaxWidth + box.width + defaultSpacingX*2;
				preferedSize.height = Math.max(childStructureTotalHeight,box.height) + defaultSpacingY*2;
			}
		}else{
			preferedSize.width = labelSize.width + defaultSpacingX*2;
			preferedSize.height = labelSize.height + defaultSpacingY*2;
...
public void layout() throws LayoutException {
	...
	int spacingY;
	int extraSpacingY;
	int currentX = screenSize.width/2 - memWidth/2;
	int spacingX = defaultSpacingX;
	//
	// position column2 (membranes)
	//
	totalSpacingY = (screenSize.height-currentY) - memHeight;
	if (totalSpacingY<0){
	...
	for (int i=0;i<childShapeList.size();i++){
		...
		if (child instanceof StructureShape){
			// Dimension childDim = child.screenSize;
			child.screenPos.x = centerX - child.screenSize.width/2;
			child.screenPos.y = col2Y;
			col2Y += child.screenSize.height + spacingY;
		}	
	}
	if (col2Y != screenSize.height){
		throw new RuntimeException("layout for column2 incorrect ("+getLabel()+"), currentY="+currentY+", screenSize.height="+screenSize.height);
	}
	if (memCount>0){
			...
			if (child instanceof StructureShape){
				java.awt.geom.Area childArea =
					new java.awt.geom.Area(new java.awt.geom.Ellipse2D.Double(
						child.screenPos.x+screenPos.x,
						child.screenPos.y+screenPos.y,
						child.screenSize.width,child.screenSize.height));
		...
		while(true){
			...
			for (int i=0;i<childShapeList.size();i++){
				if (childShapeList.elementAt(i) instanceof SpeciesContextShape){
					...
					final int XSTEP = child.screenSize.width + boxSpacingX;
					final int YSTEP = child.screenSize.height + boxSpacingY;
					while(
					...
					if(!bLayoutFailed){
						child.screenPos.x = boxX;
						child.screenPos.y = boxY;
						boxY+= YSTEP;
					}else{
						break;
					}
