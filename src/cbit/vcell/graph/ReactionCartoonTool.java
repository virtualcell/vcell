...
public class ReactionCartoonTool extends BioCartoonTool {
	...
	@Override
	protected void menuAction(Shape[] shapes, String menuAction) {
			...
			try {
				if(shapes.length > 1){
					...
					if(graphModel instanceof ModelCartoon){
						Model currentModel = ((ModelCartoon)graphModel).getModel();
						Vector<Shape> shapesV = new Vector<Shape>();
						shapesV.addAll(Arrays.asList(shapes));
						for (int i = 0; i < shapes.length; i++) {
							Shape currentShape = shapes[i];
							if(currentShape instanceof ReactionStepShape){
								ReactionStep currentReactstep = ((ReactionStepShape) currentShape).getReactionStep();
//								System.out.println("----Reaction: "+currentReactstep.getName());
								shapesV.remove(currentShape);
								currentModel.removeReactionStep(currentReactstep);
							}
						}
						Vector<SpeciesContext> vetoedSpeciesContextV = new Vector<SpeciesContext>();
						for (int i = 0; i < shapes.length; i++) {
							Shape currentShape = shapes[i];
							if(currentShape instanceof SpeciesContextShape){
								SpeciesContext currentSpeciesconContext = ((SpeciesContextShape) currentShape).getSpeciesContext();
//								System.out.println("----SpeciesContext: "+currentSpeciesconContext.getName());
								shapesV.remove(currentShape);
								try{
									currentModel.removeSpeciesContext(currentSpeciesconContext);
								}catch(PropertyVetoException pve){
//									System.out.println("--Delete Vetoed: "+currentSpeciesconContext.getName());
									vetoedSpeciesContextV.add(currentSpeciesconContext);
								}
							}
						}
//						for (int i = 0; i < shapesV.size(); i++) {
//							System.out.println("leftovers: "+shapesV.get(i).getClass().getCanonicalName()+" "+shapesV.get(i).toString());
//						}
						if(vetoedSpeciesContextV.size() > 0){
							StringBuffer vetoMessage = new StringBuffer();
							vetoMessage.append("The following Species were not deleted because of dependencies:");
							for (int i = 0; i < vetoedSpeciesContextV.size(); i++) {
								vetoMessage.append("\n"+vetoedSpeciesContextV.get(i).getName());
							}
							DialogUtils.showWarningDialog(getGraphPane(), vetoMessage.toString());
						}
						return;
					}
				}
			...
			case SELECT: {
				if (event.getClickCount() == 2) {
					...
					if (selectedShape != null) {
						menuAction(new Shape[] {selectedShape}, CartoonToolMiscActions.Properties.MENU_ACTION);
					}
				}
				break;
			}
					...
					if (endShape instanceof SimpleReactionShape){
						...
						if(startShapeObject instanceof SpeciesContext) {
							...
							if(StructureUtil.reactionHereCanHaveParticipantThere(
									simpleReaction.getStructure(), speciesContext.getStructure())) {
								int stoichiometry = 1;
							...
							if(StructureUtil.reactionHereCanHaveParticipantThere(
									structureReactionStart, structureReaction)) {
								structureSpecies = structureReaction;
							} else if(StructureUtil.reactionHereCanHaveParticipantThere(
									structureReaction, structureReactionStart)) {
								structureSpecies = structureReactionStart;
							}
							if(structureSpecies != null) {
								SpeciesContext speciesContext = 
									getReactionCartoon().getModel().createSpeciesContext(structureSpecies);
							...
							if(StructureUtil.reactionHereCanHaveParticipantThere(
								...
								SpeciesContext speciesContext =  
									getReactionCartoon().getModel().createSpeciesContext(structure);
								simpleReaction.addReactant(speciesContext, 1);
								getReactionCartoon().notifyChangeEvent();
							...
							if(StructureUtil.reactionHereCanHaveParticipantThere(
								...
								for(ReactionParticipant participant : 
									...
									if(participant instanceof Product && 
											participant.getSpeciesContext().equals(speciesContextEnd)) {
								...
								if(product != null) {
									...
									simpleReaction.addProduct(speciesContextEnd, stoichiometry);								
								}
								getReactionCartoon().notifyChangeEvent();
							}
							break;
						...
						if(startObject instanceof SimpleReaction) {
							...
							if(StructureUtil.reactionHereCanHaveParticipantThere(
									reaction.getStructure(), endStructure)) {
								SpeciesContext speciesContext = 
				...
				if (bLineStretch
					...
					if(startObject instanceof ReactionStep) {
						...
						if(endObject instanceof ReactionStep) {
							if(StructureUtil.reactionHereCanHaveParticipantThere(
									((ReactionStep) endObject).getStructure(), 
									speciesContext.getStructure())) {
						...
						if(endObject instanceof ReactionStep) {
							reactionStep = (ReactionStep) endObject;
							if(StructureUtil.reactionHereCanHaveParticipantThere(
									reactionStep.getStructure(), startStructure)) {
								speciesContext = 
									getReactionCartoon().getModel().createSpeciesContext(startStructure);
								getReactionCartoon().notifyChangeEvent();
							...
							speciesContext = (SpeciesContext) endObject;
							if(StructureUtil.reactionHereCanHaveParticipantThere(
									startStructure, speciesContext.getStructure())) {
								reactionStep = 
