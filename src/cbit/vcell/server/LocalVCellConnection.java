...
public LocalVCellConnection(UserLoginInfo userLoginInfo, String host, SessionLog sessionLog, LocalVCellServer aLocalVCellServer) throws RemoteException, java.sql.SQLException, FileNotFoundException {
	...
	sessionLog.print("new LocalVCellConnection(" + getUserLoginInfo().getUser().getName() + ")");
	...
	PerformanceMonitoringFacility pmf = new PerformanceMonitoringFacility(getUserLoginInfo().getUser(), sessionLog);
...
private VCellConnection getSimDataServerRemoteConnection() {
	...
	if (connectionPool != null) {
		try {
			VCellConnection vcConn = connectionPool.getSimDataServerVCellConnection(getUserLoginInfo());
			//
...
public UserMetaDbServer getUserMetaDbServer() throws RemoteException, DataAccessException {
	...
	if (userMetaDbServer == null) {
		userMetaDbServer = new cbit.vcell.modeldb.LocalUserMetaDbServer(conFactory, keyFactory, dbCacheTable, getUserLoginInfo().getUser(), getSessionLog());
	}
