...
public MediaMovie(MediaTrack[] tracks, int duration) {
	...
	setTimeScale(Atoms.defaultTimeScale);
	...
	for (int i=0;i<tracks.length;i++) {
		for (int j=0;j<tracks[i].getChunks().length;j++) {
			if (tracks[i].getChunks()[j].getDataReference().equals("self"))
				getSelfreferencedChunks().addElement(tracks[i].getChunks()[j]);
		}
	}
}
/**
 ...
 */
public Vector getSelfreferencedChunks() {
	return selfreferencedChunks;
}
/**
 * This method was created in VisualAge.
 * @return int
