				...
				if (compartment.getSpatialDimensions() == 3) {
					...
					if (outsideStructure instanceof Membrane) {
						...
						if (compartment.isSetSize() && compartment.getSize() > 0.0) {
							...
							logger.sendMessage(VCLogger.MEDIUM_PRIORITY, TranslationMessage.COMPARTMENT_ERROR, "compartment "+compartment.getId()+" size is not set.");
						}
		...
		if (!(vcRxn instanceof FluxReaction)) {
			...
			for (int j = 0; j < (int)sbmlRxn.getNumReactants(); j++){
				...
				if ( ((int)(spRef.getStoichiometry()) != spRef.getStoichiometry()) || spRef.isSetStoichiometryMath()) {
					logger.sendMessage(VCLogger.HIGH_PRIORITY, TranslationMessage.REACTION_ERROR, "Non-integer stoichiometry or stoichiometryMath not handled in VCell at this time.");
				}
	...
	try {
		for (int i = 0; i < sbmlModel.getNumReactions(); i++) {
				...
				if (embeddedRxnElement != null) {
					if (embeddedRxnElement.getName().equals(XMLTags.FluxStepTag)) {
						...
						if (fluxOptionStr.equals(XMLTags.FluxOptionMolecularOnly)) {
							...
							logger.sendMessage(VCLogger.MEDIUM_PRIORITY, TranslationMessage.REACTION_ERROR, "Unknown FluxOption : " + fluxOptionStr + " for SBML reaction : " + rxnName);
						}
			...
			if ((bLocalParamMatchesCompId && kLawRateExpr.hasSymbol(COMPARTMENTSIZE_SYMBOL)) || (!bLocalParamMatchesCompId && !kLawRateExpr.hasSymbol(COMPARTMENTSIZE_SYMBOL))) {
				...
				logger.sendMessage(VCLogger.HIGH_PRIORITY, TranslationMessage.REACTION_ERROR, "Cannot have a local parameter which does not occur in kinetic law expression for SBML reaction : " + rxnName);
			}
			...
			if (kinetics instanceof GeneralKinetics) {
				...
				if (vcRateExpression.hasSymbol(COMPARTMENTSIZE_SYMBOL)) {
					...
					logger.sendMessage(VCLogger.HIGH_PRIORITY, TranslationMessage.REACTION_ERROR, "Reaction " + rxnName + " cannot have GeneralKinetics since kinetic law expression is not in terms of concentration/time");
				}
					...
					if (vcRateExpression.hasSymbol(comp1.getId()) && !bCompFoundInLocalParams) {
						if (!comp1.getId().equals(compartment.getId())) {
							if (comp1.isSetSize()) {
								...
								logger.sendMessage(VCLogger.HIGH_PRIORITY, TranslationMessage.REACTION_ERROR, "Reaction " + rxnName + " has a compartment whose size is not set");
							}
							...
							logger.sendMessage(VCLogger.HIGH_PRIORITY, TranslationMessage.REACTION_ERROR, "Reaction " + rxnName + " rate expression contains the compartment in which the reaction takes place; the kinetics should not be Lumped kinetics");
						}
					}
				...
				if (kinetics.getKineticsParameter(paramName) != null) {
					...
					while (refSpIterator.hasNext()) {
						...
						if (vcUnit.isCompatible(sbUnit)) {
							...
							logger.sendMessage(VCLogger.HIGH_PRIORITY, TranslationMessage.UNIT_ERROR, "Unable to scale the unit for species concentration: " + vcUnit.getSymbol() + " " + sbUnit.getSymbol());
						}
		...
		try {
			...
			for (int i = 0; i < sbmlModel.getNumSpecies(); i++) {
				...
				if (dimension == 0 || dimension == 1){
					logger.sendMessage(VCLogger.HIGH_PRIORITY, TranslationMessage.UNIT_ERROR, dimension+" dimensional compartment "+compartment.getId()+" not supported");
				}
				String spatialDimBuiltinName = getSpatialDimensionBuiltInName(dimension);
				...
				if (sbmlSpecies.isSetInitialConcentration()) { 		// If initial Concentration is set
					...
					if (compartment.isSetSize()) {
						...
						if (compartmentSize != 0.0) {
							...
							logger.sendMessage(VCLogger.HIGH_PRIORITY, TranslationMessage.UNIT_ERROR, "compartment "+compartment.getId()+" has zero size, unable to determine initial concentration for species "+speciesName);
						}
						initExpr = getValueFromRule(speciesName);
						if (initExpr == null) {
							initExpr = new Expression(initConcentration);
						...
						logger.sendMessage(VCLogger.HIGH_PRIORITY, TranslationMessage.SPECIES_ERROR, " Compartment " + compartment.getId() + " size not set or is defined by a rule; cannot calculate initConc.");
					}
				} else {
					initExpr = getValueFromRule(speciesName);
					if (initExpr == null) {
...
private Structure getReactionStructure(org.sbml.libsbml.Reaction sbmlRxn, SpeciesContext[] speciesContexts) throws Exception {
    ...
    if (compartmentNamesHash.size() == 1) {
    	...
    	if (compartmentNamesHash.size() > 3) {
    		logger.sendMessage(VCLogger.HIGH_PRIORITY, TranslationMessage.COMPARTMENT_ERROR, "Cannot resolve location of reaction : " + sbmlRxn.getId());
    	}
