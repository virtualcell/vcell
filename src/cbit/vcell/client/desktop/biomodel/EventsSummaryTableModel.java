		...
		try{
			...
			switch (column) {
				case COLUMN_EVENT_NAME: {
					...
					if (simulationContext != null && simulationContext.getEntry(name) != null) {
						throw new RuntimeException("Cannot use '"+name+"' as a bioevent name.\nName '" + name + "' is already being used by another model entity.");
					}
					bioEvent.setName(name);
				} 
			}
		} catch(Exception e){
			e.printStackTrace(System.out);
