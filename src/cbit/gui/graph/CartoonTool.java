...
public abstract class CartoonTool implements MouseListener,
	...
	public static final String ADD_ENZYME_REACTION_MENU_ACTION = "Search Reactions...";
	...
	public static final String[] ACTION_COMMANDS = { "select", "feature",
			"species", "line", "step", "flux", "spline", "addCP", "complex",
			"bindingSite", "interaction" };
	...
	// Visibility modes
	JMenuItem hideSelectedJMenuItem = new JMenuItem(
			new HideSelectedShapesAction(this));
	JMenuItem unhideAllJMenuItem = new JMenuItem(
			new UnhideAllShapesAction(this));
	JMenuItem showShapeTreeJMenuItem = new JMenuItem(new ShowShapeTreeAction(
			this));
	JMenuItem collapseAsNewGroupJMenuItem = new JMenuItem(
			new CollapseAsNewGroupAction(this));
	JMenuItem collapseExistingGroupsJMenuItem = new JMenuItem(
			new CollapseExistingGroupsAction(this));
	JMenuItem expandSelectedJMenuItem = new JMenuItem(
			new ExpandSelectedGroupsAction(this));
	JMenuItem disbandGroupsJMenuItem = new JMenuItem(new DisbandGroupsAction(
			this));

	// Add new JMenuItems here too. Used for convenience to add and remove
	// actionlisteners
	private JMenuItem[] jmenuItemArr = { addFeatureJMenuItem,
			addSpeciesJMenuItem, addGlobalParamJMenuItem,
			addEnzymeReactionJMenuItem, reactionsJMenuItem,
			propertiesJMenuItem, enableJMenuItem, disableJMenuItem,
			solveJMenuItem, resetJMenuItem, showParametersJMenuItem,
			copyJMenuItem, deleteJMenuItem, cutJMenuItem, pasteJMenuItem,
			pastenewJMenuItem, moveJMenuItem, addBindingSiteJMenuItem,
			addComplexJMenuItem, saveAsImageJMenu, annotateJMenuItem,
			hideSelectedJMenuItem, unhideAllJMenuItem, showShapeTreeJMenuItem,
			collapseAsNewGroupJMenuItem, collapseExistingGroupsJMenuItem,
			expandSelectedJMenuItem, disbandGroupsJMenuItem };
	...
	public CartoonTool() {
		...
		addEnzymeReactionJMenuItem
				.setActionCommand(ADD_ENZYME_REACTION_MENU_ACTION);
		...
		copyJMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_C,
		...
		pasteJMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_V,
				Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		...
		for (int i = 0; i < jmenuItemArr.length; i += 1) {
			if (jmenuItemArr[i] instanceof JMenu) { // special handling for the
				// image-submenu
	...
	protected final void popupMenu(Shape shape, int x, int y) throws Exception {
		...
		if (shape != null) {
			...
			for (int i = 0; i < jmenuItemArr.length; i += 1) {
				if (jmenuItemArr[i] instanceof JMenu) { // accomodate the
					// image-submenu
					...
					for (int k = 0; k < ((JMenu) jmenuItemArr[i]).getItemCount(); k++) {
						...
						if (shapeHasMenuAction(shape, menuItem.getActionCommand())) {
							boolean actionEnabled = shapeHasMenuActionEnabled(
									shape, menuItem.getActionCommand());
				...
				if (shapeHasMenuAction(shape, jmenuItemArr[i].getActionCommand())) {
					currentMenuList.add(jmenuItemArr[i]);
					jmenuItemArr[i].setEnabled(shapeHasMenuActionEnabled(shape,
							jmenuItemArr[i].getActionCommand()));
				}
			}
			...
			if (!bSame) {// Make new JPopupMenu if its different than the last
				...
				for (int i = 0; i < currentMenuList.size(); i += 1) {
					...
					if (addableJMenuItem == annotateJMenuItem
						...
						popupMenu.add(addableJMenuItem);
					} else if (addableJMenuItem == copyJMenuItem
							...
							|| addableJMenuItem == collapseAsNewGroupJMenuItem
							|| addableJMenuItem == collapseExistingGroupsJMenuItem
							|| addableJMenuItem == expandSelectedJMenuItem
							|| addableJMenuItem == disbandGroupsJMenuItem) {
	...
	protected abstract boolean shapeHasMenuActionEnabled(Shape shape,
			String menuAction);
