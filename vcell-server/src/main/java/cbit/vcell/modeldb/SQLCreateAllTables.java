		...
		if (ok == JOptionPane.OK_OPTION && (c1.isSelected() || c2.isSelected())) {
			...
			try {
				...
				if (c2.isSelected()) {
					...
					createIndex(con, "export_job_time", ExportHistoryTable.table, ExportHistoryTable.table.exportDate, dbSyntax);
					createIndex(con, "export_job_userref", ExportHistoryTable.table, ExportHistoryTable.table.userRef, dbSyntax);
				}
				con.commit();
