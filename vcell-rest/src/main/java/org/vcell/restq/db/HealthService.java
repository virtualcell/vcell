...
public class HealthService {
	...
	@Inject
	public HealthService(OracleAgroalConnectionFactory oracleAgroalConnectionFactory) throws DataAccessException {
		this.host = "localhost";
		this.port = 8083; // needs to change dynamically
		this.bIgnoreCertProblems = PropertyLoader.getBooleanProperty(PropertyLoader.sslIgnoreCertProblems, false);;
		this.bIgnoreHostMismatch = true; // HealthService connects via localhost, this will never match host in production cert;
		LocalAdminDbServer localAdminDbServer = new LocalAdminDbServer(oracleAgroalConnectionFactory, oracleAgroalConnectionFactory.getKeyFactory());
		...
		this.testUserid = testUserInfo.userid;
		this.testPassword = testUserInfo.digestedPassword0;
	}
