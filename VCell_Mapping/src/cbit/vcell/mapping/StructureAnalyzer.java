			...
			if (stoichiometry!=0.0){
				...
				for (int k = 0; k < rps.length; k++){
					...
					if ((structure instanceof Membrane) && (rps[k].getStructure()!=structure)){
						...
						exp = ExpressionFactory.add(exp,ExpressionFactory.createExpression(mathMapping.getNameScope().getSymbolName(fluxCorrectionParameter)+"* ("+fastReactionSteps[j].getRateExpression(rps[k]).infix(mathMapping.getNameScope())+")"));
					}else{
						exp = ExpressionFactory.add(exp,ExpressionFactory.createExpression(fastReactionSteps[j].getRateExpression(rps[k]).infix(mathMapping.getNameScope())));
					}
				}
			}
		}
					...
					for (int k = 0; k < rps.length; k++){
						if ((structure instanceof Membrane) && (sc.getStructure()!=structure)){
							...
							if (reactionSteps[j] instanceof FluxReaction){
								exp = ExpressionFactory.add(exp, ExpressionFactory.createExpression(mathMapping.getNameScope().getSymbolName(fluxCorrectionParameter)+"*"+reactionSteps[j].getRateExpression(rps[k]).infix(mathMapping.getNameScope())));
							}else if (reactionSteps[j] instanceof SimpleReaction){
								exp = ExpressionFactory.add(exp, ExpressionFactory.createExpression(mathMapping.getNameScope().getSymbolName(fluxCorrectionParameter)+"*"+ReservedSymbol.KMOLE.getName()+"*"+reactionSteps[j].getRateExpression(rps[k]).infix(mathMapping.getNameScope())));
							}else{
							...
							exp = ExpressionFactory.add(exp, ExpressionFactory.createExpression(reactionSteps[j].getRateExpression(rps[k]).infix(mathMapping.getNameScope())));
						}
					}
				}
			}
