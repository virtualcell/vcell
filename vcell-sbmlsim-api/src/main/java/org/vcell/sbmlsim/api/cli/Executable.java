 ...
 */
protected void executeProcess(int[] expectedReturnCodes) throws ExecutableException {
...
//			ResourceUtil.setEnvForOperatingSystem(pb.environment());
		...
		if (getStatus().equals(ExecutableStatus.STOPPED)) {
...
public String getCommand() {
	...
	for (int i = 0; i < command.length; i ++) {
		...
		commandLine.append(getEscapedPathName(command[i]));		
	}
	return commandLine.toString();
}

private static String getEscapedPathName(String pathname) {
	/**
 ...
 */
private synchronized void setStatus(ExecutableStatus newStatus) {
	status = newStatus;
}

public final void start() throws ExecutableException {
	start(new int[] { 0 });
}
 ...
 * This method was created in VisualAge.
 */
public final void start(int[] expectedReturnCodes) throws ExecutableException {
