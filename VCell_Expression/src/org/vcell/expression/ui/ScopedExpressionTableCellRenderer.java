...
public java.awt.Component getTableCellRendererComponent(javax.swing.JTable theTable, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
	...
	try{
		...
		String scopedExpressInfix = ((ScopedExpression)value).toString();
		if(scopedExpressionImageIconHash.containsKey(scopedExpressInfix) && 
			...
			try{
				ExpressionPrintFormatter epf =
					new ExpressionPrintFormatter(
						((ScopedExpression)value).getExpression(),
						((ScopedExpression)value).getNameScope());
				//
				...
				//Fallback to String
				e.printStackTrace();
				templateJLabel.setText(((ScopedExpression)value).toString());
				return templateJLabel;
			}
