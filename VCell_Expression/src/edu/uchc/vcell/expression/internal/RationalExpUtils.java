...
public class RationalExpUtils {
	...
	private static IRationalExpression getRationalExp(SimpleNode node) throws ExpressionException {
		...
		if (node instanceof ASTAndNode || 
			...
			if (((ASTFuncNode)node).getFunction()==Operator.POW){
				try {
					...
					if (intExponent == 0){
						return RationalExpressionFactory.createRationalExpression(1);
					}
					
					IRationalExpression base = getRationalExp((SimpleNode)node.jjtGetChild(0));
					if (intExponent < 0){
						base = base.inverse();
			...
			IRationalExpression exp = RationalExpressionFactory.createRationalExpression(0);
			...
			IRationalExpression exp = RationalExpressionFactory.createRationalExpression(1);
			for (int i = 0; i < node.jjtGetNumChildren(); i++) {             
			...
			return RationalExpressionFactory.createRationalExpression(r.getNum(), r.getDen());
		//} else if (node instanceof ASTFuncNode) {   
		...
		//} else if (node instanceof ASTPowerNode) {
			...
			return RationalExpressionFactory.createRationalExpression(((ASTIdNode)node).getName());
		} else {
