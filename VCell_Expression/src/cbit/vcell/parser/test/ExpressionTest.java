...
public static void main(java.lang.String[] args) {
	try {
		...
		for (int i = 0; i < num; i ++){
			IExpression exp = edu.uchc.vcell.expression.internal.ExpressionUtils.generateExpression(r, 4, false);
...
public static void testDifferentiate(int numTrials, int depth, long seed) {
	...
	for (int i = 0; i < numTrials; i++) {
		try {
			IExpression exp = edu.uchc.vcell.expression.internal.ExpressionUtils.generateExpression(random, depth, false);
				...
				if (symbols!=null && symbols.length>0){
					for (int j = 0;symbols!=null && j < symbols.length; j++){
						...
						try {
							...
							boolean isSame = edu.uchc.vcell.expression.internal.ExpressionUtils.derivativeFunctionallyEquivalent(exp,symbols[j],diff,relativeTolerance,absoluteTolerance);
							if (isSame) {
								numCorrect++;
							} else {
								numWrong++;
...
public static void testEvaluateInterval(int numTrials, int depth, long seed, boolean bIsConstraint) {
    ...
    for (int i = 0; i < numTrials; i++) {
		try {
			IExpression exp = edu.uchc.vcell.expression.internal.ExpressionUtils.generateExpression(random, depth, bIsConstraint);
