 ...
 */
public static SimulationComparisonSummary comparePDEResults(Simulation testSim, DataManager testDataManager, Simulation refSim, DataManager refDataManager, String varsToCompare[]) throws org.vcell.util.DataAccessException, org.vcell.expression.ExpressionException {
			...
			if (testMesh.getSizeX() != refMesh.getSizeX() || testMesh.getSizeY() != refMesh.getSizeY() || testMesh.getSizeZ() != refMesh.getSizeZ()) {
				if (testVar instanceof VolVariable){
					if (testMathDesc.getGeometry().getDimension() == 1 && refMathDesc.getGeometry().getDimension() == 1) {
						spaceResampledData = DataResampler.resample1DSpatial(resampledTestData, testMesh, refMesh);
					} else if (testMathDesc.getGeometry().getDimension() == 2 && refMathDesc.getGeometry().getDimension() == 2) {
						spaceResampledData = DataResampler.resample2DSpatial(resampledTestData, testMesh, refMesh);
					} else if (testMathDesc.getGeometry().getDimension() == 3 && refMathDesc.getGeometry().getDimension() == 3) {
						spaceResampledData = DataResampler.resample3DSpatial(resampledTestData, testMesh, refMesh);
					} else {
 ...
 * Creation date: (8/20/2003 12:58:10 PM)
 */
public static SimulationComparisonSummary comparePDEResultsWithExact(Simulation sim, DataManager dataManager) throws org.vcell.util.DataAccessException, org.vcell.expression.ExpressionException {
...
public static IExpression[] getInsideOutsideFunctions(IExpression analyticSubDomainExp) throws ExpressionException {
	...
	return (IExpression[])org.vcell.util.BeanUtils.getArray(expList,IExpression.class);
}
