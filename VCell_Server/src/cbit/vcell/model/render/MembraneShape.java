...
public Dimension getPreferedSize(java.awt.Graphics2D g) {
	...
	preferedSize.width = featureDim.width + memSpacingX*2;
	preferedSize.height = featureDim.height + memSpacingY*2;
	return preferedSize;
}
...
private Point getRadialPosition(double angleRad) {
	...
	//
	//
	int a = screenSize.width/2 - memSpacingX/2;
	int b = screenSize.height/2 - memSpacingY/2;
...
public void layout() {
	...
	//
	Dimension featureDim = featureShape.screenSize;
	...
	featureShape.screenPos.x = centerX - featureShape.screenSize.width/2;
	featureShape.screenPos.y = currentY;
	currentY += featureShape.screenSize.height + memSpacingY;
	...
	if (numSpeciesContexts>0){
		...
		for (int i=0;i<countChildren();i++){
			...
			if (shape instanceof SpeciesContextShape){
				shape.screenPos = getRadialPosition(currentAngle);
...
public void resize(java.awt.Graphics2D g, Dimension newSize) throws Exception {
	...
	//
	FeatureShape featureShape = (FeatureShape)childShapeList.elementAt(0);
	Dimension featureSize = new Dimension(featureShape.screenSize);
