 ...
 * Creation date: (6/28/2005 9:42:55 AM)
 * @return java.awt.Rectangle
 * @param numThingsInBox int
 */
...
/**
 * Insert the method's description here.
 * Creation date: (6/20/2005 2:36:13 PM)
 */
public void paint ( java.awt.Graphics2D g, int parentOffsetX, int parentOffsetY ) {
	...
	//if(currentOval != null){
		...
		//g.translate(parentOffsetX,parentOffsetY);
		//g.draw(currentOval);
		//g.setTransform(at);
	//}


	//int boxX = (int)currentOval.getBounds().getX();
	//int boxY = (int)currentOval.getBounds().getY();
	//for (int i=0;i<childShapeList.size();i++){
		//Shape child = (Shape)childShapeList.elementAt(i);
		//if (child instanceof SpeciesContextShape){
			//while(!currentOval.contains(boxX,boxY,SpeciesContextShape.DIAMETER,SpeciesContextShape.DIAMETER)){
				//g.setColor(Color.black);
				//g.drawString("X",boxX+parentOffsetX,boxY+parentOffsetY);
				//if(boxX < screenSize.width){
					//boxX+= child.screenSize.width + defaultSpacingX;
				//}else{
					//boxX = (int)currentOval.getBounds().getX();
					//boxY+= child.screenSize.height + defaultSpacingY;
				//}
				//if(boxX > screenSize.width && boxY > screenSize.height){
					//break;//Need to do something here
				//}
			//}
			//g.setColor(Color.green);
			//g.drawString("X",boxX+parentOffsetX,boxY+parentOffsetY);
