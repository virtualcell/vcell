...
public class FRAPBatchRunWorkspace extends FRAPWorkspace
{
	...
	public static class LoadBatchRun {
		/**
		 * generated task if valid, null if not
		 */
		public final AsynchClientTask task;
		/**
		 *  is return valid
		 */
		 ...
		 * @param task
		 */
		public LoadBatchRun(AsynchClientTask task) {
			valid = true;
			this.task = task;
			message = null;
		}
		 ...
		 * @param message
		 */
		public LoadBatchRun(String message) {
			valid = false;
	...
	public LoadBatchRun getLoadBatchRunFileTask(File inputFile)
	{
		...
		if(inFile.getName().endsWith("."+VirtualFrapLoader.VFRAP_BATCH_EXTENSION) || inFile.getName().endsWith(".xml"))  { //.vfbatch
			...
			{ 
				public void run(Hashtable<String, Object> hashTable) throws Exception
				{
					...
					assert(inFile.getName().endsWith("."+VirtualFrapLoader.VFRAP_BATCH_EXTENSION) || inFile.getName().endsWith(".xml")); //.vfbatch
			...
			return new LoadBatchRun(loadTask);
		}
		return new LoadBatchRun("Invalid file name " + inFile.getName() + "; file must end with extension " + VirtualFrapLoader.VFRAP_BATCH_EXTENSION + " or xml");
	}
