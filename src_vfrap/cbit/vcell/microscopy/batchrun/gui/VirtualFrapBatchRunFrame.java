...
import cbit.vcell.microscopy.batchrun.FRAPBatchRunWorkspace.LoadBatchRun;
				...
				else if(arg.equals(OPEN_ACTION_COMMAND))
			    {
					File inputFile = null;
		  			...
		  			if (option == JFileChooser.APPROVE_OPTION){
		  	  		...
		  	  		OpenBatchRun obr = open(inputFile);
		  	  		if (obr.valid) {
			    		ClientTaskDispatcher.dispatch(VirtualFrapBatchRunFrame.this, new Hashtable<String, Object>(), obr.tasks, true);
		  	  		}
		  	  		else {
		  	  			DialogUtils.showErrorDialog(VirtualFrapBatchRunFrame.this, obr.message);
		  	  		}
			    }
...
//										}
	...
	public static class OpenBatchRun {
		/**
		 * generated tasks if valid, null if not
		 */
		public final AsynchClientTask[] tasks;
		/**
		 *  is return valid
		 */
		...
		/**
		 * constructor if valid
		 * @param task
		 */
		public OpenBatchRun(AsynchClientTask[] tasks) {
			valid = true;
			this.tasks = tasks;
			message = null;
		}

		public OpenBatchRun(String message) {
			valid = false;
	 ...
	 public OpenBatchRun open(final File inFile) 
	 {
		 ...
		 LoadBatchRun lbr = batchRunWorkspace.getLoadBatchRunFileTask(inFile);
		 if (lbr.valid) {
			 totalTasks.add(lbr.task);
			 ...
			 return new OpenBatchRun(totalTasks.toArray(new AsynchClientTask[totalTasks.size()]));
		 }
		 return new OpenBatchRun(lbr.message);
	 }
						...
						for(Object dataElement:tData)
						{
							if(dataElement instanceof File)
							{
								...
								if (obr.valid) {
									...
									DialogUtils.showErrorDialog(this, obr.message);
									break;
								}
