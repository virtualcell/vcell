...
public Shape[] getAllSelectedShapes() {
	Vector selectedShapeList = new Vector();
	for (int i=0;i<shapeList.size();i++){
		Shape fs = (Shape)shapeList.elementAt(i);
		if (fs.isSelected()){
...
ElipseShape getNodeFromIndex(int index) {
	...
	Shape shape = (Shape)shapeList.elementAt(index);
...
public Shape[] getSelectedShapes() {
	Vector theSelectedShapesV = new Vector();
	for (int i=0;i<shapeList.size();i++){
		Shape fs = (Shape)shapeList.elementAt(i);
		if (fs.isSelected()){
...
public Shape[] pickWorld(java.awt.Rectangle argRectWorld) {
	Shape topShape = getTopShape();
	if (topShape==null) return null;
	Vector pickedList = new Vector();
	for (int i = 0; i < shapeList.size(); i++){
		Shape shape = (Shape)shapeList.elementAt(i);
...
public void showShapeHierarchyBottomUp() {
	...
	Vector shapes = new Vector(shapeList);
	...
	for (int i=0;i<shapes.size();i++){
		if (((Shape)shapes.elementAt(i)).getParent() == null){
	...
	for (int j=0;j<topList.size();j++){
		Shape top = (Shape)topList.elementAt(j);
	...
	if (shapes.size()>0){
		...
		for (int i=0;i<shapes.size();i++){
			System.out.println(((Shape)shapes.elementAt(i)).toString());
		}
	}
...
public void showShapeHierarchyTopDown() {
	...
	for (int j=0;j<topList.size();j++){
		...
		while (stack.size()>0){
			...
			Enumeration enum_children = currShape.getChildren();
			while (enum_children.hasMoreElements()){
				Shape shape = (Shape)enum_children.nextElement();
