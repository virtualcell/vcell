...
public static BNGOutputSpec createBngOutputSpec(File bngOutputFile) throws FileNotFoundException, IOException {
	...
	while (lineTokenizer.hasMoreTokens()) {
		...
		if (list == SPECIES_LIST) {
			...
			IExpression speciesConcExpr = null;
			...
			while (nextLine.hasMoreTokens()) {
				...
				if (i == 0) {
					...
					try {
						speciesConcExpr = ExpressionFactory.createExpression(token2);
		...
		if (list == RXN_LIST) {
			...
			IExpression paramExpression = null;
			...
			while (nextLine.hasMoreTokens()) {
				...
				if (i == 0) {
					...
					//
					try {
						paramExpression = ExpressionFactory.createExpression(token2);
					} catch (ExpressionException e) {
