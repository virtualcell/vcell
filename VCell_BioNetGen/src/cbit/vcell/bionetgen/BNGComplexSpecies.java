 ...
 */
public BNGComplexSpecies(String argName, IExpression argConc, int argNtwkFileIndx) {
...
public BNGSpecies[] parseBNGSpeciesName() {
	...
	while (complexSpeciesNameTokenizer.hasMoreTokens()) {
		...
		if (token1.indexOf("(") > 0) {
			BNGMultiStateSpecies msSpecies = new BNGMultiStateSpecies(token1, ExpressionFactory.createExpression(0.0), -1);
			complexSpeciesComponentsVector.addElement(msSpecies);
		} else {
			BNGSingleStateSpecies ssSpecies = new BNGSingleStateSpecies(token1, ExpressionFactory.createExpression(0.0), -1);
