...
//		}
	...
	public FRAPStudy getFrapStudy(Element param, ClientTaskStatusSupport progressListener) throws XmlParseException{
		...
		{
			frapStudy.getData().setModels(getFrapModels(frapModelElement));
		}
		...
		Element timeSeriesExternalDataElement = param.getChild(MicroscopyXMLTags.ImageDatasetExternalDataInfoTag);
		if (timeSeriesExternalDataElement!=null){
			frapStudy.getData().setFrapDataExternalDataInfo(getExternalDataInfo(timeSeriesExternalDataElement));
		}
		Element roiExternalDataElement = param.getChild(MicroscopyXMLTags.ROIExternalDataInfoTag);
		if (roiExternalDataElement!=null){
			frapStudy.getData().setRoiExternalDataInfo(getExternalDataInfo(roiExternalDataElement));
		}
	...
	{
		FRAPModel[] frapModels = new FRAPModel[ModelType.values().length];
		...
		if(diffOneElement != null)
		{
			...
			if(paramElement != null){
				parameters = getModelParameters(paramElement, ModelType.DiffOne.index);
			}
			frapModels[ModelType.DiffOne.index] = new FRAPModel(ModelType.fromDescription(id), parameters, null, null);
		}
		...
		if(diffTwoElement != null)
		{
			...
			if(paramElement != null)
			{
				parameters = getModelParameters(paramElement, ModelType.DiffTwoWithPenalty.index);
			}
			frapModels[ModelType.DiffTwoWithPenalty.index] = new FRAPModel(ModelType.fromDescription(id), parameters, null, null);
		}
		...
		if(reacOffRateElement != null)
		{
			...
			if(paramElement != null)
			{
				parameters = getModelParameters(paramElement, ModelType.ReactDominant.index);
			}
			frapModels[ModelType.ReactDominant.index] = new FRAPModel(ModelType.fromDescription(id), parameters, null, null);
		}
...
//		}
	...
	private Parameter[] getModelParameters(Element paramElement, int modelType) {
		if(modelType == ModelType.DiffOne.index)
		{
		...
		else if(modelType ==ModelType.DiffTwoWithPenalty.index)
		{
		...
		else if(modelType ==ModelType.DiffAndBinding.index)
		{
