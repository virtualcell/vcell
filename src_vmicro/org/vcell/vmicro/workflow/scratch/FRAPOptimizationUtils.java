...
public class FRAPOptimizationUtils {
	...
	public static double FTOL = 1.0e-6;
...
//						double newValue = (mFracFast * fastData[i][j] + mFracSlow * slowData[i][j] + immobileFrac * firstPostBleach[i]) * Math.exp(-(monitoringRate * expTimePoints[j]));
	...
	public static double estimate(FRAPStudy frapStudy, LocalWorkspace localWorkspace, FRAPOptData argOptData, Parameter[] inParams, String[] outParaNames, double[] outParaVals, boolean[] eoi) throws Exception
	{
		...
		DefaultScalarFunction scalarFunc = new LookupTableObjectiveFunction_old(argOptData, eoi, frapStudy,localWorkspace); // add opt function 
...
//				System.arraycopy(allTimesData, startingIndex, truncatedTimesData, 0, truncatedTimes.length);
	 ...
	 * Here we want to generate a double array which includes all roi data (bleached, bg, cell, ring1 to ring8)
	 */
	public static double[][] extendSimToFullROIData_new(FRAPData fData, double[][] rawData, int numTimePoints)
	{
		double[][] results = null;
	...
	public static double[][] refreshNormalizedMeasurementError(FRAPStudy frapStudy) throws Exception
	{
		...
		double[] prebleachAvg = fData.calculatePreBleachAverageXYZ(startIndexRecovery);
...
//			double logLowerBound = (lowerBound == 0)? 0: Math.log10(lowerBound);
	...
	{
		ProfileSummaryData[][] summaryData = new ProfileSummaryData[ModelType.values().length][FRAPModel.NUM_MODEL_PARAMETERS_BINDING];
		...
		if(frapStudy.getProfileData_oneDiffComponent() !=null)
		{
			...
			{
				summaryData[ModelType.DiffOne.index][i] = getSummaryFromProfileData(profileData[i]);
			}
		}
		if(frapStudy.getProfileData_twoDiffComponents() !=null)
		{
			...
			for(int i=0; i<profileData.length; i++)
			{
				summaryData[ModelType.DiffTwoWithPenalty.index][i] = getSummaryFromProfileData(profileData[i]);
			}
		}
		if(frapStudy.getProfileData_reactionOffRate() != null)
		{
			...
			for(int i=0; i<profileData.length; i++)
			{
				...
				{
					summaryData[ModelType.ReactDominant.index][FRAPModel.INDEX_BLEACH_MONITOR_RATE] = getSummaryFromProfileData(profileData[i]);
				}
				...
				{
					summaryData[ModelType.ReactDominant.index][FRAPModel.INDEX_OFF_RATE] = getSummaryFromProfileData(profileData[i]);
				}
			}
		}
		return summaryData;
