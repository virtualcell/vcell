...
//		frapData.setOriginalGlobalScaleInfo(null);
	...
	public static org.vcell.vmicro.workflow.scratch.FRAPData importFRAPDataFromImageDataSet_new(ImageDataset imageDataSet){
		org.vcell.vmicro.workflow.scratch.FRAPData frapData =
			new org.vcell.vmicro.workflow.scratch.FRAPData(imageDataSet,
				new String[]{ FRAPData.VFRAP_ROI_ENUM.ROI_BLEACHED.name(),FRAPData.VFRAP_ROI_ENUM.ROI_CELL.name(),FRAPData.VFRAP_ROI_ENUM.ROI_BACKGROUND.name()}
			);
//		frapData.setOriginalGlobalScaleInfo(null);
		return frapData;
	...
	public static org.vcell.vmicro.workflow.scratch.FRAPData importFRAPDataFromVCellSimulationData_new(File vcellSimLogFile,String variableName, String bleachedMaskVarName, Double maxIntensity, boolean bNoise,
...
//		double SCALE_MAX = maxIntensity.doubleValue();/*Math.pow(2,16)-1;*///Scale to 16 bits
		...
		org.vcell.vmicro.workflow.scratch.FRAPData frapData = new org.vcell.vmicro.workflow.scratch.FRAPData(imageDataSet, new String[]{ FRAPData.VFRAP_ROI_ENUM.ROI_BLEACHED.name(),FRAPData.VFRAP_ROI_ENUM.ROI_CELL.name(),FRAPData.VFRAP_ROI_ENUM.ROI_BACKGROUND.name()});
	...
	public static org.vcell.vmicro.workflow.scratch.FRAPData importFRAPDataFromHDF5Data_new(File inputHDF5File, Double maxIntensity, ClientTaskStatusSupport progressListener) throws Exception{
		if(progressListener != null){
			progressListener.setMessage("Loading HDF5 file " + inputHDF5File.getAbsolutePath() + "...");
		}		
		DataOperationResults.DataProcessingOutputInfo dataProcessingOutputInfo =
		...
		ArrayList<SourceDataInfo> sdiArr =
			dataProcessingOutputDataValues.createSourceDataInfos(
				...
				dataProcessingOutputInfo.getVariableOrigin(SimulationContext.FLUOR_DATA_NAME),
				dataProcessingOutputInfo.getVariableExtent(SimulationContext.FLUOR_DATA_NAME));
		return createFrapData_new(sdiArr, dataProcessingOutputInfo.getVariableTimePoints(), 0, maxIntensity, progressListener);
	}
	...
	public static org.vcell.vmicro.workflow.scratch.FRAPData createFrapData_new(ArrayList<SourceDataInfo> sourceDataInfoArr,double[] times,int slice,Double maxIntensity,ClientTaskStatusSupport progressListener) throws ImageException{
		if(sourceDataInfoArr.size() != times.length){
			throw new ImageException("Error FRAPData.createFrapData: times array length must equal SourceDataInfo vector size");
		}
		// construct
