...
import org.vcell.vmicro.workflow.data.FRAPOptData_Data;
...
public class FRAPOptData {
	...
	public static final double REF_STARTINGTIME = 0;
	public static final double MAX_DIFF_RATE_FOR_TIMEBOUNDS = 50;
	...
	private final FRAPOptData_Data data = new FRAPOptData_Data(0, 0, false, null, null,
			null, null);

	public FRAPOptData(org.vcell.vmicro.workflow.scratch.FRAPStudy argExpFrapStudy, int numberOfEstimatedParams, LocalWorkspace argLocalWorkSpace,
			ClientTaskStatusSupport progressListener) throws Exception
	{
		...
		data.dimensionReducedRefData = getDimensionReducedRefData(argExpFrapStudy, argLocalWorkSpace, progressListener, null);
	}
	
	public FRAPOptData(org.vcell.vmicro.workflow.scratch.FRAPStudy argExpFrapStudy, int numberOfEstimatedParams, LocalWorkspace argLocalWorkSpace, SimpleReferenceData simRefData) throws Exception
	{
		...
		setNumEstimatedParams(numberOfEstimatedParams);
		data.dimensionReducedRefData = getDimensionReducedRefData(argExpFrapStudy, argLocalWorkSpace, null, simRefData); 
	}
...
//		return new UniformOutputTimeSpec(timeStamps[startIdx+1]-timeStamps[startIdx]);
	...
	public static TimeBounds getEstimatedRefTimeBound(org.vcell.vmicro.workflow.scratch.FRAPStudy fStudy)
	{
	...
	public double[] getRefDataTimePoints() {
		return data.refDataTimePoints;
	}

	public double[][] getDimensionReducedRefData(org.vcell.vmicro.workflow.scratch.FRAPStudy expFrapStudy, LocalWorkspace localWorkspace, ClientTaskStatusSupport progressListener, SimpleReferenceData srData) throws Exception
	{
		if(data.dimensionReducedRefData == null)
		{
			if(srData != null)
			{
				getFromStoredRefData(expFrapStudy,srData);
			}
			else
			{
				refreshDimensionReducedRefData(expFrapStudy, localWorkspace, progressListener);
			}
		}
		return data.dimensionReducedRefData;
	}
	
	public static double[][] getDimensionReducedExpData(org.vcell.vmicro.workflow.scratch.FRAPStudy expFrapStudy) throws Exception
	{
		return expFrapStudy.getDimensionReducedExpData();
	}
	
	public static double[] getReducedExpTimePoints(org.vcell.vmicro.workflow.scratch.FRAPStudy expFrapStudy) {
		
		return expFrapStudy.getReducedExpTimePoints();
	}
	
	public static void refreshDimensionReducedRefData(org.vcell.vmicro.workflow.scratch.FRAPStudy expFrapStudy, LocalWorkspace localWorkspace, ClientTaskStatusSupport progressListener) throws Exception
	{
		
		System.out.println("run simulation...");
		KeyValue referenceSimKeyValue = null;
		referenceSimKeyValue = runRefSimulation(expFrapStudy,localWorkspace,progressListener);
...
//		DataProcessingOutput dataProcessingOutput = getRawReferenceDataFromHDF5(hdf5File);
		...
		double[] refDataTimePoints = shiftTimeForBaseDiffRate(rawRefDataTimePoints);
		...
		double[][] dimensionReducedRefData = FRAPOptimizationUtils.extendSimToFullROIData(expFrapStudy.getFrapData(), rawData, refDataTimePoints.length);
		...
		expFrapStudy.setStoredRefData(FRAPOptimizationUtils.doubleArrayToSimpleRefData(dimensionReducedRefData, refDataTimePoints, 0, selectedROIs));

		//remove reference simulation files
		FRAPStudy.removeSimulationFiles(referenceSimKeyValue, localWorkspace); 
		//remove experimental and roi external files
		FRAPStudy.removeExternalFiles(expFrapStudy.getData().getFrapDataExternalDataInfo().getExternalDataIdentifier(), 
				                      expFrapStudy.getData().getRoiExternalDataInfo().getExternalDataIdentifier(), localWorkspace);
	}
	
//	public static double[][] getDataFromSourceDataInfo(DataProcessingOutput dataProcessingOutput)
//	{
//		double[][] results = null;
...
//	}
//	
//	public static DataProcessingOutput getRawReferenceDataFromHDF5(File hdf5File) throws DataAccessException {
//		try {
...
//	}
	...
	private static double[] shiftTimeForBaseDiffRate(double[] timePoints)
	{ 
	...
	public static KeyValue runRefSimulation(org.vcell.vmicro.workflow.scratch.FRAPStudy expFrapStudy, LocalWorkspace localWorkspace, ClientTaskStatusSupport progressListener) throws Exception
	{
		...
		try{
			FieldDataIdentifierSpec psfFieldFunc = FRAPStudy.getPSFFieldData(localWorkspace);
			bioModel = org.vcell.vmicro.workflow.scratch.FRAPStudy.createNewRefBioModel(expFrapStudy,
			...
			ROIDataGenerator roiDataGenerator = expFrapStudy.getROIDataGenerator(localWorkspace);
			...
			FRAPStudy.runFVSolverStandalone_ref(
				...
				expFrapStudy.getData().getFrapDataExternalDataInfo().getExternalDataIdentifier(),
				expFrapStudy.getData().getRoiExternalDataInfo().getExternalDataIdentifier(),
			...
			if(bioModel != null && bioModel.getSimulations() != null){
				FRAPStudy.removeExternalDataAndSimulationFiles(
					bioModel.getSimulations()[0].getVersion().getVersionKey(), null, null, localWorkspace);
			}
			throw e;
		}
	}
	
	public double computeError(org.vcell.vmicro.workflow.scratch.FRAPStudy expFrapStudy, LocalWorkspace localWorkspace, double newParamVals[], boolean[] eoi) throws Exception
	{
		double error = 0;
		int numberFixedParam = (data.fixedParam == null)? 0:1;
		if((getNumEstimatedParams() + numberFixedParam) == FRAPModel.NUM_MODEL_PARAMETERS_ONE_DIFF)
		{
			error = FRAPOptimizationUtils.getErrorByNewParameters_oneDiffRate(FRAPModel.REF_DIFFUSION_RATE_PARAM.getInitialGuess(), 
					                                              newParamVals,
					                                              getDimensionReducedRefData(expFrapStudy, localWorkspace, null, null),
					                                              getDimensionReducedExpData(expFrapStudy),
					                                              data.refDataTimePoints,
					                                              getReducedExpTimePoints(expFrapStudy),
					                                              expFrapStudy.getFrapData().getROILength(), 
					                                              eoi,
...
//		System.out.println("--------------------------------");
	...
	public double[][] getFitData(FRAPStudy expFrapStudy, LocalWorkspace localWorkspace, Parameter[] newParams) throws Exception
	{
		double[][] result = null;
		if(newParams.length == FRAPModel.NUM_MODEL_PARAMETERS_ONE_DIFF)
		{
			result = getFitData_oneDiffRate(expFrapStudy, localWorkspace, newParams);
		}
		else if(newParams.length == FRAPModel.NUM_MODEL_PARAMETERS_TWO_DIFF)
		{
			result = getFitData_twoDiffRates(expFrapStudy, localWorkspace, newParams);
		}
		else
		{
			throw new Exception("Wrong parameter size in FRAP optimazition.");
	...
	private double[][] getFitData_oneDiffRate(FRAPStudy expFrapStudy, LocalWorkspace localWorkspace, Parameter[] newParams) throws Exception
	{
		double[] reducedExpTimePoints = getReducedExpTimePoints(expFrapStudy);
		int roiLen = expFrapStudy.getFrapData().getROILength();
...
//			System.out.println("In getFitData() the variance computed is :" + error);
	...
	private double[][] getFitData_twoDiffRates(FRAPStudy expFrapStudy, LocalWorkspace localWorkspace, Parameter[] newParams) throws Exception
	{
		double[] reducedExpTimePoints = getReducedExpTimePoints(expFrapStudy);
		int roiLen = expFrapStudy.getFrapData().getROILength();
...
//					double newValue = (mFracFast * fastData[i][j] + mFracSlow * slowData[i][j] + immobileFrac * firstPostBleach[i]) * Math.exp(-(monitoringRate * expTimePoints[j]));
	...
	{
		data.fixedParam = arg_fixedParam;
	}
	
	public Parameter[] getBestParamters(FRAPStudy expFrapStudy, LocalWorkspace localWorkspace, Parameter[] inParams, boolean[] errorOfInterest) throws Exception
	{
		return getBestParamters(expFrapStudy, localWorkspace, inParams, errorOfInterest, null, true);
	}
	...
	public Parameter[] getBestParamters(FRAPStudy expFrapStudy, LocalWorkspace localWorkspace, Parameter[] inParams, boolean[] errorOfInterest, Parameter fixedParam, boolean bApplyMeasurementError) throws Exception
	{
		//refresh number of pixels in each roi 
		if(data.measurementErrors == null)
		{
			data.measurementErrors = FRAPOptimizationUtils.refreshNormalizedMeasurementError(expFrapStudy);
		}
		...
		//optimization
		double lestError = FRAPOptimizationUtils.estimate(expFrapStudy, localWorkspace, this, inParams, outParaNames, outParaVals, errorOfInterest);
...
//			cons[1] = con2;
			...
			optData.getBestParamters(expFrapStudy, localWorkspace, inParams, errorOfInterest);
...
//			}
	...
	public ProfileData[] evaluateParameters(FRAPStudy frapStudy, LocalWorkspace localWorkspace, Parameter[] currentParams, ClientTaskStatusSupport clientTaskStatusSupport) throws Exception
	{
//		long startTime =System.currentTimeMillis();
		...
		for(int j=0; j<totalParamLen; j++)
		{
			...
			Parameter[] newBestParameters = getBestParamters(frapStudy, localWorkspace, currentParams, frapStudy.getSelectedROIsForErrorCalculation(), null, true);
			...
			while(true)
			{
				...
				Parameter[] newParameters = getBestParamters(frapStudy, localWorkspace, unFixedParams, frapStudy.getSelectedROIsForErrorCalculation(), increasedParam, true);
...
//				}
			...
			while(true)
			{
				...
				//getBestParameters returns the whole set of parameters including the fixed parameters
				setNumEstimatedParams(totalParamLen-1);
				Parameter[] newParameters = getBestParamters(frapStudy, localWorkspace, unFixedParams, frapStudy.getSelectedROIsForErrorCalculation(), decreasedParam, true);
				for(int i=0; i < newParameters.length; i++)//use last step unfixed parameter values to optimize
				{
					for(int k=0; k<unFixedParams.length; k++)
					{
...
//		System.out.println("total time used:" + (endTime - startTime));
	...
	public void getFromStoredRefData(org.vcell.vmicro.workflow.scratch.FRAPStudy expFrapStudy, SimpleReferenceData argSimRefData)
	{
		int roiLen = expFrapStudy.getFrapData().getROILength();
		if(argSimRefData.getNumDataColumns() == (1+roiLen))
		{
			//t is in the first column of simpleReferenceData
			data.refDataTimePoints = argSimRefData.getDataByColumn(0);
			...
			data.dimensionReducedRefData = result;
		}
	}
}
