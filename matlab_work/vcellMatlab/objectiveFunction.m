...
%             v3 = this.sampledAbsProjections(1,3)*(1-u3_fract);
            ...
            u2 = u2_fract;
            u3 = u3_fract;
            v1 = (1-u1_fract); % by using normalized data
            v2 = (1-u2_fract); % by using normalized data
            v3 = (1-u3_fract); % by using normalized data
...
%                 ((u3*exp(-w1(3)*t)+v3*exp(-w2(3)*t))-this.sampledAbsProjections(:,3)).^2
            % use normalized data
            f = ((u1*exp(-w1(1)*t)+v1*exp(-w2(1)*t))-this.normalizedProjections(:,1)).^2 + ...
                ((u2*exp(-w1(2)*t)+v2*exp(-w2(2)*t))-this.normalizedProjections(:,2)).^2 + ...
                ((u3*exp(-w1(3)*t)+v3*exp(-w2(3)*t))-this.normalizedProjections(:,3)).^2
            objVal = sum(f)
