...
/**
 This is a freely available class (http://www.gurge.com/amd/java/quantize/) that performs 
 a color reduction algorithm for an image.
 It has been reported that the class has a problem: 
 'in case of large images the image takes on an off-tint.'
...
public class Quantize {
    ...
    /**
     * Get the 1D RGB pixel array from a 2D RGB pixel array.
     */
    protected static int [] getPixels(int pixels[][]) {
        int w = pixels.length;
    ...
    /**
     * Get the pixels (1D array) from an indexed color array, and a 2D pixel array.
     */
    protected static int [] getPixels(int palette[], int pixels[][]) {
        int w = pixels.length;
        int h = pixels[0].length;
        int pix[] = new int[w * h];
	 ...
	 /**
		Returns a 2D pixel array given an image object.
	 */
	protected static int [][] getPixels(Image image) throws IOException {
        ...
        try {
            if (grabber.grabPixels() != true) {
                throw new IOException("Grabber returned false: " + grabber.getStatus());
            }
    ...
    public static void main(String args []) throws IOException {
	    ...
	    String usage = "Usage: Quantize [image file] [# colors (to reduce image to)]";
	    
        if (args.length != 2) {
			System.err.println(usage);
			System.exit(0);
        }
        java.io.File imageFile = new java.io.File(args[0]);
        if (!imageFile.exists()) {
	        System.err.println("Image file: " + args[0] + " does not exist.");
			System.err.println(usage);
			System.exit(0);
        }
        int numColor = 0;
        try {
	        numColor = Integer.parseInt(args[1]);
        } catch (NumberFormatException nfe) {
			System.err.println("Invalid setting for the number of colors: " + args[1]);
			System.err.println(usage);
			System.exit(0);
        }
        javax.swing.JFrame frame = new javax.swing.JFrame();
 	    Image image = frame.getToolkit().getImage(imageFile.getAbsolutePath());
 	    //wait for the image to entirely load
        java.awt.MediaTracker tracker = new java.awt.MediaTracker(frame);
        ...
        if (tracker.statusID(0, true) != java.awt.MediaTracker.COMPLETE) {
            throw new IOException("Could not load: " + imageFile + " " + tracker.statusID(0, true));
        }
 	    long startTime = System.currentTimeMillis();
		int pixels [][] = Quantize.getPixels(image);
        int palette[] = Quantize.quantizeImage(pixels, numColor);
        int newPixels[] = Quantize.getPixels(palette, pixels);
    	int w = pixels.length;
        int h = pixels[0].length;
        image = frame.createImage(new java.awt.image.MemoryImageSource(w, h, newPixels, 0, w)); 
		System.out.println("Reduced image: " + imageFile.getName() + " to " + numColor + " in " + (System.currentTimeMillis() - startTime));
