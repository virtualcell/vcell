 ...
 */
private String getLineScan(User user, DataServerImpl dsc, VCDataIdentifier vcdID, String variableNames[], int timeIndex, CoordinateIndex begin, CoordinateIndex end) throws DataAccessException, RemoteException {

	String simID = vcdID.getID();
	double timepoint = dsc.getDataSetTimes(user,vcdID)[timeIndex];
	PlotData plotData = dsc.getLineScan(user,vcdID,variableNames[0],timepoint,begin,end);
	double distances[] = plotData.getIndependent();
	...
	if (variableNames.length>1) {
		for (int i=1;i<variableNames.length;i++) {
			plotData = dsc.getLineScan(user,vcdID,variableNames[i],timepoint,begin,end);
			for (int j=0;j<distances.length;j++) {
 ...
 * @exception java.rmi.RemoteException The exception description.
 */
private String getLineTimeSeries(User user, DataServerImpl dsc, VCDataIdentifier vcdID, String variable, CoordinateIndex begin, CoordinateIndex end, int beginIndex, int endIndex, boolean switchRowsColumns) throws DataAccessException, RemoteException {

	String simID = vcdID.getID();
	double[] allTimes = dsc.getDataSetTimes(user,vcdID);
	PlotData plotData = dsc.getLineScan(user,vcdID,variable, allTimes[beginIndex], begin, end);
	double[] distances = plotData.getIndependent();
	double[][] variableValues = new double[endIndex - beginIndex + 1][distances.length];
	for (int i=beginIndex;i<=endIndex;i++) {
		plotData = dsc.getLineScan(user,vcdID,variable, allTimes[i], begin, end);
		for (int j=0;j<distances.length;j++) {
//			setExportProgress((int)(100 * (step + ((double)((i - beginIndex) * distances.length + j)) / ((endIndex - beginIndex) * distances.length)) / numberOfSteps));
			variableValues[i - beginIndex][j] = plotData.getDependent()[j];
		}
