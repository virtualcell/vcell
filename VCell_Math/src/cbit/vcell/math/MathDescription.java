 ...
 * 
 */
public class MathDescription implements org.vcell.util.document.Versionable, Matchable, SymbolTable, Serializable {
			...
			for (int i = 0; i < subDomainsOld.length; i++){
				for (int j = 0; j < oldVars.length; j++){
					...
					if (!org.vcell.util.Compare.isEqualOrNull(oldEqu,newEqu)){
						...
						IExpression oldExps[] = (IExpression[])org.vcell.util.BeanUtils.getArray(oldEqu.getExpressions(oldMathDesc),IExpression.class);
						IExpression newExps[] = (IExpression[])org.vcell.util.BeanUtils.getArray(newEqu.getExpressions(newMathDesc),IExpression.class);
					...
					if (subDomainsOld[i] instanceof MembraneSubDomain && oldVars[j] instanceof VolVariable){
						...
						if (!org.vcell.util.Compare.isEqualOrNull(oldJumpCondition,newJumpCondition)){
							...
							IExpression oldExps[] = (IExpression[])org.vcell.util.BeanUtils.getArray(oldJumpCondition.getExpressions(oldMathDesc),IExpression.class);
							IExpression newExps[] = (IExpression[])org.vcell.util.BeanUtils.getArray(newJumpCondition.getExpressions(newMathDesc),IExpression.class);
							if (oldExps.length != newExps.length){
								reasonForDecision.append(DIFFERENT_NUMBER_OF_EXPRESSIONS);
								return false;
							}
 ...
 */
public static MathDescription createCanonicalMathDescription(MathDescription originalMathDescription) throws MathException, ExpressionException {
	//
 ...
 * @return cbit.vcell.math.MathDescription
 */
public static MathDescription createMathWithExpandedEquations(MathDescription originalMathDescription, HashSet varNamesToKeep) throws MathException, ExpressionException {
	//
	// clone current mathdescription
	//
	MathDescription newMath = new MathDescription(originalMathDescription);
 ...
 * @return cbit.vcell.math.MathDescription
 */
public static Function[] getFlattenedFunctions(MathDescription originalMathDescription, String functionNames[]) throws MathException, ExpressionException {
	//
	// clone current mathdescription
	//
	MathDescription newMath = new MathDescription(originalMathDescription);
 ...
 * @return cbit.vcell.math.MathDescription
 */
private void makeCanonical() throws MathException, ExpressionException {
...
//	cbit.vcell.simulation.Simulation tempSimulation = new cbit.vcell.simulation.Simulation(newMath);
	...
	for (int i = 0; i < newMath.subDomainList.size(); i++){
		...
		while (equEnum.hasMoreElements()){
			Equation equ = (Equation)equEnum.nextElement();
			equ.flatten(newMath,bRoundCoefficients);
		}
		if (subDomain instanceof MembraneSubDomain){
			...
			while (jcEnum.hasMoreElements()){
				...
				jc.flatten(newMath,bRoundCoefficients);
			}
		}
	}
	...
	for (int i = 0; i < newMath.subDomainList.size(); i++){
		...
		FastSystem fastSystem = subDomain.getFastSystem();
		if (fastSystem != null){
			fastSystem.flatten(newMath,bRoundCoefficients);
		}
	}
	...
	for (int i = 0; i < newMath.subDomainList.size(); i++){
		...
		while (equEnum.hasMoreElements()){
			Equation equ = (Equation)equEnum.nextElement();
			equ.bind(newMath,this);
		}
		if (subDomain instanceof MembraneSubDomain){
			Enumeration jcEnum = ((MembraneSubDomain)subDomain).getJumpConditions();
			while (jcEnum.hasMoreElements()){
				JumpCondition jc = (JumpCondition)jcEnum.nextElement();
				jc.bind(newMath,this);
			}
		}
	}
}
 ...
 * @return cbit.vcell.math.MathDescription
 */
private void substituteInPlace(Function functionsToSubstitute[]) throws MathException, ExpressionException {
	//
	// make a "identity" simulation (no overrides), this will help to substitute/flatten expressions.
	//
