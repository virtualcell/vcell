 ...
 */
public void addFunction(cbit.vcell.math.AnnotatedFunction argFunction) throws cbit.util.DataAccessException {
 ...
 */
public void addFunctions(cbit.vcell.math.AnnotatedFunction[] argFunctions) throws cbit.util.DataAccessException {
...
public cbit.vcell.math.DataIdentifier[] getDataIdentifiers() throws cbit.util.DataAccessException {
	cbit.vcell.math.DataIdentifier[] dataIdentifiers = new cbit.vcell.math.DataIdentifier[functions.length];
	for (int i = 0; i < dataIdentifiers.length; i++){
		dataIdentifiers[i] = new cbit.vcell.math.DataIdentifier(functions[i].getName(),cbit.vcell.math.VariableType.VOLUME);
	}
...
public static DataManagerTest getPDEExample1() {
	try {
		...
		functionList.add(new cbit.vcell.math.AnnotatedFunction("F1",new Expression("sin(x+2*y)"),"",VariableType.VOLUME,false));
...
public cbit.vcell.simdata.SimDataBlock getSimDataBlock(String varName, double time) throws cbit.util.DataAccessException {
	...
	cbit.vcell.simdata.SimDataBlock simDataBlock = new cbit.vcell.simdata.SimDataBlock(pdeDataInfo, data, cbit.vcell.math.VariableType.VOLUME);
 ...
 */
public double[][] getTimeSeriesValues(String varName, int[] indices) throws cbit.util.DataAccessException {
	throw new RuntimeException("not yet implemented");
}
/**
