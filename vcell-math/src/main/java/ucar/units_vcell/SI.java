...
/**
 * Provides support for the International System of Units
 * (SI).  A good on-line reference for the SI can be found at
 * <http://physics.nist.gov/cuu/Units/>.
 *
 * @author Steven R. Emmerson
 * @version $Id: SI.java,v 1.4 2000/07/18 20:15:27 steve Exp $
 */
public final class
SI
    extends	UnitSystemImpl
{
    ...
    /**
     * The unit corresponding to the quantity of Item.
     */
    public static final Unit	ITEM_UNIT;
     ...
     * 10^12 hertz and that has a radiant intensity in that direction of
     * (1/683) watt per steradian.
     */
    public static final BaseUnit	CANDELA;

    /**
     * Base unit of thermodynamic temperature.  The kelvin, unit of
     ...
     * radiation corresponding to the trasition between the two
    ...
    /*
     * Supplementary units:
     * These units are more accurately referred to as "dimensionless
     * derived units" -- for the purposes of this Java package,
    ...
    /**
     * Base unit item (molecules) - used in SBML.
     */
    public static final BaseUnit	ITEM;
    ...
    /**
     * Special derived unit for the quantity of energy, work, or quantity
     * of heat.
     */
    ...
    /**
     * Special derived unit for the quantity of electric charge or quantity
     * of electricity.
     */
    ...
    /*
     * Derived units with special names and symbols admitted for reasons
     * of safegarding human health.
     */
    ...
    /**
     * Special derived unit for the quantity of absorbed dose, specific
     * energy (imparted), or kerms.
     */
    public static final Unit	GRAY;

    /**
     * Special derived unit for the quantity of dose equivalent, ambient
     * dose equivalent, directional dose equivalent, personal dose
     * equivalent, or equivalent dose.
     */
    public static final Unit	SIEVERT;

    /*
     * Units outside the SI but accepted for use with the SI according to
     * the USA National Institute of Standards and Technology (see 
     * <http://physics.nist.gov/cuu/Units/>):
     */

    /**
     * Special derived unit -- accepted for use with the SI -- for the 
     * quantity of time.
     */
     ...
     * according to the USA National Institute of Standards and Technology
     * (see <http://physics.nist.gov/cuu/Units/>):
     */

    /**
     * Special derived unit -- temporarily accepted for use with the SI --
     * for the quantity of length.
     */
    public static final Unit	NAUTICAL_MILE;

    /**
     * Special derived unit -- temporarily accepted for use with the SI --
     * for the quantity of speed.
     */
    ...
    /**
     * Special derived unit -- temporarily accepted for use with the SI --
     * for the quantity of pressure.
     */
    public static final Unit	BAR;

    /**
     * Special derived unit -- temporarily accepted for use with the SI --
     * for the quantity of acceleration.
     */
    public static final Unit	GAL;

    /*
     * Units outside the SI but temporarily accepted for use with the SI
     * until the year 2000 according to the USA National Institute of
     * Standards and Technology (see <http://physics.nist.gov/cuu/Units/>):
     */

    /**
     * Special derived unit -- temporarily accepted for use with the SI 
     * until the year 2000 according to the USA National Institute of 
     * Standards and Technology -- for the quantity of activity of a
     * radionuclide.
     */
     ...
     * Special derived unit -- temporarily accepted for use with the SI 
     * until the year 2000 according to the USA National Institute of 
     * Standards and Technology -- for the quantity of electric charge
     * per mass.
     */
    public static final Unit	ROENTGEN;

    /**
     * Special derived unit -- temporarilty accepted for use with the SI 
     * until the year 2000 according to the USA National Institute of 
     * Standards and Technology -- for the quantity of absorbed dose.
     */
    public static final Unit	RAD;

    /**
     * Special derived unit -- temporarilty accepted for use with the
     * SI until the year 2000 according to the USA National Institute
     * of Standards and Technology -- for the quantity of equivalent
     * absorbed dose.
     */
    public static final Unit	REM;

    /**
     * Scaled Unit for kilogram <code>GRAM</code>.
     */
	...
	try
	{
	    ...
	    metre       	= bu("metre",     "m",   BaseQuantity.LENGTH);
	    ...
	    item			= bu("item",      "item", BaseQuantity.ITEM);
            ...
            hertz           = du("hertz", "Hz", second.raiseTo(new RationalNumber(-1)));
            newton          = du("newton", "N", kilogram.multiplyBy(metre).divideBy(second.raiseTo(new RationalNumber(2))));
            pascal          = du("pascal", "Pa", newton.divideBy(metre.raiseTo(new RationalNumber(2))));
            joule           = du("joule", "J", newton.multiplyBy(metre));
            ...
            siemens         = du("siemens", "S", ohm.raiseTo(new RationalNumber(-1)));
            weber           = du("weber", "Wb", volt.multiplyBy(second));
            tesla           = du("tesla", "T", weber.divideBy(metre.raiseTo(new RationalNumber(2))));
            ...
            hour            = du("hour", "h", new ScaledUnit(60, (UnitImpl)minute));
            day             = du("day", "d", new ScaledUnit(24, (UnitImpl)hour));
            arc_degree      = du("arc degree", "deg", new ScaledUnit(Math.PI/180, radian));
            arc_minute      = du("arc minute", "'", new ScaledUnit(1./60., (UnitImpl)arc_degree));
            arc_second      = du("arc second", "\"", new ScaledUnit(1./60., (UnitImpl)arc_minute));
            liter           = du("liter", "L", new ScaledUnit(1e-3, (UnitImpl)metre.raiseTo(new RationalNumber(3))));
		// exact. However, from 1901 to 1964, 1 liter = 1.000028 dm3
            metric_ton      = du("metric ton", "t", new ScaledUnit(1e3, kilogram));

            nautical_mile   = du("nautical mile", "nmi", new ScaledUnit(1852, metre));
            knot            = du("knot", "kt", nautical_mile.divideBy(hour));
            angstrom        = du("angstrom", null, new ScaledUnit(1e-10, metre));
            are             = du("are", "are", new ScaledUnit(10, metre).raiseTo(new RationalNumber(2)));
            hectare         = du("hectare", "ha", new ScaledUnit(100, (UnitImpl)are));
            barn            = du("barn", "b", new ScaledUnit(1e-28, (UnitImpl)metre.raiseTo(new RationalNumber(2))));
            bar             = du("bar", "bar", new ScaledUnit(1e5, (UnitImpl)pascal));
            gal             = du("gal", "Gal", new ScaledUnit(1e-2, metre).divideBy(second.raiseTo(new RationalNumber(2))));

            curie           = du("curie", "Ci", new ScaledUnit(3.7e10, (UnitImpl)becquerel));
            roentgen        = du("roentgen", "R", new ScaledUnit(2.58e-4, (UnitImpl)coulomb.divideBy(kilogram)));
            rad             = du("rad", "rd", new ScaledUnit(1e-2, (UnitImpl)gray));
            rem             = du("rem", "rem", new ScaledUnit(1e-2, (UnitImpl)sievert));
            gram			= du("gram", "g", new ScaledUnit(1e-3, kilogram));
	}
	catch(UnitException e)
	{
	    String	reason = e.getMessage();
	    System.err.println("Couldn't initialize class SI" +
		reason == null ? "" : (": " + reason));
	}
    ...
    private
    SI()
	throws UnitExistsException, NameException, PrefixDBException,
	    NoSuchUnitException
    {
     ...
     */
    private static BaseUnit
    bu(String name, String symbol, BaseQuantity quantity)
	throws NameException, UnitExistsException
    {
	return 
	    BaseUnit.getOrCreate(
		UnitName.newUnitName(name, null, symbol), quantity);
    }
     ...
     */
    private static Unit
    du(String name, String symbol, Unit definition)
     ...
     * @throws UnitSystemException	Couldn't create an instance of the 
     *					SI system of units.
     */
    public static SI
    instance()
