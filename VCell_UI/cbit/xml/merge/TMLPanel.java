		...
		if ( firstNode.getName().equalsIgnoreCase(secondNode.getName()) ) {
			if (firstNode.getStatus()!= secondNode.getStatus()) {
				...
				try {
					...
					XmlTreeDiff partialDiffTree = XmlTreeDiff.compareMerge(firstNode.toXmlString(),secondNode.toXmlString(), XmlTreeDiff.COMPARE_DOCS_SAVED, fieldDiffTree.isIgnoringVersionInfo());
...
public static void main(java.lang.String[] args) {
	try {
		...
		XmlTreeDiff diffTree = XmlTreeDiff.compareMerge(baselineXML, modifiedXML, XmlTreeDiff.COMPARE_DOCS_SAVED, ignoreVersion);
	...
	public VCDocument processComparisonResult() throws Exception {
		try { 
			...
			String schemaLocation = cbit.vcell.vcml.Translator.VCML_NS + " " + 
			                        PropertyLoader.getProperty(PropertyLoader.vcmlSchemaUrlProperty, DEF_VCML_SL); 
