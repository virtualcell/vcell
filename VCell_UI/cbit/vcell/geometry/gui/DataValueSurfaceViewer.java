 ...
 * @author: Frank Morgan
 */
public class DataValueSurfaceViewer extends javax.swing.JPanel implements
	...
	public class SurfaceCollectionDataInfo {
		...
		// public SurfaceCollectionDataInfoProvider
		// getSurfaceCollectionDataInfoProvider(){return
		// surfaceCollectionDataInfoProvider;}
	}

	private class DataValueSurfaceViewerTableModel extends
		...
		private final String[] columnNames = new String[] { "Name", "Size",
				"Visible", "Wireframe", "Transparent" };

		private static final int TRANSPARENT_NONE = 0;

		private final String[] transparentValues = new String[] { "None",
				"25%", "50%", "75%" };

		protected javax.swing.JComboBox transparentEditor = new javax.swing.JComboBox(
				transparentValues);
		...
		public int getRowCount() {
			return (getSurfaceCollectionDataInfo() != null ? getSurfaceCollectionDataInfo()
					.getSurfaceCollection().getSurfaceCount()
					: 0);
		}
		...
		public java.lang.Object getValueAt(int row, int col) {
			if (col == NAME_COLUMN) {
				return (getSurfaceCollectionDataInfo() != null
						&& getSurfaceCollectionDataInfo().getSurfaceNames() != null ? getSurfaceCollectionDataInfo()
						.getSurfaceNames()[row]
						: null);
			} else if (col == SIZE_COLUMN) {
				return (getSurfaceCollectionDataInfo() != null
						&& getSurfaceCollectionDataInfo().getSurfaceAreas() != null ? getSurfaceCollectionDataInfo()
						.getSurfaceAreas()[row]
						: null);
			} else if (col == VISIBLE_COLUMN) {
				return (bDisplay != null ? new Boolean(bDisplay[row]) : null);
			} else if (col == WIREFRAME_COLUMN) {
				return (bWireframe != null ? new Boolean(bWireframe[row])
						: null);
			} else if (col == TRANSPARENT_COLUMN) {
				return (transparentSelection != null ? transparentValues[(int) ((1 - transparentSelection[row]
						.getAlpha()) * 4)]
						: null);
			}
			return null;
		...
		public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
			if (columnIndex == VISIBLE_COLUMN) {
				...
				int index = ((javax.swing.DefaultComboBoxModel) transparentEditor
						.getModel()).getIndexOf(aValue);
				if (index > 0) {
					float alpha = 1.0f - (float) (.25 * (index));
					transparentSelection[rowIndex] = java.awt.AlphaComposite
							.getInstance(java.awt.AlphaComposite.SRC_OVER,
									alpha);
				} else {
					transparentSelection[rowIndex] = java.awt.AlphaComposite
							.getInstance(java.awt.AlphaComposite.SRC);
				}
			}
		...
		public void tableChanged(javax.swing.event.TableModelEvent e) {
			if (e.getType() == javax.swing.event.TableModelEvent.UPDATE) {
				if (e.getColumn() == VISIBLE_COLUMN) {
					getSurfaceCanvas1().setSurfacesShowing(
							(boolean[]) bDisplay.clone());
				} else if (e.getColumn() == WIREFRAME_COLUMN) {
					getSurfaceCanvas1().setSurfacesWireframe(
							(boolean[]) bWireframe.clone());
				} else if (e.getColumn() == TRANSPARENT_COLUMN) {
					getSurfaceCanvas1().setSurfaceTransparency(
							(java.awt.AlphaComposite[]) transparentSelection
									.clone());
				}
			}
	...
	class IvjEventHandler implements java.awt.event.ActionListener,
		...
		public void actionPerformed(java.awt.event.ActionEvent e) {
			if (e.getSource() == DataValueSurfaceViewer.this
					.getJButtonHomeView())
				connEtoC4(e);
			if (e.getSource() == DataValueSurfaceViewer.this
					.getJButtonFrontView())
			...
			if (e.getSource() == DataValueSurfaceViewer.this
				...
				connEtoC10(e);
			if (e.getSource() == DataValueSurfaceViewer.this
					.getJTextFieldXViewAngle())
				connEtoM3(e);
			if (e.getSource() == DataValueSurfaceViewer.this
					.getJTextFieldYViewAngle())
				connEtoM4(e);
			if (e.getSource() == DataValueSurfaceViewer.this
					.getJTextFieldZViewAngle())
				connEtoM5(e);
			if (e.getSource() == DataValueSurfaceViewer.this
					.getJButtonSetViewAngle())
				connEtoC11(e);
			if (e.getSource() == DataValueSurfaceViewer.this
					.getJCheckBoxBoundingbox())
				connEtoC13(e);
		...
		public void propertyChange(java.beans.PropertyChangeEvent evt) {
			if (evt.getSource() == DataValueSurfaceViewer.this
					&& (evt.getPropertyName()
							.equals("surfaceCollectionDataInfo")))
				connEtoC2(evt);
			if (evt.getSource() == DataValueSurfaceViewer.this
					.getSurfaceViewerTool1()
					&& (evt.getPropertyName().equals("viewAngleRadians")))
				connEtoC12(evt);
			if (evt.getSource() == DataValueSurfaceViewer.this
					.getSurfaceViewerTool1()
					&& (evt.getPropertyName().equals("currentManipulation")))
				connEtoC14(evt);
		};
	};
	...
	public void actionPerformed(java.awt.event.ActionEvent e) {
		...
		if (e.getSource() == dsr) {
			...
			// else if(e.getID() ==
			// cbit.vcell.client.data.DataSelectRetrieve.SELECTION_TOAOI_LINE){
			// if(polylinePicks.size() > 0 && bAOI != null){
			// boolean pickOP = (dsr.getDSRState().selectionType ==
			// cbit.vcell.client.data.DataSelectRetrieve.SELECT_TYPE_ADD?true:false);
			// for(int i=0;i<polylinePicks.size();i+= 1){
			// SurfaceCanvas.SurfaceCollectionPick lineElement =
			// (SurfaceCanvas.SurfaceCollectionPick)polylinePicks.elementAt(i);
			// bAOI[lineElement.surfaceIndex][lineElement.polygonIndex] =
			// pickOP;
			// }
			// polylinePicks.clear();
			// updateAreaOfInterest();
			...
			// }
			// else if(e.getID() ==
			// cbit.vcell.client.data.DataSelectRetrieve.RETRIEVE_DATA){
			...
			// if(bAOI[i][j]){
			// indices[0][count] =
			// getSurfaceCollectionDataInfoProvider().getMembraneIndex(i,j);
			// count+= 1;
			// }
			// }
			// }
			// try{
			// cbit.vcell.client.data.DataSelectRetrieve.DSRState dsrState =
			// dsr.getDSRState();
			...
			// //double[][] mins =
			// ((cbit.util.TSJobResultsTimeStats)tsjr).getMinimums();
	...
	private void configureView(java.awt.event.ActionEvent actionEvent) {
		...
		if (actionEvent.getSource() == getJButtonHomeView()) {
			...
			getSurfaceViewerTool1().setViewAngleRadians(
					new Vect3d(-Math.PI / 2, 0, 0));
			...
			getSurfaceViewerTool1().setViewAngleRadians(
					new Vect3d(0, Math.PI, 0));

		} else if (actionEvent.getSource() == getJButtonLeftView()) {
			getSurfaceViewerTool1().setViewAngleRadians(
					new Vect3d(-Math.PI / 2, -Math.PI / 2, 0));

		} else if (actionEvent.getSource() == getJButtonRightView()) {
			getSurfaceViewerTool1().setViewAngleRadians(
					new Vect3d(0, Math.PI / 2, 0));
		} else if (actionEvent.getSource() == getJButtonSetViewAngle()) {
			try {
				double angleX = Double.parseDouble(getJTextFieldXViewAngle()
						.getText())
						* 2 * Math.PI / 360;
				double angleY = Double.parseDouble(getJTextFieldYViewAngle()
						.getText())
						* 2 * Math.PI / 360;
				double angleZ = Double.parseDouble(getJTextFieldZViewAngle()
						.getText())
						* 2 * Math.PI / 360;
				getSurfaceViewerTool1().setViewAngleRadians(
						new Vect3d(angleX, angleY, angleZ));
	...
	/**
	 * connEtoC1: (DataValueSurfaceViewer.initialize() -->
	 * DataValueSurfaceViewer.dataValueSurfaceViewer_Initialize()V)
	 */
	...
	/**
	 * connEtoC10:
	 * (JButtonRightView.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * DataValueSurfaceViewer.configureView(Ljava.awt.event.ActionEvent;)V)
	 * 
	 * @param arg1
	...
	/**
	 * connEtoC11:
	 * (JButtonSetViewAngle.action.actionPerformed(java.awt.event.ActionEvent)
	 * --> DataValueSurfaceViewer.configureView(Ljava.awt.event.ActionEvent;)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	...
	/**
	 * connEtoC12: (SurfaceViewerTool1.viewAngleRadians -->
	 * DataValueSurfaceViewer.surfaceViewerTool1_ViewAngleRadians(Lcbit.vcell.render.Vect3d;)V)
	 * 
	 * @param arg1
	...
	private void connEtoC12(java.beans.PropertyChangeEvent arg1) {
		try {
			// user code begin {1}
			// user code end
			this.surfaceViewerTool1_ViewAngleRadians(getSurfaceViewerTool1()
					.getViewAngleRadians());
			// user code begin {2}
			// user code end
		} catch (java.lang.Throwable ivjExc) {
	...
	/**
	 * connEtoC13:
	 * (JCheckBoxBoundingbox.action.actionPerformed(java.awt.event.ActionEvent)
	 * -->
	 * DataValueSurfaceViewer.jCheckBoxBoundingbox_ActionPerformed(Ljava.awt.event.ActionEvent;)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	 */
	...
	/**
	 * connEtoC14: (SurfaceViewerTool1.currentManipulation -->
	 * DataValueSurfaceViewer.setModeCursor()V)
	 * 
	 * @param arg1
	 *            java.beans.PropertyChangeEvent
	 */
	...
	/**
	 * connEtoC15:
	 * (JButtonDSR.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * DataValueSurfaceViewer.jButtonDSR_ActionPerformed(Ljava.awt.event.ActionEvent;)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	 */
	...
	/**
	 * connEtoC16:
	 * (SurfaceCanvas1.mouseMotion.mouseMoved(java.awt.event.MouseEvent) -->
	 * DataValueSurfaceViewer.surfaceCanvas1_MouseMoved(Ljava.awt.event.MouseEvent;)V)
	 * 
	 * @param arg1
	...
	/**
	 * connEtoC17: (SurfaceCanvas1.mouse.mouseExited(java.awt.event.MouseEvent)
	 * -->
	 * DataValueSurfaceViewer.surfaceCanvas1_MouseExited(Ljava.awt.event.MouseEvent;)V)
	 * 
	 * @param arg1
	 *            java.awt.event.MouseEvent
	 */
	...
	/**
	 * connEtoC18:
	 * (JButtonStats.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * DataValueSurfaceViewer.jButtonStats_ActionPerformed(Ljava.awt.event.ActionEvent;)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	 */
	...
	/**
	 * connEtoC2: (DataValueSurfaceViewer.surfaceCollectionDataInfo -->
	 * DataValueSurfaceViewer.refresh()V)
	 * 
	 * @param arg1
	 *            java.beans.PropertyChangeEvent
	 */
	...
	/**
	 * connEtoC3: (SurfaceCanvas1.mouse.mouseClicked(java.awt.event.MouseEvent)
	 * --> DataValueSurfaceViewer.pickPolygon(Ljava.awt.event.MouseEvent;)V)
	 * 
	 * @param arg1
	 *            java.awt.event.MouseEvent
	 */
	...
	/**
	 * connEtoC4:
	 * (JButtonHomeView.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * DataValueSurfaceViewer.configureView(Ljava.awt.event.ActionEvent;)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	...
	/**
	 * connEtoC5:
	 * (JButtonFrontView.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * DataValueSurfaceViewer.configureView(Ljava.awt.event.ActionEvent;)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	...
	/**
	 * connEtoC6:
	 * (JButtonBackView.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * DataValueSurfaceViewer.configureView(Ljava.awt.event.ActionEvent;)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	...
	/**
	 * connEtoC7:
	 * (JButtonTopView.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * DataValueSurfaceViewer.configureView(Ljava.awt.event.ActionEvent;)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	...
	/**
	 * connEtoC8:
	 * (JButtonBottomView.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * DataValueSurfaceViewer.configureView(Ljava.awt.event.ActionEvent;)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	...
	/**
	 * connEtoC9:
	 * (JButtonLeftView.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * DataValueSurfaceViewer.configureView(Ljava.awt.event.ActionEvent;)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	...
	/**
	 * connEtoM1: (DataValueSurfaceViewer.initialize() -->
	 * SurfaceViewerTool1.surfaceCanvas)
	 */
	/* WARNING: THIS METHOD WILL BE REGENERATED. */
	...
	/**
	 * connEtoM3:
	 * (JTextField3.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * JButtonSetViewAngle.doClick(I)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	 */
	...
	/**
	 * connEtoM4:
	 * (JTextField2.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * JButtonSetViewAngle.doClick(I)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	...
	/**
	 * connEtoM5:
	 * (JTextField1.action.actionPerformed(java.awt.event.ActionEvent) -->
	 * JButtonSetViewAngle.doClick(I)V)
	 * 
	 * @param arg1
	 *            java.awt.event.ActionEvent
	...
	private void dataValueSurfaceViewer_Initialize() {
		...
		getScrollPaneTable().getColumnModel().getColumn(
				DataValueSurfaceViewerTableModel.TRANSPARENT_COLUMN)
				.setCellEditor(
						new javax.swing.DefaultCellEditor(
								dvsvtm.transparentEditor));

		getScrollPaneTable().setPreferredScrollableViewportSize(
	...
	private javax.swing.JLabel getJLabelDirections() {
		if (ivjJLabelDirections == null) {
			try {
				...
				ivjJLabelDirections
						.setText("ROTATE(leftMB-drag)  MOVE(rightMB-drag)  ZOOM(leftMB-rigthMB-drag)");
				// user code begin {1}
				// user code end
			} catch (java.lang.Throwable ivjExc) {
				// user code begin {2}
	...
	private javax.swing.JPanel getJPanel3() {
		if (ivjJPanel3 == null) {
			try {
				...
				constraintsJButtonHomeView.gridx = 0;
				constraintsJButtonHomeView.gridy = 6;
				constraintsJButtonHomeView.gridwidth = 2;
				constraintsJButtonHomeView.fill = java.awt.GridBagConstraints.HORIZONTAL;
				constraintsJButtonHomeView.insets = new java.awt.Insets(4, 4,
						4, 4);
				getJPanel3().add(getJButtonHomeView(),
						constraintsJButtonHomeView);

				java.awt.GridBagConstraints constraintsJButtonFrontView = new java.awt.GridBagConstraints();
				constraintsJButtonFrontView.gridx = 0;
				constraintsJButtonFrontView.gridy = 9;
				constraintsJButtonFrontView.gridwidth = 2;
				constraintsJButtonFrontView.fill = java.awt.GridBagConstraints.HORIZONTAL;
				constraintsJButtonFrontView.insets = new java.awt.Insets(4, 4,
						4, 4);
				getJPanel3().add(getJButtonFrontView(),
						constraintsJButtonFrontView);

				java.awt.GridBagConstraints constraintsJButtonTopView = new java.awt.GridBagConstraints();
				constraintsJButtonTopView.gridx = 0;
				constraintsJButtonTopView.gridy = 7;
				constraintsJButtonTopView.gridwidth = 2;
				constraintsJButtonTopView.fill = java.awt.GridBagConstraints.HORIZONTAL;
				constraintsJButtonTopView.insets = new java.awt.Insets(4, 4, 4,
						4);
				getJPanel3()
						.add(getJButtonTopView(), constraintsJButtonTopView);

				java.awt.GridBagConstraints constraintsJButtonLeftView = new java.awt.GridBagConstraints();
				constraintsJButtonLeftView.gridx = 0;
				constraintsJButtonLeftView.gridy = 11;
				constraintsJButtonLeftView.gridwidth = 2;
				constraintsJButtonLeftView.fill = java.awt.GridBagConstraints.HORIZONTAL;
				constraintsJButtonLeftView.insets = new java.awt.Insets(4, 4,
						4, 4);
				getJPanel3().add(getJButtonLeftView(),
						constraintsJButtonLeftView);

				java.awt.GridBagConstraints constraintsJButtonBackView = new java.awt.GridBagConstraints();
				constraintsJButtonBackView.gridx = 0;
				constraintsJButtonBackView.gridy = 10;
				constraintsJButtonBackView.gridwidth = 2;
				constraintsJButtonBackView.fill = java.awt.GridBagConstraints.HORIZONTAL;
				constraintsJButtonBackView.insets = new java.awt.Insets(4, 4,
						4, 4);
				getJPanel3().add(getJButtonBackView(),
						constraintsJButtonBackView);

				java.awt.GridBagConstraints constraintsJButtonBottomView = new java.awt.GridBagConstraints();
				constraintsJButtonBottomView.gridx = 0;
				constraintsJButtonBottomView.gridy = 8;
				constraintsJButtonBottomView.gridwidth = 2;
				constraintsJButtonBottomView.fill = java.awt.GridBagConstraints.HORIZONTAL;
				constraintsJButtonBottomView.insets = new java.awt.Insets(4, 4,
						4, 4);
				getJPanel3().add(getJButtonBottomView(),
						constraintsJButtonBottomView);

				java.awt.GridBagConstraints constraintsJButtonRightView = new java.awt.GridBagConstraints();
				constraintsJButtonRightView.gridx = 0;
				constraintsJButtonRightView.gridy = 12;
				constraintsJButtonRightView.gridwidth = 2;
				constraintsJButtonRightView.fill = java.awt.GridBagConstraints.HORIZONTAL;
				constraintsJButtonRightView.insets = new java.awt.Insets(4, 4,
						4, 4);
				getJPanel3().add(getJButtonRightView(),
						constraintsJButtonRightView);

				java.awt.GridBagConstraints constraintsJTextFieldZViewAngle = new java.awt.GridBagConstraints();
				constraintsJTextFieldZViewAngle.gridx = 1;
				constraintsJTextFieldZViewAngle.gridy = 3;
				constraintsJTextFieldZViewAngle.fill = java.awt.GridBagConstraints.HORIZONTAL;
				constraintsJTextFieldZViewAngle.weightx = 1.0;
				constraintsJTextFieldZViewAngle.insets = new java.awt.Insets(4,
						4, 4, 4);
				getJPanel3().add(getJTextFieldZViewAngle(),
						constraintsJTextFieldZViewAngle);

				java.awt.GridBagConstraints constraintsJTextFieldYViewAngle = new java.awt.GridBagConstraints();
				constraintsJTextFieldYViewAngle.gridx = 1;
				constraintsJTextFieldYViewAngle.gridy = 2;
				constraintsJTextFieldYViewAngle.fill = java.awt.GridBagConstraints.HORIZONTAL;
				constraintsJTextFieldYViewAngle.weightx = 1.0;
				constraintsJTextFieldYViewAngle.insets = new java.awt.Insets(4,
						4, 4, 4);
				getJPanel3().add(getJTextFieldYViewAngle(),
						constraintsJTextFieldYViewAngle);

				java.awt.GridBagConstraints constraintsJTextFieldXViewAngle = new java.awt.GridBagConstraints();
				constraintsJTextFieldXViewAngle.gridx = 1;
				constraintsJTextFieldXViewAngle.gridy = 1;
				constraintsJTextFieldXViewAngle.fill = java.awt.GridBagConstraints.HORIZONTAL;
				constraintsJTextFieldXViewAngle.weightx = 1.0;
				constraintsJTextFieldXViewAngle.insets = new java.awt.Insets(4,
						4, 4, 4);
				getJPanel3().add(getJTextFieldXViewAngle(),
						constraintsJTextFieldXViewAngle);
				...
				constraintsJButtonSetViewAngle.gridx = 0;
				constraintsJButtonSetViewAngle.gridy = 4;
				constraintsJButtonSetViewAngle.gridwidth = 2;
				constraintsJButtonSetViewAngle.insets = new java.awt.Insets(4,
						4, 4, 4);
				getJPanel3().add(getJButtonSetViewAngle(),
						constraintsJButtonSetViewAngle);

				java.awt.GridBagConstraints constraintsJCheckBoxBoundingbox = new java.awt.GridBagConstraints();
				constraintsJCheckBoxBoundingbox.gridx = 0;
				constraintsJCheckBoxBoundingbox.gridy = 13;
				constraintsJCheckBoxBoundingbox.gridwidth = 2;
				constraintsJCheckBoxBoundingbox.insets = new java.awt.Insets(4,
						4, 4, 4);
				getJPanel3().add(getJCheckBoxBoundingbox(),
						constraintsJCheckBoxBoundingbox);

				java.awt.GridBagConstraints constraintsJButtonDSR = new java.awt.GridBagConstraints();
				constraintsJButtonDSR.gridx = 0;
				constraintsJButtonDSR.gridy = 14;
				...
				constraintsJButtonStats.gridx = 0;
				constraintsJButtonStats.gridy = 15;
	...
	private javax.swing.JScrollPane getJScrollPane1() {
		if (ivjJScrollPane1 == null) {
			try {
				...
				ivjJScrollPane1
						.setMinimumSize(new java.awt.Dimension(468, 100));
	...
	/**
	 * Gets the surfaceCollectionDataInfoProvider property (java.lang.Object)
	 * value.
	 * 
	...
	/**
	 * Insert the method's description here. Creation date: (9/21/2005 1:51:15
	 * PM)
	 */
	public void init(SurfaceCollection argSurfaceCollection,
		...
		setSurfaceCollectionDataInfo(new SurfaceCollectionDataInfo(
				argSurfaceCollection, argOrigin, argExtent, argSurfaceNames,
				argSurfaceAreas, argDimension));
	...
	private void initialize() {
		try {
			...
			constraintsSurfaceCanvas1.gridx = 1;
			constraintsSurfaceCanvas1.gridy = 0;
			...
			constraintsJLabelDirections.gridx = 1;
			constraintsJLabelDirections.gridy = 1;
			...
			constraintsJLabelInfo.gridx = 1;
			constraintsJLabelInfo.gridy = 2;
	 ...
	 * Comment
	 */
	private void jButtonDSR_ActionPerformed(
			java.awt.event.ActionEvent actionEvent) {

		// if(dsrJDialog == null){
		dsr
				.setModeInstruction("(CNTRL-CLICK on surface to add/remove from selection)");
		// dsrJDialog = new
		// javax.swing.JDialog((java.awt.Frame)cbit.util.BeanUtils.findTypeParentOfComponent(this,java.awt.Frame.class));
		...
		getSurfaceCollectionDataInfoProvider().showComponentInFrame(dsr,
	 ...
	 * Comment
	 */
	private void jButtonStats_ActionPerformed(
			java.awt.event.ActionEvent actionEvent) {
		if (bAOI == null || bAOI.length == 0) {
			return;
		}
		...
		if (count > 0) {
			...
			for (int i = 0; i < bAOI.length; i += 1) {
				for (int j = 0; j < bAOI[i].length; j += 1) {
					if (bAOI[i][j]) {
						indices[0][count] = getSurfaceCollectionDataInfoProvider()
								.getMembraneIndex(i, j);
						count += 1;
					}
				}
			}
			try {
				getSurfaceCollectionDataInfoProvider().getTimeSeriesData(
						indices, true, false, true);
			} catch (Exception e2) {
				cbit.vcell.client.PopupGenerator.showErrorDialog(e2
						.getMessage());
			}
		} else {
			cbit.vcell.client.PopupGenerator
					.showErrorDialog("Region of Interest must be defined to calculate Statistics");
		}
	}
	 ...
	 * Comment
	 */
	private void jCheckBoxBoundingbox_ActionPerformed(
			java.awt.event.ActionEvent actionEvent) {
		getSurfaceCanvas1().setShowBoundingBox(
				getJCheckBoxBoundingbox().isSelected());
	...
	/**
	 * Insert the method's description here. Creation date: (2/22/2006 5:28:13
	 * PM)
	 */
	public void pickByAnalytic() {

		try {
			String[] surfNames = getSurfaceCollectionDataInfo()
					.getSurfaceNames();
			String surfSelect = (String) cbit.vcell.client.PopupGenerator
					.showListDialog(this, surfNames,
							"Apply Analytic Area to Surface");

			cbit.vcell.client.data.DataSelectRetrieve.DSRState dsrState = dsr
					.getDSRState();
			boolean pickOP = (dsrState.selectionType == cbit.vcell.client.data.DataSelectRetrieve.SELECT_TYPE_ADD ? true
					: false);
			...
			if (xyzExp != null) {
				for (int surf = 0; surf < surfNames.length; surf += 1) {
					if (surfNames[surf].equals(surfSelect)) {
						int polygonCount = getSurfaceCollectionDataInfo()
								.getSurfaceCollection().getSurfaces(surf)
								.getPolygonCount();
						for (int i = 0; i < polygonCount; i += 1) {
							Vect3d coord = getSurfaceCollectionDataInfoProvider()
									.getCentroid(surf, i);
							if (coord == null) {
								coord = ((Quadrilateral) getSurfaceCollectionDataInfo()
										.getSurfaceCollection().getSurfaces(
												surf).getPolygons(i))
										.calculateCentroid();
							}
							if (xyzExp.evaluateVector(new double[] {
									coord.getX(), coord.getY(), coord.getZ() }) == 1.0) {
								bAOI[surf][i] = pickOP;
							}
						}
					}
			...
			cbit.vcell.client.PopupGenerator
					.showErrorDialog("Error while picking Analytic AreaOfInterest\n"
							+ e.getClass().getName() + "\n" + e.getMessage());
		}
	...
	private void pickByRange(cbit.util.Range pickRange) {
		...
		String surfSelect = (String) cbit.vcell.client.PopupGenerator
				.showListDialog(this, surfNames, "Pick Surface");

		boolean pickOP = (dsr.getDSRState().selectionType == cbit.vcell.client.data.DataSelectRetrieve.SELECT_TYPE_ADD ? true
				: false);
		...
		for (int surf = 0; surf < surfNames.length; surf += 1) {
			if (surfNames[surf].equals(surfSelect)) {
				int polygonCount = getSurfaceCollectionDataInfo()
						...
						.getPolygonCount();
				for (int i = 0; i < polygonCount; i += 1) {
					double value = getSurfaceCollectionDataInfoProvider()
							.getValue(surf, i);
					if (value >= pickRange.getMin()
							&& value <= pickRange.getMax()) {
						bAOI[surf][i] = pickOP;
					}
				}
			}
					...
					for (int k = 0; k < pickedNodes.length; k += 1) {
						if (surfNodes[j].equals(pickedNodes[k])) {
							...
							if (newNeighbors.length >= newNeighborCount) {
								...
								System.arraycopy(newNeighbors, 0, temp, 0,
										newNeighborCount);
								newNeighbors = temp;
							}
	...
	private void pickPolygon(java.awt.event.MouseEvent mouseEvent) {
		...
		if ((mouseEvent.getModifiers() & java.awt.event.InputEvent.CTRL_MASK) == java.awt.event.InputEvent.CTRL_MASK) {
			...
			SurfaceCanvas.SurfaceCollectionPick lastPick = getSurfaceCanvas1()
					.pickPolygon(mouseEvent.getX(), mouseEvent.getY());
			if (lastPick != null) {
				if (dsrState.selectionMode == cbit.vcell.client.data.DataSelectRetrieve.SELECT_MODE_AREA_RANGE) {
					if (selectRangeMinMaxFlag) {
						dsr.setSelectMax(getSurfaceCollectionDataInfoProvider()
								.getValue(lastPick.surfaceIndex,
										lastPick.polygonIndex));
					} else {
						dsr.setSelectMin(getSurfaceCollectionDataInfoProvider()
								.getValue(lastPick.surfaceIndex,
										lastPick.polygonIndex));
					}
					...
					// SurfaceCanvas.SurfaceCollectionPick lastPick =
					// getSurfaceCanvas1().pickPolygon(mouseEvent.getX(),mouseEvent.getY());
					// if(lastPick != null){
					SurfaceCollection sc = getSurfaceCollectionDataInfo()
					...
					for (int i = 0; i < dsrState.selectAreaRadius; i += 1) {// Find
																			// all
																			// neighbors
																			// around
																			// target
					...
					if (!pickOP) {
						int selectedIndex = -1;
						for (int i = 0; i < polylinePicks.size(); i += 1) {
							SurfaceCanvas.SurfaceCollectionPick ppElement = (SurfaceCanvas.SurfaceCollectionPick) polylinePicks
									.elementAt(i);
							if (ppElement.surfaceIndex == lastPick.surfaceIndex
									&& ppElement.polygonIndex == lastPick.polygonIndex) {
								selectedIndex = i;
								break;
							}
						}
						...
						if (polylinePicks.size() == 0) {
							...
							int prevPickPolyIndex = ((SurfaceCanvas.SurfaceCollectionPick) polylinePicks
									.lastElement()).polygonIndex;
							SurfaceCollection sc = getSurfaceCollectionDataInfo()
									.getSurfaceCollection();
							Surface surf = sc
									.getSurfaces(lastPick.surfaceIndex);
							Vect3d destination = ((Quadrilateral) surf
									.getPolygons(lastPick.polygonIndex))
									.calculateCentroid();
							boolean[] bPicked = new boolean[surf
									.getPolygonCount()];
							while (true) {
								java.util.Arrays.fill(bPicked, false);
								bPicked[prevPickPolyIndex] = true;
								int[] neighbors = pickNeighbors(surf,
										new int[] { prevPickPolyIndex },
										bPicked);
								boolean bDone = false;
								...
								Vect3d vertex = ((Quadrilateral) surf
										.getPolygons(prevPickPolyIndex))
										.calculateCentroid();
								for (int i = 0; i < neighbors.length; i += 1) {
									Vect3d neighborCoord = ((Quadrilateral) surf
											.getPolygons(neighbors[i]))
											.calculateCentroid();
									double angle = cbit.vcell.geometry.Curve
											.getAngle(new Coordinate(
													vertex.q[0], vertex.q[1],
													vertex.q[2]),
													new Coordinate(
															destination.q[0],
															destination.q[1],
								...
								polylinePicks
										.add(new SurfaceCanvas.SurfaceCollectionPick(
												lastPick.surfaceIndex,
												lowAnglePolyIndex));
	...
	private void refresh() {
		...
		if (getSurfaceCollectionDataInfo() == null) {
			...
			if (getSurfaceCollectionDataInfo().getSurfaceCollection() != null) {
				bAOI = new boolean[getSurfaceCollectionDataInfo()
						.getSurfaceCollection().getSurfaceCount()][];
				for (int i = 0; i < bAOI.length; i += 1) {
					bAOI[i] = new boolean[getSurfaceCollectionDataInfo()
							.getSurfaceCollection().getSurfaces(i)
							.getPolygonCount()];
					java.util.Arrays.fill(bAOI[i], false);
				}
			}
			dsr.setSelectAreaRegionNames(getSurfaceCollectionDataInfo()
					.getSurfaceNames());
		}

		bDisplay = (getSurfaceCollectionDataInfo() != null ? new boolean[getSurfaceCollectionDataInfo()
				.getSurfaceCollection().getSurfaceCount()]
				: null);
		if (bDisplay != null) {
			java.util.Arrays.fill(bDisplay, true);
		}
		bWireframe = (getSurfaceCollectionDataInfo() != null ? new boolean[getSurfaceCollectionDataInfo()
				.getSurfaceCollection().getSurfaceCount()]
				: null);
		if (bWireframe != null) {
			java.util.Arrays.fill(bWireframe, false);
		}

		// init with NO transparency
		transparentSelection = (getSurfaceCollectionDataInfo() != null ? new java.awt.AlphaComposite[getSurfaceCollectionDataInfo()
				.getSurfaceCollection().getSurfaceCount()]
				: null);
		if (transparentSelection != null) {
			java.util.Arrays.fill(transparentSelection, java.awt.AlphaComposite
					.getInstance(java.awt.AlphaComposite.SRC));
		}

		getScrollPaneTable().repaint();

		getSurfaceCanvas1().setOrigin(
				getSurfaceCollectionDataInfo().getSurfaceCollectionOrigin());
		getSurfaceCanvas1().setExtent(
				getSurfaceCollectionDataInfo().getSurfaceCollectionExtent());
		getSurfaceCanvas1().setSurfaceCollection(
				getSurfaceCollectionDataInfo().getSurfaceCollection());
		getSurfaceCanvas1().setSurfacesShowing(bDisplay);
		getSurfaceCanvas1().setSurfacesWireframe(bWireframe);
		getSurfaceCanvas1()
				.setWireFrameThickness(
						(getSurfaceCollectionDataInfo().getDimension() < 3 ? 2.0
								: 1.0));
		...
		if (getSurfaceCollectionDataInfoProvider() != null) {
			getSurfaceCanvas1().setSurfacesColors(
					getSurfaceCollectionDataInfoProvider()
							.getSurfacePolygonColors());
		}

	}

	/**
	 * Insert the method's description here. Creation date: (9/24/2005 12:45:17
	 * PM)
	 */
	private void setModeCursor() {
		...
		if (getSurfaceViewerTool1().isZooming()) {
			if (getSurfaceCanvas1().getCursor() == null
					|| getSurfaceCanvas1().getCursor().getType() != java.awt.Cursor.NE_RESIZE_CURSOR) {
				getSurfaceCanvas1()
						.setCursor(
								java.awt.Cursor
										.getPredefinedCursor(java.awt.Cursor.NE_RESIZE_CURSOR));
			}
		} else if (getSurfaceViewerTool1().isRotating()) {
			if (getSurfaceCanvas1().getCursor() == null
					|| getSurfaceCanvas1().getCursor().getType() != java.awt.Cursor.MOVE_CURSOR) {
				getSurfaceCanvas1()
						.setCursor(
								java.awt.Cursor
			...
			if (getSurfaceCanvas1().getCursor() == null
					|| getSurfaceCanvas1().getCursor().getType() != java.awt.Cursor.HAND_CURSOR) {
				getSurfaceCanvas1()
						.setCursor(
								java.awt.Cursor
	 ...
	 * 
	 * @param surfaceCollectionDataInfo
	 *            The new value for the property.
	 * @see #getSurfaceCollectionDataInfo
	 */
	private void setSurfaceCollectionDataInfo(
			SurfaceCollectionDataInfo surfaceCollectionDataInfo) {
		...
		firePropertyChange("surfaceCollectionDataInfo", oldValue,
				surfaceCollectionDataInfo);
	}

	/**
	 * Sets the surfaceCollectionDataInfoProvider property (java.lang.Object)
	 * value.
	 * 
	 * @param surfaceCollectionDataInfoProvider
	 *            The new value for the property.
	 * @see #getSurfaceCollectionDataInfoProvider
	 */
	public void setSurfaceCollectionDataInfoProvider(
			SurfaceCollectionDataInfoProvider surfaceCollectionDataInfoProvider) {
		...
		if (fieldSurfaceCollectionDataInfoProvider != null) {
			getSurfaceCanvas1().setSurfacesColors(
					fieldSurfaceCollectionDataInfoProvider
							.getSurfacePolygonColors());
			getSurfaceCanvas1().setAOIHighlightColor(
					fieldSurfaceCollectionDataInfoProvider
							.getROIHighlightColor());
			getSurfaceViewerTool1().fullRepaint();
			// getSurfaceCanvas1().repaint();
		}

		firePropertyChange("surfaceCollectionDataInfoProvider", oldValue,
				surfaceCollectionDataInfoProvider);
	}
	...
	private void surfaceViewerTool1_ViewAngleRadians(Vect3d arg1) {
		...
		getJTextFieldXViewAngle().setText(
				df.format(arg1.getX() * 180 / Math.PI));
		getJTextFieldYViewAngle().setText(
				df.format(arg1.getY() * 180 / Math.PI));
		getJTextFieldZViewAngle().setText(
				df.format(arg1.getZ() * 180 / Math.PI));
	}

	/**
	 * Insert the method's description here. Creation date: (2/16/2006 11:33:51
	 * AM)
	 */
	private void updateAreaOfInterest() {
		...
		if (polylinePicks.size() > 0) {
			for (int i = 0; i < polylinePicks.size(); i += 1) {
				SurfaceCanvas.SurfaceCollectionPick lineElement = (SurfaceCanvas.SurfaceCollectionPick) polylinePicks
						.elementAt(i);
		...
		updatePickInfoDisplay();
		getSurfaceCanvas1().setAOIHighlightColor(
				getSurfaceCollectionDataInfoProvider().getROIHighlightColor());
		getSurfaceCanvas1().setAreaOfInterest(compositeAOI);
		getSurfaceViewerTool1().fullRepaint();
	}

	/**
	 * Insert the method's description here. Creation date: (10/2/2005 8:12:14
	 * AM)
	 */
	private void updatePickInfoDisplay() {
		...
		if (getSurfaceCollectionDataInfoProvider() != null) {
			...
			if (lastMouse != null) {
				currentPick = getSurfaceCanvas1().pickPolygon(lastMouse.getX(),
						lastMouse.getY());
			}
			if (currentPick != null) {
				Vect3d centroid = ((Quadrilateral) getSurfaceCollectionDataInfo()
						.getSurfaceCollection().getSurfaces(
								currentPick.surfaceIndex).getPolygons(
								currentPick.polygonIndex)).calculateCentroid();
				df.setMaximumFractionDigits(5);
				double area = getSurfaceCollectionDataInfoProvider().getArea(
						currentPick.surfaceIndex, currentPick.polygonIndex);
				getJLabelInfo()
						.setText(
								" ("
										+ getSurfaceCollectionDataInfoProvider()
												.getValueDescription(
														currentPick.surfaceIndex,
														currentPick.polygonIndex)
										+ "["
										+ getSurfaceCollectionDataInfoProvider()
												.getMembraneIndex(
														currentPick.surfaceIndex,
										...
										+ ")"
										+ " val= "
										+ (float) getSurfaceCollectionDataInfoProvider()
												.getValue(
														currentPick.surfaceIndex,
														currentPick.polygonIndex)
										+ " area="
										+ (area != cbit.vcell.mesh.MembraneElement.AREA_UNDEFINED ? area
												+ ""
												: "No Calc") + " centroid=("
										+ df.format(centroid.getX()) + ","
										+ df.format(centroid.getY()) + ","
										+ df.format(centroid.getZ()) + ")");
				return;
			} else {
				String aoiString = " ";
				int count = 0;
				...
				if (bAOI != null) {
					for (int i = 0; i < bAOI.length; i += 1) {
						for (int j = 0; j < bAOI[i].length; j += 1) {
							if (bAOI[i][j]) {
								count += 1;
								double area = getSurfaceCollectionDataInfoProvider()
										.getArea(i, j);
								...
								double val = getSurfaceCollectionDataInfoProvider()
										.getValue(i, j);
								mean += val;
								if (val < min) {
									min = val;
				...
				if (count > 0) {
					aoiString = "AOI("
							+ count
							+ ") Area="
							+ (isAreaUndefined ? "No Calc" : ""
									+ (float) aoiArea) + " WMEAN="
							+ (isAreaUndefined ? "No Calc" : "" + wmean)
				...
				if (polylinePicks.size() > 0) {
					...
					for (int i = 0; i < polylinePicks.size(); i += 1) {
						SurfaceCanvas.SurfaceCollectionPick lineElement = (SurfaceCanvas.SurfaceCollectionPick) polylinePicks
								.elementAt(i);
						Quadrilateral quad = (Quadrilateral) getSurfaceCollectionDataInfo()
								.getSurfaceCollection().getSurfaces(
										lineElement.surfaceIndex).getPolygons(
										lineElement.polygonIndex);
					...
					lineString = "Line(" + polylinePicks.size() + ") Len="
							+ (float) lineLength;
				}
				getJLabelInfo()
						.setText(
