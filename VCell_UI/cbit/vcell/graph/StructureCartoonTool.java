...
import cbit.vcell.model.render.FeatureShape;
import cbit.vcell.model.render.MembraneShape;
import cbit.vcell.model.render.SpeciesContextShape;
import cbit.vcell.model.render.StructureCartoon;
import cbit.vcell.model.render.StructureShape;
				...
				if(message.length() > 0){
					String result =
						cbit.vcell.client.PopupGenerator.showWarningDialog(getJDesktopPane(),
							...
							message,
							new String[] {cbit.vcell.client.server.UserMessage.OPTION_YES,cbit.vcell.client.server.UserMessage.OPTION_NO},
							cbit.vcell.client.server.UserMessage.OPTION_NO);
					if(result.equals(cbit.vcell.client.server.UserMessage.OPTION_YES)){
						for(int i=0;i<cleanupWantedV.size();i+= 1){
							try{
...
//	tabbedPane.insertTab("Tree View", null, aNameScopeDisplayPanel, null, 2);
	...
	org.vcell.util.BeanUtils.centerOnComponent(internalFrame,getJDesktopPane());
	internalFrame.show();
}
